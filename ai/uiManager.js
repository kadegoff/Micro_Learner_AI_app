...
function updateAuthDropdown() {
  // Update both the main chat controls and edit section dropdowns
  const containers = document.querySelectorAll(".chat-controls");
  ...
  containers.forEach((container) => {
    ...
    if (!appState.isAuthenticated) {
      ...
      // Check if this is an edit container by looking at parent structure
      const isEditContainer = container.closest(".message-edit-container");

      if (isEditContainer) {
        // For edit section - only update the dropdown, preserve other elements
        const dropdownContainer = container.querySelector(
          ".ai-dropdown-container"
        );
        if (dropdownContainer) {
          dropdownContainer.innerHTML = aiSection;
        } else {
          // If dropdown container doesn't exist, create it
          const newDropdown = document.createElement("div");
          newDropdown.className = "ai-dropdown-container";
          newDropdown.innerHTML = aiSection;

          // Insert the dropdown before the message-edit-actions
          const actionsContainer = container.querySelector(
            ".message-edit-actions"
          );
          if (actionsContainer) {
            container.insertBefore(newDropdown, actionsContainer);
          } else {
            container.prepend(newDropdown);
          }
        }
      } else {
        // Regular chat controls when authenticated
        const isProcessing = chatManager.currentStreamMessageId !== null;
        const buttonClass = isProcessing
          ? "send-button processing"
          : "send-button";
        const buttonTitle = isProcessing ? "Stop processing" : "Send message";

        // Token limit indicator HTML
        const tokenIndicatorHTML = `<div id="token-limit-indicator" class="token-indicator" style="display: none; padding: 5px; text-align: center"></div>`;

        container.innerHTML = `${aiSection}
                ${tokenIndicatorHTML}
                <button class="${buttonClass}" id="sendButton" title="${buttonTitle}">
        ...
        sendButton?.addEventListener("click", function () {
          ...
          if (chatManager.currentStreamMessageId) {
            if ($DebugTestMode) {
              console.log(
                "🧭 ❌ Cannot send message while processing a message"
              );
            }
...
function toggleDropdown(button) {
  // Find the specific menu associated with this button
  const menu = button.nextElementSibling;

  if (menu && menu.id === "dropdownMenu") {
...
`;
  ...
  document.addEventListener("click", function (e) {
    ...
    if (dropdownButton) {
      // Toggle only the dropdown associated with the clicked button
      toggleDropdown(dropdownButton);
    } else {
      // Close all dropdowns if clicking outside
      let shouldCloseAll = true;

      // Check if we're clicking inside any dropdown menu
      dropdownMenus.forEach((menu) => {
        if (e.target.closest("#dropdownMenu") === menu) {
