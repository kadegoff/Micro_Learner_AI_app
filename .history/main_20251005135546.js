...
console.log("Directory exists?", fs.existsSync(logPath));
...
try {
  fs.appendFileSync(
    logFile,
    `[${new Date().toISOString()}] Testing log file\n`
  );
...
logToFile("testing where these console logs go");
...
function getSafePath(conversationId, fileName) {
  ...
  if (!/^[a-zA-Z0-9_.-]+$/.test(fileName)) {
    logToFile("Problematic filename:", JSON.stringify(fileName));
    logToFile(
      "Character codes:",
...
ipcMain.handle(
  ...
  async (event, fileName, content, conversationId) => {
    try {
      ...
      logToFile("Save file error:", error);
...
ipcMain.handle("get-file", async (event, fileName, conversationId) => {
  try {
    ...
    logToFile("Get file error:", error);
    return { success: false, error: error.message };
  }
});
...
ipcMain.handle("get-file-preview", async (event, fileName, conversationId) => {
  try {
    ...
    logToFile("Get file preview error:", error);
    return { success: false, error: error.message };
  }
});
...
// 🔥 FIX: Monkey patch PythonShell to hide console windows
let PythonShell;
try {
  const pythonShellModule = require("python-shell");

  // Store the original _begin method
  const originalBegin = pythonShellModule.PythonShell.prototype._begin;

  // Override the _begin method to force windowsHide
  pythonShellModule.PythonShell.prototype._begin = function (callback) {
    // Ensure windowsHide is set for Windows
    if (process.platform === "win32") {
      if (!this.options) this.options = {};
      this.options.windowsHide = true;

      // Also ensure stdio uses pipes
    ...
    return originalBegin.call(this, callback);
  };

  // Also patch the static run method
  const originalRun = pythonShellModule.PythonShell.run;
  pythonShellModule.PythonShell.run = function (script, options, callback) {
    if (process.platform === "win32") {
      options = options || {};
      options.windowsHide = true;
      options.stdio = options.stdio || ["pipe", "pipe", "pipe"];
    }
    return originalRun(script, options, callback);
  };

  PythonShell = pythonShellModule.PythonShell;

  if ($DebugTestMode) {
    logToFile("PythonShell loaded successfully with console hiding patch");
  }
} catch (error) {
  if ($DebugTestMode) {
    logToFile("Failed to load python-shell:", error);
  }
}
// Windows
let controlWindow;
...
async function downloadLanguageToolOnStartup() {
  if (!languageToolManager) {
    if ($DebugTestMode) {
      logToFile("❌ LanguageTool manager not initialized");
    }
    return false;
  }
  ...
  if ($DebugTestMode) {
    logToFile("🔍 Double-checking LanguageTool installation status:");
    logToFile("  - languageToolInstalled flag:", languageToolInstalled);
    logToFile("  - actualStatus.installed:", isActuallyInstalled);
    logToFile("  - Full status:", actualStatus);
  }

  // Use the actual status, not just the flag
  if (isActuallyInstalled) {
    if ($DebugTestMode) {
      logToFile("✅ LanguageTool already installed, skipping download");
    }
  ...
  try {
    if ($DebugTestMode) {
      logToFile("📥 Starting LanguageTool download during app startup...");
    }

    // Send progress updates to all windows
    const progressCallback = (progress) => {
      if ($DebugTestMode) {
        logToFile("📊 LanguageTool download progress:", progress);
      }
    ...
    if ($DebugTestMode) {
      logToFile("✅ LanguageTool download and installation completed");
    }
    ...
    if ($DebugTestMode) {
      logToFile("❌ LanguageTool download failed:", error);
    }
...
async function startLanguageToolServer() {
  ...
  if (!languageToolInstalled) {
    if ($DebugTestMode) {
      logToFile("⚠️ LanguageTool not installed, cannot start server");
    }
    return false;
  }
  ...
  if (languageToolReady) {
    if ($DebugTestMode) {
      logToFile("✅ LanguageTool server already running");
    }
    return true;
  }
  ...
  try {
    if ($DebugTestMode) {
      logToFile("🚀 Starting LanguageTool server...");
    }
    ...
    if ($DebugTestMode) {
      logToFile("✅ LanguageTool server started successfully");
    }
    ...
    if ($DebugTestMode) {
      logToFile("❌ LanguageTool server start failed:", error);
    }
...
async function correctTextWithLanguageTool(text) {
  if (!languageToolManager || !languageToolReady) {
    if ($DebugTestMode) {
      logToFile("⚠️ LanguageTool not ready, using basic correction");
    }
  ...
  try {
    ...
    if (response.ok) {
      ...
      if ($DebugTestMode) {
        logToFile("⚠️ LanguageTool API error, falling back to NLP correction");
      }
...
async function initializeCompromise() {
  return new Promise((resolve) => {
    if ($DebugTestMode) {
      logToFile("Initializing compromise NLP...");
    }
    ...
    try {
      ...
      nlpReady = true;
      if ($DebugTestMode) {
        logToFile("Compromise NLP initialized in main process");
      }
      resolve(true);
    } catch (error) {
      if ($DebugTestMode) {
        logToFile("Failed to initialize compromise:", error);
      }
      resolve(false);
    }
  });
...
async function initializeLanguageTool() {
  if ($DebugTestMode) {
    logToFile("🔧 Initializing LanguageTool manager...");
  }
  ...
  try {
    ...
    if ($DebugTestMode) {
      logToFile("🔍 LanguageTool actual status:", status);
      logToFile("🔧 languageToolInstalled flag set to:", languageToolInstalled);
      logToFile("🔧 languageToolReady flag set to:", languageToolReady);
    }

    return true;
  } catch (error) {
    if ($DebugTestMode) {
      logToFile("❌ Failed to initialize LanguageTool manager:", error);
    }
    return false;
  }
}
...
async function correctText(text) {
  ...
  try {
    ...
    if ($DebugTestMode) {
      logToFile("🔤 Original text:", correctedText);
    }
    ...
    try {
      ...
      if ($DebugTestMode && !ltError.name?.includes("Abort")) {
        logToFile("LanguageTool not available:", ltError.message);
      }
    }
    ...
    if (text !== finalText && $DebugTestMode) {
      logToFile(`🔤 Text correction: "${text}" -> "${finalText}"`);
    }

    return finalText;
  } catch (error) {
    if ($DebugTestMode) {
      logToFile("Text correction error:", error);
    }
    return String(text || "");
  }
}
...
function loadWindowStates() {
  try {
    ...
    if (fs.existsSync(statesPath)) {
      ...
      Object.keys(savedStates).forEach((key) => {
        if (savedStates[key] && typeof savedStates[key] === "object") {
          ...
          if (
            ...
            if ($DebugTestMode) {
              logToFile(`Invalid saved state for ${key}, will use defaults`);
            }
          }
        }
      });
      ...
      if ($DebugTestMode) {
        logToFile("Window states loaded successfully:", windowStates);
      }
    } else {
      if ($DebugTestMode) {
        logToFile("No saved window states found, will use defaults");
      }
    }
  } catch (error) {
    if ($DebugTestMode) {
      logToFile("Failed to load window states:", error);
      logToFile("Will use default window positions");
    }
  }
}
...
function saveWindowStates() {
  try {
    ...
    if ($DebugTestMode) {
      logToFile("Window states saved successfully:", finalStates);
    }
  } catch (error) {
    if ($DebugTestMode) {
      logToFile("Failed to save window states:", error);
    }
  }
}
...
function createWindows() {
  ...
  function getValidWindowState(windowName, defaultState) {
    ...
    if (!saved || typeof saved.x !== "number" || typeof saved.y !== "number") {
      if ($DebugTestMode) {
        logToFile(`No saved state for ${windowName}, using default position`);
      }
      return defaultState;
    }
    ...
    if ($DebugTestMode) {
      logToFile(`Using saved state for ${windowName}:`, saved);
    }
    return {
      x: validX,
      y: validY,
  ...
  logToFile("Width:", controlState.width);
  logToFile("Height:", controlState.height);
  ...
  if ($DebugTestMode) {
    logToFile("🎛️ === CONTROL WINDOW CREATED ===");
    logToFile(`🎛️ Width: ${controlState.width}px`);
    logToFile(`🎛️ Height: ${controlState.height}px`);
    logToFile(`🎛️ Position: (${controlState.x}, ${controlState.y})`);
    logToFile(`🎛️ Size: ${controlState.width}x${controlState.height}`);
    logToFile("🎛️ ================================");
  }
  ...
  function onWindowReady() {
    ...
    if (windowsReadyCount === totalWindows) {
      ...
      setTimeout(() => {
        ...
        if ($DebugTestMode) {
          logToFile("All windows ready and shown");
        }
  ...
  controlWindow.once("ready-to-show", () => {
    if ($DebugTestMode) {
      ...
      logToFile("🎛️ === CONTROL WINDOW READY-TO-SHOW ===");
      logToFile(`🎛️ Actual Width: ${actualBounds.width}px`);
      logToFile(`🎛️ Actual Height: ${actualBounds.height}px`);
      logToFile(`🎛️ Actual Position: (${actualBounds.x}, ${actualBounds.y})`);
      logToFile(`🎛️ Actual Size: ${actualBounds.width}x${actualBounds.height}`);
      logToFile("🎛️ ====================================");
    }
  ...
  controlWindow.once("show", () => {
    if ($DebugTestMode) {
      ...
      logToFile("🎛️ === CONTROL WINDOW SHOWN ===");
      logToFile(`🎛️ Final Width: ${showBounds.width}px`);
      logToFile(`🎛️ Final Height: ${showBounds.height}px`);
      logToFile(`🎛️ Final Position: (${showBounds.x}, ${showBounds.y})`);
      logToFile(`🎛️ Final Size: ${showBounds.width}x${showBounds.height}`);
      logToFile("🎛️ ==============================");
    }
  });
  ...
  aiWindow.once("ready-to-show", () => {
    ...
    if ($DebugTestMode) {
      logToFile("✅ AI Window ready to show");
      const bounds = aiWindow.getBounds();
      logToFile(
  ...
  [controlWindow, transcriptWindow, aiWindow].forEach((win) => {
    win.webContents.once("dom-ready", () => {
      if (windowsReady && !authStateUpdateInProgress) {
        if ($DebugTestMode) {
          logToFile("Window DOM ready, sending auth state");
        }
  ...
  if ($DebugTestMode) {
    logToFile("Windows created with preserved positions:");
    logToFile(
...
function setupWindowEventListeners() {
      ...
      if (!win.isDestroyed() && startupComplete) {
        ...
        if (windowName === "transcript" || windowName === "ai") {
          ...
          if ($DebugTestMode && wasSnapped) {
            logToFile(`✅ ${windowName} was snapped, getting final bounds...`);
          }
        }
  ...
  app.on("before-quit", () => {
    if ($DebugTestMode) {
      logToFile("App closing, saving final window states...");
    }
    saveWindowStates();
  });
}
...
function snapWindowToCorner(windowName, corner) {
  ...
  if ($DebugTestMode) {
    logToFile(`Snapped ${windowName} to ${corner}`);
  }
}
...
function setupCornerSnapHandlers() {
  ipcMain.handle("snap-to-corner", (event, windowName, corner) => {
    try {
      ...
      if ($DebugTestMode) {
        logToFile("Error snapping to corner:", error);
      }
      return { success: false, error: error.message };
    }
  });
  ...
  try {
    ...
    if ($DebugTestMode) {
      logToFile("Error registering corner snap shortcuts:", error);
    }
  }
}
...
function registerGlobalShortcuts() {
  try {
    ...
    globalShortcut.register("Control+Shift+M", () => {
      [transcriptWindow, aiWindow].forEach((win, index) => {
        if (win && !win.isDestroyed()) {
          try {
            ...
            if ($DebugTestMode) {
              logToFile(`Error showing window ${index}:`, error);
            }
          }
        }
      });
      ...
      if (controlWindow && !controlWindow.isDestroyed()) {
        try {
          ...
          if ($DebugTestMode) {
            logToFile("Error showing control window:", error);
          }
        }
      }
    });
    ...
    if ($DebugTestMode) {
      logToFile("Global shortcuts registered successfully");
    }
  } catch (error) {
    if ($DebugTestMode) {
      logToFile("Error registering global shortcuts:", error);
    }
  }
}
...
function safeSendToWindow(window, channel, data = null) {
  if (!window || window.isDestroyed()) {
    if ($DebugTestMode) {
      logToFile(`Window is destroyed, cannot send ${channel}`);
    }
    return false;
  }
  ...
  if (!window.webContents || window.webContents.isDestroyed()) {
    if ($DebugTestMode) {
      logToFile(`WebContents is destroyed, cannot send ${channel}`);
    }
    return false;
  }
  ...
  try {
    ...
    if ($DebugTestMode) {
      logToFile(`Error sending ${channel} to window:`, error.message);
      logToFile("Original data:", data);
      logToFile("Data type:", typeof data);
      if (data && typeof data === "object") {
        logToFile("Object keys:", Object.keys(data));
      }
    }
    return false;
  }
...
function safeExecuteInWindow(window, script) {
  if (!window || window.isDestroyed()) {
    if ($DebugTestMode) {
      logToFile("Window is destroyed, cannot execute script");
    }
    return false;
  }
  ...
  if (!window.webContents || window.webContents.isDestroyed()) {
    if ($DebugTestMode) {
      logToFile("WebContents is destroyed, cannot execute script");
    }
    return false;
  }
  ...
  try {
    ...
    if ($DebugTestMode) {
      logToFile("Error executing script in window:", error.message);
    }
    return false;
  }
}
...
function setupIpcHandlers() {
  ...
  function safeRegisterListener(channel, handler) {
    try {
      ...
      logToFile(`Error registering listener for ${channel}:`, error);
    }
  }
  ...
  safeRegisterListener("broadcast-ai-thinking", (event) => {
    if ($DebugTestMode) {
      logToFile("Broadcasting AI thinking...");
    }
  ...
  safeRegisterListener("broadcast-transcript", (event, data) => {
    if ($DebugTestMode) {
      logToFile("Broadcasting transcript:", data);
    }
  ...
  safeRegisterListener("broadcast-ai-response", (event, data) => {
    if ($DebugTestMode) {
      logToFile("Broadcasting AI response:", data);
    }
  ...
  safeRegisterListener("send-chat-message", (event, data) => {
    if ($DebugTestMode) {
      logToFile("Chat message received in main:", data);
    }
  ...
  safeRegisterListener("minimize-all", () => {
    if ($DebugTestMode) {
      logToFile("Minimizing all windows...");
    }
    [controlWindow, transcriptWindow, aiWindow].forEach((win, index) => {
      if (win && !win.isDestroyed()) {
        try {
          win.minimize();
          if ($DebugTestMode) {
            logToFile(`Window ${index} minimized successfully`);
          }
        } catch (error) {
          if ($DebugTestMode) {
            logToFile(`Error minimizing window ${index}:`, error);
          }
        }
      }
    });
  ...
  safeRegisterListener("close-all", () => {
    if ($DebugTestMode) {
      logToFile("Close all windows requested");
    }
    ...
    try {
      ...
      if ($DebugTestMode) {
        logToFile("Error saving window states:", error);
      }
    }
    ...
    if (voskProcess) {
      try {
        ...
        voskProcess = null;
        if ($DebugTestMode) {
          logToFile("Vosk process terminated");
        }
      } catch (error) {
        if ($DebugTestMode) {
          logToFile("Error terminating Vosk:", error);
        }
      }
    }
    ...
    windows.forEach(({ window, name }) => {
      if (window && !window.isDestroyed()) {
        try {
          ...
          window.close();
          if ($DebugTestMode) {
            logToFile(`${name} window closed successfully`);
          }
        } catch (error) {
          if ($DebugTestMode) {
            logToFile(`Error closing ${name} window:`, error);
          }
        }
      }
    });
    ...
    setTimeout(() => {
      try {
        ...
        if ($DebugTestMode) {
          logToFile("Error quitting app:", error);
        }
  ...
  safeRegisterListener("open-sign-in", () => {
    try {
      ...
      if ($DebugTestMode) {
        logToFile("Error opening sign-in window:", error);
      }
    }
  });
  ...
  safeRegisterListener("transcript-question", (event, question) => {
    if ($DebugTestMode) {
      logToFile("Transcript question received:", question);
    }
  ...
  logToFile("🎵 REGISTERING send-audio-to-vosk listener");
  ...
  safeRegisterListener("send-audio-to-vosk", (event, audioBuffer) => {
    try {
      ...
      if (!audioBuffer || !(audioBuffer instanceof ArrayBuffer)) {
        logToFile("❌ MAIN: Expected ArrayBuffer, got:", typeof audioBuffer);
        return;
      }

      if (audioBuffer.byteLength === 0) {
        logToFile("❌ MAIN: Empty ArrayBuffer received");
        return;
      }
      ...
      if (
        ...
        logToFile("❌ MAIN: Vosk process not healthy");
        return;
      }
      ...
      logToFile(`✅ MAIN: Sent ${buffer.length} bytes to Vosk`);
    } catch (error) {
      logToFile("❌ MAIN: Error processing audio:", error);
    }
  });
  ...
  safeRegisterHandler(
    ...
    async (event, constraints = {}) => {
      logToFile("🎵 SAFE: System audio capture requested in main process");
      ...
      try {
        ...
        logToFile("🎵 Returning source list to renderer");
        ...
        logToFile("🎵 Main process system audio error:", error);
        return {
          success: false,
          error: error.message,
          sources: [],
  ...
  safeRegisterHandler("expand-control-window", async () => {
    if (controlWindow && !controlWindow.isDestroyed()) {
      try {
        ...
        if ($DebugTestMode) {
          logToFile("🎛️ Control window smoothly expanded to 80px (centered)");
        }

        return { success: true };
      } catch (error) {
        if ($DebugTestMode) {
          logToFile("❌ Error expanding control window:", error);
        }
        return { success: false, error: error.message };
      }
    }

    logToFile("⚠️ Control window not available or already destroyed");
    logToFile("🔍 Window status:", {
  ...
  safeRegisterHandler("collapse-control-window", async () => {
    if (controlWindow && !controlWindow.isDestroyed()) {
      try {
        ...
        if ($DebugTestMode) {
          logToFile("🎛️ Control window smoothly collapsed to 50px");
        }

        return { success: true };
      } catch (error) {
        if ($DebugTestMode) {
          logToFile("❌ Error collapsing control window:", error);
    ...
    if ($DebugTestMode) {
      logToFile("⚠️ Control window not available for collapsing");
      logToFile("Control window state:", {
        exists: !!controlWindow,
  ...
  safeRegisterHandler("enumerate-audio-sources", async (event) => {
    logToFile("🎵 Enumerating audio sources...");
    ...
    try {
      ...
      logToFile(`🎵 Found ${audioSources.length} audio sources`);
      return audioSources;
    } catch (error) {
      logToFile("🎵 Error enumerating audio sources:", error);
      return [];
    }
  });
  ...
  safeRegisterHandler(
    ...
    async (event, constraints) => {
      logToFile("🎵 Enhanced getDisplayMedia requested");
      logToFile("🎵 Constraints:", constraints);
      ...
      try {
        ...
        logToFile("🎵 Enhanced getDisplayMedia failed:", error);
        return {
          success: false,
          error: error.message,
        };
  ...
  safeRegisterHandler("minimize-window", (event) => {
    try {
      ...
      if ($DebugTestMode) {
        logToFile("Error minimizing window:", error);
      }
      return { success: false, error: error.message };
    }
  });
  ...
  safeRegisterHandler("maximize-window", (event) => {
    try {
      ...
      if ($DebugTestMode) {
        logToFile("Error maximizing window:", error);
      }
      return { success: false, error: error.message };
    }
  });
  ...
  safeRegisterHandler("restore-window", (event) => {
    try {
      ...
      if ($DebugTestMode) {
        logToFile("Error restoring window:", error);
      }
      return { success: false, error: error.message };
    }
  });
  ...
  safeRegisterHandler("close-window", (event) => {
    try {
      ...
      if ($DebugTestMode) {
        logToFile("Error closing window:", error);
      }
      return { success: false, error: error.message };
    }
  });
  ...
  safeRegisterHandler("toggle-always-on-top", (event) => {
    try {
      ...
      if ($DebugTestMode) {
        logToFile("Error toggling always on top:", error);
      }
      return { success: false, error: error.message };
    }
  });
  ...
  safeRegisterHandler("transcribe-with-vosk", async (event, audioData) => {
    ...
    try {
      ...
      if ($DebugTestMode) {
        logToFile("Vosk transcription error:", error);
      }
  ...
  safeRegisterHandler("test-ipc", async () => {
    if ($DebugTestMode) {
      logToFile("Main: test-ipc called");
    }
  ...
  safeRegisterHandler("init-vosk", async () => {
    if ($DebugTestMode) {
      logToFile("Main: init-vosk IPC handler called");
    }
    try {
      const result = await initializeVosk();
      if ($DebugTestMode) {
        logToFile("Main: init-vosk returning:", result);
      }
      return result;
    } catch (error) {
      if ($DebugTestMode) {
        logToFile("Main: init-vosk handler error:", error);
      }
      return {
        success: false,
  ...
  safeRegisterHandler("cleanup-vosk", async () => {
    try {
      ...
      if ($DebugTestMode) {
        logToFile("Vosk cleaned up");
      }
    } catch (error) {
      if ($DebugTestMode) {
        logToFile("Vosk cleanup error:", error);
      }
    }
  });
  ...
  safeRegisterHandler("get-transcript-words", async () => {
    try {
      if ($DebugTestMode) {
        logToFile("🎵 MAIN: Getting transcript words...");
      }
      ...
      (async () => {
        try {
          ...
          logToFile("Error in getTranscriptWords:", error);
          throw error;
        }
      })()
    `);
      ...
      if ($DebugTestMode) {
        ...
        if (words.length > 0) {
          logToFile("🎵 MAIN: Sample words:", words.slice(0, 5));
        }
      }
      ...
      if ($DebugTestMode) {
        logToFile("🎵 MAIN: ❌ Error getting transcript words:", error);
      }
      return {
        success: false,
        error: error.message,
  ...
  safeRegisterHandler("send-words-to-ai", async (event, words) => {
    try {
      if ($DebugTestMode) {
        logToFile("🎵 MAIN: Sending transcript words to AI...");
        logToFile("🎵 MAIN: Words received:", words);
      }
      ...
      (async () => {
        try {
          ...
          logToFile("Error in receiveTranscriptWordsAndSend:", error);
          throw error;
        }
      })()
    `);
      ...
      if ($DebugTestMode) {
        logToFile("🎵 MAIN: Transcript words sent to AI successfully");
        logToFile("🎵 MAIN: AI response:", result);
      }
      ...
      if ($DebugTestMode) {
        logToFile("🎵 MAIN: ❌ Error sending transcript words to AI:", error);
      }
      return {
        success: false,
        error: error.message,
  ...
  safeRegisterHandler("get-auth-state", async () => {
    if ($DebugTestMode) {
      logToFile("get-auth-state called, returning:", {
        isAuthenticated: authState.isAuthenticated,
        user: authState.user,
        token: authState.token,
      });
  ...
  safeRegisterHandler("logout", async () => {
    ...
    try {
      ...
      if ($DebugTestMode) {
        logToFile("Failed to remove auth file:", error);
      }
    }
  ...
  safeRegisterHandler("ensure-control-window-visible", async () => {
    if (controlWindow && !controlWindow.isDestroyed()) {
      try {
        ...
        if ($DebugTestMode) {
          logToFile("Control window restored and made visible");
        }

        return { success: true };
      } catch (error) {
        if ($DebugTestMode) {
          logToFile("Error ensuring control window visible:", error);
        }
        return { success: false, error: error.message };
      }
    }
  ...
  safeRegisterHandler("open-external-url", async (event, url) => {
    ...
    try {
      ...
      if ($DebugTestMode) {
        logToFile("✅ Successfully opened external URL:", url);
      }

      return { success: true, url: url };
    } catch (error) {
      if ($DebugTestMode) {
        logToFile("❌ Failed to open external URL:", error);
      }
      return { success: false, error: error.message };
    }
  });
  ...
  app.on("renderer-process-unresponsive", (event, webContents) => {
    logToFile("Renderer process unresponsive");
    const unresponsiveWindow = BrowserWindow.fromWebContents(webContents);
    if (unresponsiveWindow) {
      logToFile("Unresponsive window:", unresponsiveWindow.id);
    }
  });
  ...
  safeRegisterListener("auth-success", (event, userData) => {
    if ($DebugTestMode) {
      logToFile("🔐 Received auth success from login window:", userData);
    }
  ...
  safeRegisterHandler("start-languagetool", async () => {
    if ($DebugTestMode) {
      logToFile("🔧 IPC: Start LanguageTool server requested");
    }
    ...
    if (!languageToolInstalled) {
      if ($DebugTestMode) {
        logToFile("❌ LanguageTool not installed, cannot start server");
      }
      return {
        success: false,
        error:
  ...
  safeRegisterHandler("stop-languagetool", async () => {
    if ($DebugTestMode) {
      logToFile("🛑 IPC: Stop LanguageTool server requested");
    }
    ...
    if (!languageToolReady) {
      if ($DebugTestMode) {
        logToFile("✅ LanguageTool server already stopped");
      }
      return {
        success: true,
    ...
    try {
      if ($DebugTestMode) {
        logToFile("🛑 Stopping LanguageTool server...");
      }
      ...
      if ($DebugTestMode) {
        logToFile("✅ LanguageTool server stopped successfully");
      }
      ...
      if ($DebugTestMode) {
        logToFile("❌ LanguageTool server stop failed:", error);
      }
    ...
    async (event, text) => {
      try {
        ...
        if (!languageToolReady || !languageToolManager) {
          if ($DebugTestMode) {
            logToFile("🔧 LanguageTool not ready, returning original text");
          }
          return {
            success: true,
            text: text,
  ...
  safeRegisterHandler("get-screen-capture-source", async () => {
    try {
      if ($DebugTestMode) {
        logToFile("🎵 MAIN: Getting screen capture source for system audio...");
      }
      ...
      if ($DebugTestMode) {
        logToFile("🎵 MAIN: Found", sources.length, "screen sources");
      }
      ...
      if (primarySource) {
        if ($DebugTestMode) {
          ...
          logToFile("🎵 MAIN: Source ID:", primarySource.id);
        }
        ...
        if ($DebugTestMode) {
          logToFile("🎵 MAIN: ❌ No suitable screen source found");
        }
        return {
          success: false,
          error: "No screen sources available",
      ...
      if ($DebugTestMode) {
        logToFile("🎵 MAIN: ❌ Error getting screen capture source:", error);
      }
      return {
        success: false,
        error: error.message,
  ...
  safeRegisterHandler("get-screen-sources", async () => {
    try {
      if ($DebugTestMode) {
        logToFile("🎵 MAIN: Getting all screen sources...");
      }
      ...
      if ($DebugTestMode) {
        logToFile("🎵 MAIN: Found", sources.length, "total sources");
      }
      ...
      if ($DebugTestMode) {
        logToFile("🎵 MAIN: ❌ Error getting screen sources:", error);
      }
      return {
        success: false,
        error: error.message,
  ...
  safeRegisterHandler(
    ...
    async (event, constraints = {}) => {
      try {
        if ($DebugTestMode) {
          logToFile("🎵 MAIN: Native system audio capture requested");
          logToFile("🎵 MAIN: Constraints:", constraints);
        }
        ...
        if ($DebugTestMode) {
          logToFile("🎵 MAIN: Using source:", primarySource.name);
          logToFile("🎵 MAIN: Source ID:", primarySource.id);
        }
        ...
        if ($DebugTestMode) {
          logToFile("🎵 MAIN: ❌ Native capture failed:", error);
        }
        return {
          success: false,
          error: error.message,
  ...
  safeRegisterHandler("get-desktop-capturer-info", async () => {
    try {
      if ($DebugTestMode) {
        logToFile("🎵 MAIN: Getting desktop capturer info...");
      }
      ...
      if ($DebugTestMode) {
        logToFile("🎵 MAIN: Desktop capturer info:", info);
      }
      return info;
    } catch (error) {
      if ($DebugTestMode) {
        logToFile("🎵 MAIN: ❌ Error getting desktop capturer info:", error);
      }
      return {
        success: false,
        error: error.message,
  ...
  safeRegisterHandler("test-desktop-capturer", async () => {
    try {
      if ($DebugTestMode) {
        logToFile("🧪 MAIN: Testing desktop capturer...");
      }
      ...
      if ($DebugTestMode) {
        logToFile("🧪 MAIN: Desktop capturer test result:", result);
      }
      return result;
    } catch (error) {
      if ($DebugTestMode) {
        logToFile("🧪 MAIN: ❌ Desktop capturer test failed:", error);
      }
      return {
        success: false,
        available: false,
  ...
  safeRegisterHandler("debug-desktop-capturer", async () => {
    try {
      if ($DebugTestMode) {
        logToFile("🔍 === DESKTOP CAPTURER DEBUG INFO ===");
      }
      ...
      if ($DebugTestMode) {
        logToFile("🔍 Total sources found:", sources.length);

        // Log all sources
        sources.forEach((source, index) => {
          logToFile(`🔍 Source ${index + 1}:`, {
            id: source.id,
            name: source.name,
            display_id: source.display_id,
      ...
      if ($DebugTestMode) {
        logToFile("🔍 Screen sources:", screenSources.length);
      }
      ...
      if ($DebugTestMode) {
        logToFile("🔍 === END DEBUG INFO ===");
      }
      return debugInfo;
    } catch (error) {
      if ($DebugTestMode) {
        logToFile("🔍 ❌ Desktop capturer debug failed:", error);
      }
      return {
        success: false,
        error: error.message,
...
function monitorVoskProcess() {
  setInterval(() => {
    if (voskProcess && voskProcess.childProcess) {
      ...
      logToFile(`🏥 VOSK HEALTH: ${isHealthy ? "HEALTHY" : "UNHEALTHY"}`);

      if (!isHealthy) {
        logToFile("🔄 VOSK: Restarting unhealthy process...");
        try {
          if (voskProcess) {
            voskProcess.terminate();
          }
          voskProcess = startVoskProcess();
          logToFile("✅ VOSK: Process restarted");
        } catch (error) {
          logToFile("❌ VOSK: Restart failed:", error);
        }
      }
    } else {
      logToFile("⚠️ VOSK HEALTH: No process running");
    }
...
function createAuthWindow() {
  ...
  authWindow.webContents.on("dom-ready", () => {
    if ($DebugTestMode) {
      logToFile("Auth window loaded successfully");
...
function handleSuccessfulAuth(userData) {
  if ($DebugTestMode) {
    logToFile("🔐 === HANDLING SUCCESSFUL AUTH ===");
    logToFile("🔐 Auth data received:", userData);
  }
  ...
  windows.forEach((window, index) => {
    if (window && !window.isDestroyed()) {
      try {
        if ($DebugTestMode) {
          logToFile(`🔐 Sending auth-success to window ${index}:`, {
  ...
  if ($DebugTestMode) {
    logToFile("🔐 === AUTH SUCCESS HANDLING COMPLETE ===");
  }
}

function checkAndNotifyMaximizedState() {
  logToFile("🔍 checkAndNotifyMaximizedState called");

  try {
    // Get screen and window dimensions
    const screenSize = screen.getPrimaryDisplay().workAreaSize;
    logToFile(`🖥️ Screen work area: ${screenSize.width}x${screenSize.height}`);

    if (!aiWindow) {
      logToFile("❌ aiWindow is null or undefined");
      return;
    }

    if (aiWindow.isDestroyed()) {
      logToFile("⚠️ aiWindow is destroyed");
      return;
    }
    ...
    logToFile(`📏 Margin tolerance: ${margin}px`);

    const isMaximized = widthDiff <= margin && heightDiff <= margin;

    if (isMaximized) {
      logToFile("✅ Window detected as maximized");
      if ($DebugTestMode) {
        logToFile("📊 Debug details:");
        logToFile(`Screen: ${screenSize.width}x${screenSize.height}`);
        logToFile(`Window: ${windowBounds.width}x${windowBounds.height}`);
        logToFile(`Width difference: ${widthDiff}px (<= ${margin}px)`);
        logToFile(`Height difference: ${heightDiff}px (<= ${margin}px)`);
      }

      // Notify AI window that it's maximized
      logToFile("📤 Calling notifyAIWindowMaximized(true)");
      notifyAIWindowMaximized(true);
    } else {
      logToFile("❌ Window is not maximized");
      if ($DebugTestMode) {
        logToFile("📊 Debug details:");
        logToFile(`Screen: ${screenSize.width}x${screenSize.height}`);
        logToFile(`Window: ${windowBounds.width}x${windowBounds.height}`);
    ...
    logToFile("💥 Error in checkAndNotifyMaximizedState:", error);
  }

  logToFile("🏁 checkAndNotifyMaximizedState execution completed");
}

function notifyAIWindowMaximized(isMaximized) {
  logToFile(`notifyAIWindowMaximized called with isMaximized: ${isMaximized}`);

  try {
    if (aiWindow && !aiWindow.isDestroyed()) {
      logToFile("Sending window-maximized-changed message to AI window");
      aiWindow.webContents.send("window-maximized-changed", isMaximized);
    } else {
      if (!aiWindow) {
        logToFile("aiWindow is null or undefined");
      } else if (aiWindow.isDestroyed()) {
        logToFile("aiWindow is destroyed");
      }
    }
  } catch (error) {
    logToFile("Error in notifyAIWindowMaximized:", error);
  }

  logToFile("notifyAIWindowMaximized execution completed");
}
...
function saveAuthData(authData) {
  try {
    ...
    if ($DebugTestMode) {
      logToFile("Auth data saved locally");
    }
  } catch (error) {
    if ($DebugTestMode) {
      logToFile("Failed to save auth data:", error);
    }
  }
}
  ...
  try {
    ...
    if (fs.existsSync(authFilePath)) {
      ...
      if (authData.timestamp > oneDayAgo) {
        ...
        if ($DebugTestMode) {
          logToFile("Loaded cached auth data for:", authState.user.email);
        }
        return true;
      } else {
        if ($DebugTestMode) {
          logToFile("Cached auth data expired, removing...");
        }
        fs.unlinkSync(authFilePath);
      }
    }
    ...
    if ($DebugTestMode) {
      logToFile("Failed to load auth data:", error);
    }
  }
  return false;
}
...
function updateWindowsWithAuthState() {
  ...
  if ($DebugTestMode) {
    logToFile("🔐 === UPDATING ALL WINDOWS WITH AUTH STATE ===");
    logToFile("🔐 Auth data being sent:", {
  ...
  windows.forEach(({ window, name }) => {
    if (window && !window.isDestroyed()) {
      try {
        ...
        if ($DebugTestMode) {
          logToFile(`🔐 Failed to send auth state to ${name} window:`, error);
        }
      }
    }
  });
  ...
  if ($DebugTestMode) {
    logToFile("🔐 === FINISHED UPDATING ALL WINDOWS ===");
  }
...
async function initializeVosk() {
  if ($DebugTestMode) {
    logToFile("Main process: initializeVosk() called");
  }

  try {
    if ($DebugTestMode) {
      logToFile("Main process: Starting Python-Vosk bridge initialization...");
    ...
    if ($DebugTestMode) {
      logToFile("Checking models directory:", modelsPath);
      logToFile("Models directory exists?", fs.existsSync(modelsPath));
      logToFile("Vosk model exists?", fs.existsSync(modelPath));
    }
    ...
    if ($DebugTestMode) {
      logToFile("Checking if PythonShell is available...");
      logToFile("PythonShell type:", typeof PythonShell);
    }

    if (!PythonShell) {
      if ($DebugTestMode) {
        logToFile("PythonShell not available");
      }
      return {
        success: false,
    ...
    if (!fs.existsSync(testScriptPath)) {
      ...
      if ($DebugTestMode) {
        logToFile("Test script created at:", testScriptPath);
      }
    } else {
      if ($DebugTestMode) {
        logToFile("Test script already exists at:", testScriptPath);
      }
    }

    if ($DebugTestMode) {
      logToFile("Testing Python installation...");
    }
    ...
    return new Promise((resolve) => {
      ...
      pythonProcess.on("message", function (message) {
        if ($DebugTestMode) {
          logToFile("Python output:", message);
        }
        output.push(message);
      });
      ...
      pythonProcess.on("close", function () {
        if ($DebugTestMode) {
          logToFile("Python process closed");
        }
        ...
        if ($DebugTestMode) {
          logToFile("Python works, testing Vosk import...");
        }
...
`;
          ...
          if ($DebugTestMode) {
            logToFile("Vosk test script created at:", testVoskPath);
          }
        } else {
          if ($DebugTestMode) {
            logToFile("Vosk test script already exists at:", testVoskPath);
          }
        }
        ...
        voskProcess.on("message", function (message) {
          if ($DebugTestMode) {
            logToFile("Vosk test output:", message);
          }
        ...
        voskProcess.on("close", function () {
          ...
          if ($DebugTestMode) {
            logToFile("All tests passed, Vosk is ready");
          }
          voskReady = true;
          ...
          if (!fs.existsSync(realtimeScriptPath)) {
            ...
            if ($DebugTestMode) {
              logToFile("Realtime script created at:", realtimeScriptPath);
            }
          } else {
            if ($DebugTestMode) {
              logToFile(
    ...
    if ($DebugTestMode) {
      logToFile("Main process: Error in initializeVosk:", error);
    }
...
function startVoskProcess() {
  if ($DebugTestMode) {
    logToFile("startVoskProcess called");
  }

  if (voskProcess) {
    if ($DebugTestMode) {
      logToFile("Terminating existing Vosk process");
    }
    try {
      voskProcess.terminate();
    } catch (error) {
      if ($DebugTestMode) {
        logToFile("Error terminating existing Vosk process:", error);
      }
    }
  }
  ...
  if ($DebugTestMode) {
    logToFile("Starting Vosk with model:", modelPath);
  }
  ...
  try {
    voskProcess = new PythonShell("vosk_realtime.py", {
      ...
      scriptPath: __dirname,
      args: [modelPath],
      // ADD THESE OPTIONS TO HIDE CONSOLE WINDOWS:
      options: {
        detached: false, // Keep attached but hidden
        windowsHide: true, // ✅ CRITICAL: Hide Windows console windows
        stdio: ["pipe", "pipe", "pipe"], // Use pipes instead of console
      },
    });

    voskProcess.on("message", async (message) => {
      if ($DebugTestMode) {
        logToFile("Vosk message received:", message);
      }

      if (message === "VOSK_READY") {
        if ($DebugTestMode) {
          logToFile("Vosk process ready");
        }
        return;
      }
      ...
      try {
        ...
        if (result.type === "final") {
          if (
            ...
            if ($DebugTestMode) {
              logToFile("Duplicate Vosk result ignored:", result.text);
            }
        ...
        if ($DebugTestMode) {
          logToFile("Parsed Vosk result:", result);
        }
        ...
        if (result.type === "final" || result.type === "partial") {
          ...
          if (nlpReady && originalText.trim()) {
            try {
              ...
              if ($DebugTestMode) {
                logToFile("Error in text correction:", correctionError);
              }
          ...
          if ($DebugTestMode) {
            ...
            logToFile("Clean result object:", cleanResult);
          }
          ...
          if (!success && $DebugTestMode) {
            logToFile("Failed to send Vosk transcription to control window");
          }
          ...
          if ($DebugTestMode) {
            logToFile("Vosk error:", cleanError.error);
          }
          ...
          if (!success && $DebugTestMode) {
            logToFile("Failed to send Vosk error to control window");
          }
        }
      } catch (parseError) {
        if ($DebugTestMode) {
          logToFile("Failed to parse Vosk output:", parseError);
          logToFile("Raw message was:", message);
        }
        ...
        if (!success && $DebugTestMode) {
          logToFile("Failed to send parse error to control window");
        }
      }
    });
    ...
    voskProcess.on("error", (err) => {
      if ($DebugTestMode) {
        logToFile("Vosk process error:", err);
      }
    });

    voskProcess.on("close", () => {
      if ($DebugTestMode) {
        logToFile("Vosk process closed");
      }
      voskProcess = null;
    });
    ...
    voskProcess.on("stderr", (stderr) => {
      if ($DebugTestMode) {
        logToFile("Vosk stderr:", stderr);
      }
    });
  } catch (error) {
    if ($DebugTestMode) {
      logToFile("Error starting Vosk process:", error);
    }
    voskProcess = null;
  }
...
app.whenReady().then(async () => {
  ...
  session.defaultSession.setPermissionRequestHandler(
    (webContents, permission, callback) => {
      logToFile("🔐 Permission requested:", permission);
      ...
      if (allowedPermissions.includes(permission)) {
        callback(true);
        logToFile(`✅ GRANTED permission: ${permission}`);
      } else {
        logToFile(`❌ DENIED permission: ${permission}`);
        callback(false);
      }
    }
  );
  ...
  session.defaultSession.setDevicePermissionHandler((details) => {
    logToFile("🔐 Device permission requested:", details);
    ...
    if (allowedDevices.includes(details.deviceType)) {
      logToFile(`✅ GRANTED device permission: ${details.deviceType}`);
      return true;
    }

    logToFile(`❌ DENIED device permission: ${details.deviceType}`);
    return false;
  });

  // ✅ CRITICAL: Enhanced display media handler
  session.defaultSession.setDisplayMediaRequestHandler((request, callback) => {
    logToFile("🖥️ Display media requested:", request);
    ...
    logToFile("✅ GRANTED display media with audio");
  });

  // ✅ NEW: Permission check handler
  session.defaultSession.setPermissionCheckHandler(
    (webContents, permission, requestingOrigin) => {
      logToFile("🔍 Permission check:", permission, "from:", requestingOrigin);
      ...
      if (mediaPermissions.includes(permission)) {
        logToFile(`✅ ALLOWED permission check: ${permission}`);
        return true;
      }
  ...
  setTimeout(async () => {
    if (!languageToolInstalled) {
      ...
      downloadLanguageToolOnStartup().catch((error) => {
        if ($DebugTestMode) {
          logToFile("❌ Background LanguageTool download failed:", error);
        }
      });
    } else {
      if ($DebugTestMode) {
  ...
  setTimeout(() => {
    monitorVoskProcess();
    logToFile("🏥 Started Vosk process monitoring");
...
if ($DebugTestMode) {
  logToFile("Enhanced main.js loaded with improved window management");
}
...
function snapIfNearCorner(win, windowName) {
  ...
  if (corners.topLeft < threshold) {
    ...
    if ($DebugTestMode) {
      logToFile(`${windowName} snapped to top-left corner (0, 0)`);
    }
