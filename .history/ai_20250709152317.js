...
const AI_MODELS = {
  basic: [
    ...
    { id: "claude-3.5", name: "Claude 3.5(Creative Writing)", category: "basic" },
    { id: "deepseek-chat", name: "DeepSeek Chat", category: "basic" },
  ],
  advanced: [
    { id: "claude-4.0", name: "Claude 4.0", category: "advanced" },
    { id: "deepseek-r1", name: "DeepSeek R1(Math/Coding)", category: "advanced", default: true },
  ],
  fast: [{ id: "gemini-flash-2.5", name: "Gemini Flash 2.5", category: "fast" }],
};
...
var appState = {
  ...
  saveToStorage: function () {
    try {
      ...
      localStorage.setItem(CONFIG.STORAGE_KEYS.TOPICS, JSON.stringify(this.topics));
      localStorage.setItem(CONFIG.STORAGE_KEYS.SELECTED_MODEL, this.selectedAIModel);
      ...
      localStorage.setItem("messageResponses", JSON.stringify(this.messageResponses));

      // Save Context+ preference
      localStorage.setItem(CONFIG.STORAGE_KEYS.CONTEXT_PLUS, this.contextPlusEnabled);
  ...
  loadFromStorage: function () {
    try {
      ...
      const savedModel = localStorage.getItem(CONFIG.STORAGE_KEYS.SELECTED_MODEL);
      ...
      const savedContextPlus = localStorage.getItem(CONFIG.STORAGE_KEYS.CONTEXT_PLUS);
      ...
      if (savedHiddenMessages) this.hiddenMessages = new Set(JSON.parse(savedHiddenMessages));
  ...
  updateHiddenMessages: function (editedMessageId, versionTimestamp) {
    ...
    const messageIndex = this.chatHistory.findIndex((msg) => msg.id === editedMessageId);
    ...
    for (let i = messageIndex + 1; i < this.chatHistory.length; i++) {
      ...
      if (!this.messageResponses[editedMessageId][previousVersion].includes(msg.id)) {
  ...
  getMessagesForVersion: function (userMessageId, version) {
    if (!this.messageResponses[userMessageId] || !this.messageResponses[userMessageId][version]) {
      return [];
    }
  ...
  getContextForAI: function () {
    ...
    const currentTopicMessages = this.chatHistory.filter((msg) => {
      const matchesTopic = msg.topicId === this.currentTopicId || (!msg.topicId && !this.currentTopicId);
      ...
      if (isHidden) {
        console.log("🧠 Filtering out hidden message:", msg.id, msg.text.substring(0, 50));
      }

      return matchesTopic && !isErrorMessage && !isHidden;
    });

    console.log("🧠 Current topic messages after filtering:", currentTopicMessages.length);

    // Take the most recent messages up to the limit
    const recentMessages = currentTopicMessages.slice(-CONFIG.MAX_CONTEXT_MESSAGES);
    ...
    recentMessages.forEach((msg, index) => {
      ...
      const fileInfo = msg.files && msg.files.length ? `+ ${msg.files.length} files` : "no files";
      console.log(`  ${index}: ${msg.type} - "${msg.text.substring(0, 50)}..." ${fileInfo}`);
    });
    ...
    const contextMessages = recentMessages.map((msg) => {
      ...
      if (msg.files && Array.isArray(msg.files) && msg.files.length > 0) {
        console.log(`🧠 Including ${msg.files.length} files in context for message ${msg.id}`);
        ...
        msg.files.forEach((file, index) => {
          if (file && file.filename && file.content) {
            ...
            content += `\n**File ${index + 1}: ${file.filename}** (${file.type || "unknown type"})\n`;
            content += `\`\`\`${file.language || file.extension?.replace(".", "") || "text"}\n`;
            content += file.content; // ← FULL CONTENT
            content += `\n\`\`\`\n`;

            console.log(`🧠   - File: ${file.filename} (${file.content.length} chars, ${file.type})`);
          }
        });
      ...
      console.log(`🧠 Context message: ${role} - "${msg.text.substring(0, 50)}..."${msg.files && msg.files.length ? ` + ${msg.files.length} files` : ""}`);
    ...
    contextMessages.forEach((msg, idx) => {
      ...
      const hasFileStart = msg.content.includes("[Files created in this response:]");
      const hasFileEnd = msg.content.includes("[End of files]");
      const codeBlockCount = (msg.content.match(/```/g) || []).length;

      console.log(`  Has file markers: start=${hasFileStart}, end=${hasFileEnd}`);
      console.log(`  Code blocks: ${codeBlockCount / 2} (${codeBlockCount} markers)`);
...
var contentManager = {
  ...
  setupEventDelegation: function () {
    ...
    document.addEventListener("click", function (event) {
      console.log("🎯 CLICK DETECTED:", event.target.className, event.target.tagName);
  ...
  linkFilesToHTML: function (htmlContent, cssFiles, jsFiles) {
    ...
    cssFiles.forEach((cssFile) => {
      ...
      const cssLinkPatterns = [
        new RegExp(`<link[^>]*href\\s*=\\s*["']${cssFile.filename}["'][^>]*>`, "gi"),
        new RegExp(`<link[^>]*href\\s*=\\s*["'].*${cssFile.filename.replace(".css", "")}.*\\.css["'][^>]*>`, "gi"),
      ...
      cssLinkPatterns.forEach((pattern) => {
        if (pattern.test(linkedHTML)) {
          linkedHTML = linkedHTML.replace(pattern, `<style>\n${cssFile.content}\n</style>`);
          replaced = true;
    ...
    jsFiles.forEach((jsFile) => {
      ...
      const jsScriptPatterns = [
        new RegExp(`<script[^>]*src\\s*=\\s*["']${jsFile.filename}["'][^>]*></script>`, "gi"),
        new RegExp(`<script[^>]*src\\s*=\\s*["'].*${jsFile.filename.replace(".js", "")}.*\\.js["'][^>]*></script>`, "gi"),
        // Common generic names
      ...
      jsScriptPatterns.forEach((pattern) => {
        if (pattern.test(linkedHTML)) {
          linkedHTML = linkedHTML.replace(pattern, `<script>\n${jsFile.content}\n</script>`);
          replaced = true;
      ...
      if (!replaced && linkedHTML.includes("</body>")) {
        linkedHTML = linkedHTML.replace("</body>", `<script>\n/* Injected JavaScript from ${jsFile.filename} */\n${jsFile.content}\n</script>\n</body>`);
  ...
  previewContent: function (blockId) {
    ...
    console.log("🔍 Block type:", block.type, "isExecutable:", block.isExecutable, "isRenderable:", block.isRenderable);
...
var messageManager = {
  ...
  createMessage: function (text, type = "ai", messageType = "", model = null, addToHistory = true, timestamp = null, messageId = null, parentMessageId = null) {
    const content = document.getElementById("aiContent");
    if (!content) return;
    ...
    message.id = id;
    message.className = type === "ai" ? "message ai-response" : "message user-message";
    ...
    const modelLabel = model && type === "ai" ? `<span class="model-label">${this.getModelName(model)}</span>` : "";
    ...
    const versionNav = editHistory.length > 0 ? this.createVersionNav(id, currentVersion + 1, currentVersion + 1) : "";

    message.innerHTML = `
        <div class="message-header">
            <span class="message-type">${type === "ai" ? messageType || "AI" : "You"}</span>
            <span class="message-time">${time}</span>
            ${modelLabel}
        </div>
        <div class="message-content">${this.formatMessage(text, type === "ai")}</div>
    ...
    const isErrorMessage = text.includes("usage limit") || text.includes("Your usage will reset") || messageType === "Auth Required" || messageType === "Error";

    if (addToHistory && !isErrorMessage) {
      if (!appState.currentTopicId) {
        console.warn("⚠️ No current topic ID when creating message - creating new topic");
      ...
      if (type === "ai" && parentMessageId) {
        const parentMessage = appState.chatHistory.find((msg) => msg.id === parentMessageId);
        if (parentMessage) {
  ...
  createVersionNav: function (messageId, currentVersion, totalVersions) {
    return `
      <div class="version-nav" data-message-id="${messageId}">
        <button class="version-nav-btn prev" onclick="messageManager.navigateVersion('${messageId}', -1)" ${currentVersion <= 1 ? "disabled" : ""}>‹</button>
        <span class="version-indicator">${currentVersion}/${totalVersions}</span>
        <button class="version-nav-btn next" onclick="messageManager.navigateVersion('${messageId}', 1)" ${currentVersion >= totalVersions ? "disabled" : ""}>›</button>
      </div>
    `;
  },
  ...
  startEdit: function (messageId) {
    ...
    const historyEntry = appState.chatHistory.find((msg) => msg.id === messageId);
    const hasAttachments = historyEntry && (historyEntry.files || historyEntry.attachments);
    ...
    contentEl.innerHTML = `
      <div class="message-edit-container">
        ...
        ${hasAttachments ? '<div class="edit-attachments-notice">Note: Files and attachments from the original message will be preserved</div>' : ""}
  ...
  saveEdit: function (messageId) {
    ...
    const historyIndex = appState.chatHistory.findIndex((msg) => msg.id === messageId);
    ...
    const versionNav = this.createVersionNav(messageId, originalMessage.currentVersion, editHistory.length + 1);
  ...
  navigateVersion: function (messageId, direction) {
    ...
    const currentVersion = historyEntry.displayVersion || historyEntry.currentVersion || totalVersions;
    const newVersion = Math.max(1, Math.min(totalVersions, currentVersion + direction));
  ...
  updateVisibleResponses: function (userMessageId, version) {
    ...
    const messageIndex = appState.chatHistory.findIndex((msg) => msg.id === userMessageId);
    if (messageIndex === -1) return;
    ...
    const responsesForVersion = appState.getMessagesForVersion(userMessageId, version);
    ...
    if (version === totalVersions) {
      ...
      for (let i = messageIndex + 1; i < appState.chatHistory.length; i++) {
        ...
        for (let v = 1; v <= totalVersions; v++) {
          const trackedMessages = appState.getMessagesForVersion(userMessageId, v);
  ...
  createAIResponseWithFiles: function (responseData) {
    ...
    console.log("🗂️ File count:", responseData.files ? responseData.files.length : 0);

    // Simple duplicate check based on content and timing
    const currentTime = Date.now();
    const responseText = responseData.response || responseData.text || "";

    if (responseText === this.lastProcessedMessageText && currentTime - this.lastProcessedMessageTime < this.duplicateThresholdMs) {
      console.log("🔄 Duplicate message detected based on content + timing, skipping");
      return;
    }
    ...
    const modelLabel = responseData.model_used ? `<span class="model-label">${this.getModelName(responseData.model_used)}</span>` : "";
...
`;
    ...
    if (hasFiles && isValidFilesArray && fileCount > 0) {
      ...
      const standardizedFiles = filesArray.map((file) => ({
        ...file,
        extension: file.extension || contentManager.getFileExtension(file.filename),
        filename: file.filename,
      ...
      standardizedFiles.forEach(function (fileData, index) {
        if (!fileData || typeof fileData !== "object") {
          console.warn(`🗂️ Skipping invalid file data at index ${index}:`, fileData);
          return;
        }

        try {
          // Pass the entire standardized files array so HTML files can find CSS/JS files
          const fileBlockHtml = contentManager.createFileBlock(fileData, standardizedFiles);
          if (fileBlockHtml && typeof fileBlockHtml === "string") {
            messageContent += fileBlockHtml;
          }
        } catch (error) {
          console.error(`🗂️ Error creating file block for file ${index + 1}:`, error);
        }
      });
  ...
  `;
    ...
    historyEntry.files.forEach((file, index) => {
      console.log(`  ${index + 1}: ${file.filename} (${file.type}, ${file.content?.length || 0} chars)`);
    });
    ...
    try {
      ...
      console.log("🗂️ ✅ Verification - saved entry has files:", savedEntry.files ? savedEntry.files.length : 0);
    } catch (error) {
  ...
  showThinking: function () {
    ...
    console.log("🤔 Thinking indicator shown at", new Date(this.thinkingStartTime).toISOString());
  },
  ...
  hideThinking: function () {
    ...
    const elapsedTime = this.thinkingStartTime ? Date.now() - this.thinkingStartTime : 0;
    const remainingTime = Math.max(0, this.minimumThinkingTime - elapsedTime);

    console.log(`🤔 hideThinking called - elapsed: ${elapsedTime}ms, remaining: ${remainingTime}ms`);
...
var authManager = {
  ...
  initializeWebAuth: function () {
    ...
    if (token && userData) {
      ...
      console.log("✅ Auth loaded from localStorage:", appState.currentUser.email);
      return true;
    } else {
  ...
  resetAuthentication: function (reason = "Authentication failed") {
    ...
    appState.resetAuth();
    updateAuthDropdown();
    messageManager.createMessage("Your session has expired. Please sign in to continue using the AI assistant.", "ai", "Auth Required");
  },
  ...
  setupEventListeners: function () {
    ...
    if (window.electronAPI.onAuthStateUpdated) {
      window.electronAPI.onAuthStateUpdated(function (newAuthState) {
        ...
        if (newAuthState.isAuthenticated && newAuthState.token && newAuthState.user) {
          ...
          localStorage.setItem(CONFIG.STORAGE_KEYS.USER, JSON.stringify(newAuthState.user));
        } else {
          localStorage.removeItem(CONFIG.STORAGE_KEYS.TOKEN);
          localStorage.removeItem(CONFIG.STORAGE_KEYS.USER);
        }
    ...
    if (window.electronAPI.onAuthSuccess) {
      window.electronAPI.onAuthSuccess(function (userData) {
        ...
        localStorage.setItem(CONFIG.STORAGE_KEYS.USER, JSON.stringify(userData.user));
...
var chatManager = {
  sendMessage: function (messageText, attachments = [], skipUserMessage = false) {
      ...
      attachments.forEach((attachment, index) => {
        if (attachment.type === "large_content") {
          ...
          processedAttachments.push({
            ...
            word_count: attachment.word_count || attachment.content.split(/\s+/).length,
            extension: attachment.extension || this.getFileExtension(attachment.filename),
            language: attachment.language || attachment.contentType,
            is_executable: attachment.is_executable || false,
            mime_type: attachment.mime_type || this.getMimeType(attachment.extension || this.getFileExtension(attachment.filename)),
            content_preview: attachment.content.substring(0, 200) + "...",
            full_content_length: attachment.content.length,
          });

          console.log("🔄 Processed large content attachment:", processedAttachments[processedAttachments.length - 1]);
        ...
        displayMessage +=
          ...
          processedAttachments
            .map((att) => {
              ...
              return `${icon} ${att.filename} (${this.formatFileSize(att.size)})`;
            })
            .join("\n");
      }
    ...
    setTimeout(() => {
      console.log("🔄 Chat history length after user message:", appState.chatHistory.length);
      console.log("🔄 Current topic ID after user message:", appState.currentTopicId);
      ...
      contextMessages.forEach((msg, idx) => {
        ...
        console.log(`📤   Content length: ${msg.content.length} chars`);
        console.log(`📤   Has files: ${msg.content.includes("[Files created in this response:]")}`);
      });
      // CRITICAL: Create conversation ID on frontend if needed
      if (!appState.currentConversationId) {
        appState.currentConversationId = "conv_" + Date.now() + "_" + Math.random().toString(36).substr(2, 9);
        console.log("📝 Created new conversation ID:", appState.currentConversationId);
        appState.saveToStorage();
      }
      ...
      const messageData = {
        ...
        largeContentAttachments: attachments.filter((a) => a.type === "large_content"),
        // NEW: Context+ parameters
        context_plus_enabled: appState.contextPlusEnabled,
        retrieve_memories: appState.contextPlusEnabled,
        search_keywords: appState.contextPlusEnabled ? this.extractKeywords(messageText) : [],
      };
      ...
      console.log("📤 Context messages count:", messageData.contextMessages.length);
      console.log("📤 Processed attachments count:", messageData.attachments.length);
      console.log("📤 Large content attachments count:", messageData.largeContentAttachments.length);
  ...
  extractKeywords: function (text) {
    ...
    const keywords = words.filter((word) => word.length > 3 && !stopWords.includes(word) && !word.match(/^\d+$/));
  ...
  sendChatMessageDirect: function (data) {
    ...
    if (!authManager.checkAuthFromStorage()) {
      ...
      messageManager.createMessage("Please sign in to use the AI assistant.", "ai", "Auth Required");
      return;
    }
    ...
    if (data.largeContentAttachments && data.largeContentAttachments.length > 0) {
      processedLargeContent = data.largeContentAttachments.map((attachment) => ({
        filename: attachment.filename,
      ...
      console.log("🔄 Processed large content for backend:", processedLargeContent.length, "files");
    }
    ...
    console.log("📡 Making jQuery AJAX request to:", `${CONFIG.BACKEND_URL}/api/index.php?endpoint=chat`);
      ...
      success: function (response) {
        ...
        if (response.context_plus_info) {
          ...
          if (response.context_plus_info.memories_retrieved) {
            ...
            console.log("🧠 Memories retrieved:", response.context_plus_info.memories.length);
        ...
        if (response.success && (response.response || (response.files && response.files.length > 0))) {
          console.log("✅ Got AI response with success=true");

          if (response.has_files && response.files && Array.isArray(response.files) && response.files.length > 0) {
            ...
            console.log("📝 No files detected, using standard message creation");
            messageManager.createMessage(response.response, "ai", "Chat Response", response.model_used || data.model);
          }
          ...
          if (response.usage_recorded) {
            ...
            if (response.context_plus_tokens) {
              console.log("🧠 Context+ tokens used:", response.context_plus_tokens);
              console.log("💰 Context+ cost:", response.context_plus_cost);
            }
          }
        } else if (response.error) {
          console.error("❌ Backend returned error:", response.error);
          messageManager.createMessage(`Backend error: ${response.error}`, "ai", "Error");
        } else {
          console.error("❌ Unexpected response format:", response);
          messageManager.createMessage("Received unexpected response format from backend", "ai", "Error");
        }
      },
      error: function (xhr, status, error) {
        ...
        if (xhr.status === 401) {
          ...
          try {
            ...
            errorMessage += "\n\nBackend returned invalid response: " + xhr.responseText.substring(0, 200);
          }
        } else {
...
function updateAuthDropdown() {
  ...
  if (!appState.isAuthenticated) {
    ...
    const currentModel =
      AI_MODELS.basic.concat(AI_MODELS.advanced, AI_MODELS.fast).find(function (m) {
      ...
      return `
        <div class="ai-category">
          ...
          ${category.models
            .map(function (model) {
              return `
                <div class="ai-model-item ${model.id === appState.selectedAIModel ? "selected" : ""}" 
...
function selectAIModel(modelId) {
  ...
  messageManager.createMessage(`AI model changed to ${messageManager.getModelName(modelId)}`, "ai", "System", modelId);
}
...
function handleSignIn() {
  if (Environment.isWeb) {
    ...
    if (currentUrl.includes("memoria.html")) {
      ...
      window.location.href = baseUrl.replace(/\/[^\/]*$/, "/memoria.html#signin");
    }
    return;
  }
...
function openSettings() {
  if (Environment.isWeb) {
    const currentUrl = window.location.href;
    const baseUrl = currentUrl.split("#")[0].split("?")[0];
    window.location.href = baseUrl.replace(/\/[^\/]*$/, "/memoria.html#settings");
  } else {
...
function testChatFlow() {
  ...
  console.log("🧪 Auth state:", { isAuthenticated: appState.isAuthenticated, currentUser: appState.currentUser });

  if (!appState.isAuthenticated) {
    messageManager.createMessage("❌ Test failed: Not authenticated", "ai", "Debug Test");
    return;
  }

  messageManager.createMessage("🧪 Starting test chat message...", "ai", "Debug Test");
  ...
  messageManager.createMessage("🧪 Test message sent. Check console for details.", "ai", "Debug Test");
}
...
var historyManager = {
  ...
  deleteSubtopic: function (topicId, subtopicId) {
    const subtopicTitle = appState.topics[topicId]?.subtopics?.[subtopicId] || "this conversation";

    if (confirm(`Are you sure you want to delete "${subtopicTitle}"? This cannot be undone.`)) {
      ...
      appState.chatHistory = appState.chatHistory.filter((msg) => {
        return !(msg.topicId === topicId && (msg.subtopicId || "main") === subtopicId);
      });
      ...
      if (appState.currentTopicId === topicId && appState.currentSubtopicId === subtopicId) {
        ...
        const content = document.getElementById("aiContent");
        if (content) {
          content.innerHTML = '<div class="empty-state">AI insights will appear here...</div>';
        }
      }
  ...
  loadHistory: function () {
    ...
    if (Object.keys(appState.topics).length === 0) {
      historyContent.innerHTML = '<div class="empty-history">No conversations yet</div>';
      return;
    }
    ...
    sortedTopics.forEach(([topicId, topic]) => {
      ...
      html += `
        <div class="topic-item ${isCurrentTopic ? "active" : ""}" data-topic-id="${topicId}">
          <div class="topic-header" data-topic-id="${topicId}">
            <span class="expand-indicator">▶</span>
            <span class="topic-title">${topic.title || "Untitled Conversation"}</span>
      ...
      Object.entries(subtopics).forEach(([subtopicId, subtopicTitle]) => {
        const isCurrentSubtopic = isCurrentTopic && subtopicId === appState.currentSubtopicId;
        html += `
  ...
  loadConversation: function (topicId, subtopicId) {
    ...
    conversationMessages.forEach((msg) => {
      ...
      const message = document.createElement("div");
      message.id = msg.id;
      message.className = msg.type === "ai" ? "message ai-response" : "message user-message";
      ...
      const modelLabel = msg.model && msg.type === "ai" ? `<span class="model-label">${messageManager.getModelName(msg.model)}</span>` : "";

      // Check for edit history
      const editHistory = appState.getEditHistory(msg.id);
      const currentVersion = msg.currentVersion || 1;
      const versionNav = editHistory.length > 0 ? messageManager.createVersionNav(msg.id, msg.displayVersion || currentVersion, editHistory.length + 1) : "";

      // Handle files if present
      let messageContent = messageManager.formatMessage(msg.text, msg.type === "ai");
      ...
      if (msg.files && msg.files.length > 0) {
        ...
        msg.files.forEach((fileData) => {
          const fileBlockHtml = contentManager.createFileBlock(fileData, msg.files);
          if (fileBlockHtml) {
            messageContent += fileBlockHtml;
          }
        });
      ...
      message.innerHTML = `
        <div class="message-header">
          <span class="message-type">${msg.type === "ai" ? msg.messageType || "AI" : "You"}</span>
          <span class="message-time">${time}</span>
          ${modelLabel}
        </div>
  ...
  updateHistoryActiveState: function (topicId, subtopicId) {
    ...
    if (topicElement) {
      ...
      const subtopicElement = topicElement.querySelector(`.subtopic-item[data-topic-id="${topicId}"][data-subtopic-id="${subtopicId}"]`);
      if (subtopicElement) {
  ...
  searchHistory: function (query) {
    ...
    if (Object.keys(grouped).length === 0) {
      historyContent.innerHTML = '<div class="empty-history">No matching conversations found</div>';
      return;
    }
    ...
    Object.values(grouped).forEach((group) => {
      ...
      const subtopicTitle = topic?.subtopics?.[group.subtopicId] || "Main Conversation";
      ...
      group.messages.slice(0, 3).forEach((msg) => {
        ...
        html += `
          <div class="search-result-message ${msg.type}-message">
            <span class="message-type-indicator">${msg.type === "ai" ? "AI" : "You"}</span>
      ...
      if (group.messages.length > 3) {
        html += `<div class="more-results">...and ${group.messages.length - 3} more</div>`;
      }
...
function startNewChat() {
  ...
  messageManager.createMessage("New conversation started. How can I help you today?", "ai", "System");

  // Update the history panel to reflect no active conversation
  const allActiveElements = document.querySelectorAll(".topic-item.active, .subtopic-item.active");
  ...
  if (window.electronAPI) {
    window.electronAPI.onNewAIResponse(function (data) {
      ...
      if (data.error) {
        ...
        messageManager.createMessage(responseText, "ai", "Chat Response", data.model_used);
      } else {
        console.error("📨 Unexpected response format:", data);
        messageManager.createMessage("Received unexpected response format", "ai", "Error");
      }
    });
...
function autoResizeTextarea() {
  ...
  const newHeight = Math.min(chatInput.scrollHeight, CONFIG.AUTO_RESIZE_MAX_HEIGHT);
...
var chatInputManager = {
  ...
  autoResizeTextarea: function () {
    ...
    if (textarea) {
      textarea.style.height = "auto";
      textarea.style.height = Math.min(textarea.scrollHeight, CONFIG.AUTO_RESIZE_MAX_HEIGHT) + "px";
    }
  },
  ...
  viewFullContent: function (previewId) {
    ...
    overlay.innerHTML = `
      <div class="preview-header">
        <span class="preview-title">${this.escapeHtml(contentData.filename || "Code View")}</span>
        <div class="preview-controls">
    ...
    setTimeout(() => {
      ...
      if (codeContainer) {
        const language = contentData.language || this.detectLanguage(contentData.extension) || "plaintext";
        codeContainer.innerHTML = this.createHighlightedCode(contentData.content, language);
  ...
  createHighlightedCode: function (code, language) {
    ...
    while (i < lines.length) {
      ...
      if (foldRegion) {
        ...
        const blockLines = highlightedLines.slice(foldRegion.start, foldRegion.end + 1);
        ...
        if (blockLines.length > 1) {
          ...
          for (let j = 1; j < blockLines.length; j++) {
            html += `<div class="code-line" data-line="${i + j + 1}">${blockLines[j]}</div>`;
          }
          html += `</div>`;
        }
        html += `</div>`;
        ...
        html += `<div class="code-line" data-line="${lineNumber}">${highlightedLines[i] || ""}</div>`;
        i++;
      }
    }
  ...
  detectFoldableRegions: function (lines, language) {
    ...
    const shouldStartRegion = (line, trimmed, language) => {
      ...
      if (language === "html" || language === "xml") {
        ...
        return openTag && !trimmed.match(/\/>/) && !["br", "hr", "img", "input", "meta", "link", "area", "base", "col", "embed", "source", "track", "wbr"].includes(openTag[1]);
      }
      ...
      if (language === "javascript" || language === "typescript" || language === "jsx" || language === "tsx") {
        return (
          ...
          trimmed.match(/^(const|let|var)\s+\w+\s*=\s*(async\s*)?\w+\s*=>\s*{/) ||
          // Object methods
          ...
          // Control structures
          trimmed.match(/^(if|else if|else|for|while|do|switch|try|catch|finally)\s*.*{/) ||
          trimmed.match(/^(if|else if|for|while|switch|try|catch)\s*\(.*\)\s*{/) ||
          // Object literals
      ...
      if (language === "python") {
        return (
          trimmed.match(/^(def|class|if|elif|else|for|while|with|try|except|finally)\s*.*:/) ||
      ...
      if (language === "java" || language === "csharp" || language === "cpp" || language === "c") {
        return (
          trimmed.match(/^(public|private|protected|static|final|abstract|virtual|override)*\s*(class|interface|struct|enum)\s+.*{/) ||
          trimmed.match(/^(public|private|protected|static|final|abstract|virtual|override)*\s*\w+\s+\w+\s*\(.*\)\s*{/) || // Methods
          trimmed.match(/^(if|else if|else|for|while|do|switch|try|catch|finally)\s*.*{/) ||
    ...
    for (let i = 0; i < regions.length; i++) {
      ...
      for (let j = 0; j < finalRegions.length; j++) {
        ...
        if (region.start >= existing.start && region.end <= existing.end && !(region.start === existing.start && region.end === existing.end)) {
  ...
  toggleFold: function (foldId) {
    const block = document.querySelector(`.foldable-block[data-fold-id="${foldId}"]`);
    if (!block) return;

    const marker = block.querySelector(`.fold-marker[data-fold-id="${foldId}"]`);
    const content = block.querySelector(`.fold-content[data-fold-id="${foldId}"]`);
    const placeholder = block.querySelector(`.fold-placeholder[data-fold-id="${foldId}"]`);
...
function preventWindowGlitching() {
  ...
  window.addEventListener("resize", (e) => {
    ...
    if (currentWidth >= screen.width - 50 && currentHeight >= screen.height - 50) {
  ...
  setInterval(() => {
    ...
    if (Math.abs(currentScreenX - lastScreenX) > 50 || Math.abs(currentScreenY - lastScreenY) > 50) {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (appState.chatHistory.length > 0 && appState.currentTopicId) {
    historyManager.loadConversation(appState.currentTopicId, appState.currentSubtopicId || "main");
  }
...
window.appDebug = {
  ...
  clearAllData: function () {
    if (confirm("This will clear ALL data including authentication. Continue?")) {
      localStorage.clear();
      window.location.reload();
    }
  },
...
console.log("✅ COMPLETE AI Assistant with Context+, enhanced file handling, and syntax highlighting loaded!");
