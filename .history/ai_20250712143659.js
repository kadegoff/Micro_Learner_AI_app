...
var aiWindowState = {
  ...
  restoreFromMinimized: function () {
    console.log(
      "🔼 AI: restoreFromMinimized called, isMinimized:",
      this.isMinimized
    );

    // Don't restore if already visible
    if (!this.isMinimized && document.body.style.display !== "none") {
      console.log("🔼 AI: Already restored, ignoring");
      return;
    }

    // Show window immediately (like transcript does)
    document.body.style.display = "block";
    document.body.style.visibility = "visible";
    ...
    // Clear minimized state from both storages
    ...
    // Try Electron API if available (simple approach)
    if (window.electronAPI) {
      const methods = ["showWindow", "focusWindow", "restoreWindow"];
      methods.forEach((method) => {
        if (window.electronAPI[method]) {
          try {
            window.electronAPI[method]();
          } catch (e) {
            console.log(`⚠️ AI: ${method} failed:`, e);
          }
        }
      });
    }
