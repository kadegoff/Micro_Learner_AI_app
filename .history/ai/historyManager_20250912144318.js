...
var historyManager = {
  ...
  // 🔧 UPDATED: searchHistory method to use infinite scroll loading
  searchHistory: async function (query) {
    ...
    historyContent.innerHTML =
      '<div class="scroll-loading loading">Searching conversations...</div>';
    ...
    try {
      // Use the same pullConversationsBackup function that the infinite scroll uses
      const searchResults = await this.pullConversationsBackup(0, lowerQuery);
      console.log(`Found ${searchResults.length} matching conversations`);
      ...
      // Add "no more conversations" indicator like the infinite scroll does
      const noMoreIndicator = document.createElement("div");
      noMoreIndicator.className = "no-more-conversations";
      noMoreIndicator.textContent = "No more conversations";
      historyContent.appendChild(noMoreIndicator);
      ...
      console.log(
        `Search completed. ${searchResults.length} conversations found.`
      );
    } catch (error) {
      console.error("Failed to search conversations:", error);
      historyContent.innerHTML =
        '<div class="empty-search-state empty-history">Error searching conversations</div>';
    }
  },
