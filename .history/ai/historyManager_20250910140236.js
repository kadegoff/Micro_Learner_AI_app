...
var historyManager = {
  ...
  setupHistoryInfiniteScroll: function () {
    ...
    const resetInfiniteScroll = () => {
      ...
      // Remove any existing "no more conversations" indicator
    ...
    if (searchInput) {
      searchInput.addEventListener("input", resetInfiniteScroll);
    }
    ...
    historyContent.addEventListener("scroll", async () => {
      ...
      if (scrollPosition >= scrollThreshold) {
        ...
        let loadingIndicator = historyContent.querySelector(".scroll-loading");
        if (!loadingIndicator) {
        ...
        try {
          ...
          if (newConversations.length === 0) {
            ...
            // Show "no more conversations" message if it doesn't exist
            let noMoreIndicator = historyContent.querySelector(
              ".no-more-conversations"
            );
            if (!noMoreIndicator) {
              ...
              noMoreIndicator.style.display = "block"; // Make sure it's visible
            }
          } else {
  ...
  getTimeAgo: function (timestamp) {
    ...
    const now = new Date(); // Already in local time
    console.log("Current local time:", now);

    // Ensure messageTime is parsed in local time (if timestamp is ISO string or epoch)
    const messageTime = new Date(timestamp);
    console.log("Message local time:", messageTime);

    // Calculate difference in seconds (both in local time)
  ...
  searchHistory: async function (query) {
    ...
    // Clear any existing "no matches" message at the start of search
    ...
    // Add loading indicator if we're going to search backup
    let loadingIndicator = null;
    if (shouldCheckBackup) {
      console.log(
        `Only ${visibleCount} matches in recent history, need ${
          MIN_RESULTS - visibleCount
        } more, pulling backup conversations...`
      );

      loadingIndicator = historyContent.querySelector(".search-loading");
      if (!loadingIndicator) {
        loadingIndicator = document.createElement("div");
        loadingIndicator.className = "search-loading loading";
        loadingIndicator.textContent = "Searching older conversations...";
        historyContent.appendChild(loadingIndicator);
      }
    }
    ...
    if (shouldCheckBackup) {
      try {
        ...
        backupConversations.forEach((conversation) => {
          ...
          if (!existingItem) {
            console.log(
              "Adding backup conversation to DOM BEFORE historyContent:",
              historyContent
            );
      ...
      console.log(
        `Only ${visibleCount} matches found, but user only has ${totalConversationsInState} total conversations (less than ${MAX_CONVERSATIONS_IN_STATE}), so no backup to search.`
      );
    }

    // Show empty state only after backup search completes (if no results found anywhere)
    const totalResults = visibleCount + backupResultsFound;
    if (totalResults === 0) {
      console.log(
        "No matches found in recent history or backup, showing empty state"
      );
      let emptyState = historyContent.querySelector(".empty-search-state");
      if (!emptyState) {
      ...
      // Hide empty state if it exists (in case it was shown before)
      const emptyState = historyContent.querySelector(".empty-search-state");
      if (emptyState) {
        emptyState.style.display = "none";
      }
