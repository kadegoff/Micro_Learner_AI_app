...
var chatManager = {
  ...
  setupRealtimeTokenChecker: async function () {
    ...
    if (!messageInput || !tokenLimitIndicator) {
      if ($DebugTestMode) {
        console.error(
          "Required elements for real-time token checker are missing (e.g., #chatInput, #token-limit-indicator)."
        );
      }
      return;
    }
  ...
  startStreamingRequest: async function (requestBody, token, signal) {
    ...
    try {
      ...
      if (!contentType || !contentType.includes("text/event-stream")) {
        ...
        // Check if it's an error response from backend (both formats)
        if (jsonData.success === false || jsonData.status === "error") {
          if ($DebugTestMode) {
            console.error("🌊 Backend returned error response:", jsonData);
          }
          const errorMessage =
            jsonData.error || jsonData.message || "Backend error";
      ...
      while (true) {
        ...
        if (done) {
          if ($DebugTestMode) {
            ...
            console.log(
              "🌊 THISI S THE this.completeCalled:",
              this.completeCalled
            );
            console.log(
              "🌊 THISI S THE this.currentStreamMessageId:",
          ...
          // Check if buffer contains an error response before proceeding (both formats)
          if (buffer.trim()) {
            try {
              const potentialError = JSON.parse(buffer.trim());
              if (
        ...
        // CRITICAL FIX: Check if chunk contains error response (both formats)
        if (
          chunkText.includes('"success":false') ||
          chunkText.includes('"status":"error"')
        ) {
          if ($DebugTestMode) {
            console.error(
              "🌊 🔴 ERROR RESPONSE DETECTED IN CHUNK - ABORTING STREAM"
            );
          }

          // Try to parse the error and extract just the error message
          try {
            const errorData = JSON.parse(chunkText);
            if (errorData.success === false || errorData.status === "error") {
              reader.cancel();
              clearInterval(timeoutCheck);
              // Extract just the clean error message, not the debug log
              let cleanError = "Backend error during streaming";

              if (errorData.error) {
                cleanError = errorData.error.split(",")[0].trim();
            ...
            // If we can't parse it as JSON, try to extract just the error part
            reader.cancel();
            clearInterval(timeoutCheck);
            ...
            // Try format 1: "error":"message"
            const errorMatch1 = chunkText.match(/"error":"([^"]*)"/);
            if (errorMatch1 && errorMatch1[1]) {
              errorMessage = errorMatch1[1].split(",")[0].trim();
            }
            // Try format 2: "message":"text"
            else {
              const errorMatch2 = chunkText.match(/"message":"([^"]*)"/);
              if (errorMatch2 && errorMatch2[1]) {
                errorMessage = errorMatch2[1];
              } else {
                // Fallback: show first 200 chars of the actual error part
                const errorStart = Math.max(
                  chunkText.indexOf('"error":"'),
                  chunkText.indexOf('"message":"')
                );
                if (errorStart !== -1) {
                  const errorText = chunkText.substring(
                    errorStart,
                    errorStart + 209
                  );
                  const cleanError = errorText.split('"')[0];
                  errorMessage = cleanError || "Backend error occurred";
                }
              }
            }
  ...
  handleFileStart: function (data) {
    ...
    if ($DebugTestMode) {
      console.log(
        "📁 ✅ File block HTML created, length:",
        fileBlockHtml.length
      );
    }
    ...
    if (!fileElement) {
      if ($DebugTestMode) {
        console.error(
          "📁 ❌ CRITICAL: File element not found after insertion!"
        );
    ...
    if (typeof contentManager !== "undefined") {
      ...
      if ($DebugTestMode) {
        ...
        console.log(
          "📁 📊   - allFiles.length:",
          blockDataToAdd.allFiles.length
        );
  ...
  handleTextChunk: function (data, version = 1) {
    ...
    if (!this.streamMessageCreated) {
      ...
      // Get AI model display name from ID using AI_MODELS mapping
      const getModelDisplayName = (modelId) => {
        if (!modelId) return "AI";

        // Search through all categories to find the model
      ...
      const aiModelId = data && data.ai_model ? data.ai_model : null;
      const aiModelDisplayName = getModelDisplayName(aiModelId);

      message.innerHTML = `
      <div class="message-header">
          <span class="message-type">${aiModelDisplayName}</span>
      </div>
  ...
  createUpdatedFileDisplay: function (updatedFile) {
    if ($DebugTestMode) {
      console.log(
        "🏗️ Creating display for updated file:",
    ...
    try {
      ...
      if (typeof data === "string") {
        try {
          ...
          if ($DebugTestMode) {
            console.log(
              "Parsed JSON data and extracted user_files:",
              filesData
            );
          }
        } catch (parseError) {
