    ...
    try {
      ...
      while (true) {
        ...
        if (done) {
          ...
          // Check if buffer contains an error response before proceeding
          if (buffer.trim()) {
            try {
              ...
              if (potentialError.success === false) {
                ...
                throw new Error(potentialError.error || "Backend stream error");
              }
            } catch (e) {
        ...
        // CRITICAL FIX: Check if chunk contains error response
        if (chunkText.includes('"success":false')) {
          if ($DebugTestMode) {
            console.error(
          ...
          try {
            ...
            if (errorData.success === false) {
              ...
              const cleanError = errorData.error
                ? errorData.error.split(",")[0].trim()
                : "Backend error during streaming";
            ...
            // Extract just the error message from the JSON string
            const errorMatch = chunkText.match(/"error":"([^"]*)"/);
            if (errorMatch && errorMatch[1]) {
              throw new Error(errorMatch[1]);
            } else {
              // Fallback: show first 200 chars of the actual error part
              const errorStart = chunkText.indexOf('"error":"');
              if (errorStart !== -1) {
