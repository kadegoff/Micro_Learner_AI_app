...
var fileUpdateHandler = {
  ...
  findFileByName: function (filename) {
    ...
    if (chatManager.streamingResponseFiles) {
      const streamingFile = chatManager.streamingResponseFiles.find((f) => f.filename === filename);
  ...
  handlePartialUpdate: function (fileId, updateData) {
    ...
    console.log("🔄 Update sections:", Object.keys(updateData.sections_modified || {}));
    ...
    if (!versionData) {
      ...
      console.log("File has no version history. Creating Version 1 from initial content + update.");

      // --- FIX STARTS HERE ---
      // Get the original sections (which is likely a plain object)
      const rawInitialSections = this.getCurrentFileContent(fileId)?.sections || {};
      ...
      console.log(`File has existing versions. Creating new version from v${versionData.currentVersion}.`);

      // Get the sections from the most recent version.
      const previousVersion = versionData.versions.get(versionData.currentVersion);
    ...
    const newVersionData = {
      ...
      metadata: {
        ...
        changeCount: Object.keys(updateData.sections_modified || {}).length + Object.keys(updateData.sections_added || {}).length + (updateData.sections_removed || []).length,
      },
      changes: {
    ...
    console.log("⚙️ Input to reconstructContent (newVersionData.sections):", newVersionData.sections);
    const newContent = this.reconstructContent(newVersionData.sections);
    console.log(`➡️ [${newVersionNumber}. HPU] newContent created. Length:`, newContent.length);
    ...
    console.log(`🔄 ✅ Partial update applied. File is now at version ${newVersionNumber}.`);
  },

  // Apply section changes to the sections map
  applySectionChanges: function (sectionsMap, updateData) {
    // Apply modified sections
    for (const [sectionName, sectionData] of Object.entries(updateData.sections_modified || {})) {
      sectionsMap.set(sectionName, sectionData);
      console.log("🔄 Modified section:", sectionName);
    }

    // Apply added sections
    for (const [sectionName, sectionData] of Object.entries(updateData.sections_added || {})) {
      sectionsMap.set(sectionName, sectionData);
  ...
  updateFileContent: function (fileId, newContent) {
    ...
    if (streamingFile) {
      ...
      const responseFile = chatManager.streamingResponseFiles?.find((f) => f.id === fileId);
      if (responseFile) {
  ...
  updateFileUI: function (fileId, versionData, newContent) {
    ...
    if (!message) {
      console.error("🔄 ❌ Current message element not found:", currentMessageId);
      return;
    }
    ...
    const newFileId = `updated_${fileId}_v${versionData ? this.fileVersions.get(fileId)?.currentVersion || 1 : 1}_${Date.now()}`;
    const updatedFileData = {
      id: newFileId,
      filename: updatedFile.filename,
      content: newContent,
      extension: updatedFile.extension || chatManager.getFileExtension(updatedFile.filename),
      language: updatedFile.language || chatManager.getLanguageFromExtension(updatedFile.filename),
      type: updatedFile.type || chatManager.detectFileType(updatedFile.language),
      size: updatedFile.content.length,
      is_executable: updatedFile.is_executable || chatManager.isExecutableFile(updatedFile.filename),
      mime_type: updatedFile.mime_type || chatManager.getMimeType(updatedFile.extension),
      metadata: updatedFile.metadata || {},
      isUpdate: true,
      originalFileId: fileId,
      version: versionData ? this.fileVersions.get(fileId)?.currentVersion || 1 : 1,
    };
    ...
    if (newFileElement) {
      ...
      if (chatManager.streamingResponseFiles) {
        const existingIndex = chatManager.streamingResponseFiles.findIndex((f) => f.id === fileId);
        if (existingIndex !== -1) {
      ...
      if (codeElement && typeof hljs !== "undefined") {
        console.log(`[DEBUG] updateFileUI: Highlighting element. Class: '${codeElement.className}'`);
  ...
  createUpdatedFileBlock: function (fileData) {
    console.log("🏗️ Creating UPDATED file block for:", fileData.filename, "v" + fileData.version);

    // The helper functions exist on chatManager, not 'this' (fileUpdateHandler)
    const fileIcon = typeof contentManager !== "undefined" ? contentManager.getFileIcon(fileData.type, fileData.extension) : chatManager.getFallbackFileIcon(fileData.extension);

    const fileTypeClass = chatManager.getFileTypeClass(fileData.extension);
    const languageClass = fileData.language || chatManager.getLanguageFromExtension(fileData.filename);

    // Create update badge
    const updateBadge = fileData.updateInfo ? `<span class="update-badge" title="${fileData.updateInfo.changesSummary}">🔄 Updated</span>` : "";
      ...
      <div class="block-header">
        <div class="block-info">
          ...
          <div class="block-details">
            <div class="filename-container">
              <h4 class="filename">${chatManager.escapeHtml(fileData.filename)}</h4>
              <span class="update-badge">Updated v${fileData.version || 1}</span>
            </div>
            <div class="block-meta">
              <span class="language ${languageClass}">${languageClass}</span>
              <span class="file-size">${chatManager.formatFileSize(fileData.size)}</span>
              ${fileData.metadata ? `<span class="complexity">Complexity: ${fileData.metadata.complexity}</span>` : ""}
              ${updateBadge}
            </div>
            ${
              fileData.updateInfo
        ...
        <div class="block-actions" style="position: relative; z-index: 10;">
          <button class="block-btn primary" 
                  data-action="preview" 
                  data-file-id="file_${fileData.id}"
                  ${fileData.extension === ".html" || fileData.extension === ".htm" ? "" : "disabled"}
                  style="${
          ...
          <button class="block-btn" data-action="download" data-file-id="file_${fileData.id}">⬇️ Download</button>
          <button class="block-btn" data-action="copy" data-file-id="file_${fileData.id}">📋 Copy</button>
...
<pre><code class="language-${languageClass}" data-file-id="file_${fileData.id}">${chatManager.escapeHtml(fileData.content)}</code></pre>
        </div>
      </div>
    </div>
  `;
  ...
  addVersionSwitcher: function (fileElement, fileId) {
    ...
    const versions = Array.from(versionData.versions.keys()).sort((a, b) => b - a);
      ...
      <div class="version-controls">
        ...
        <select class="version-select" data-file-id="${fileId}">
          ${versions
            .map(
              (v) => `
            <option value="${v}" ${v === versionData.currentVersion ? "selected" : ""}>
              v${v} ${v === 1 ? "(Original)" : `(${versionData.versions.get(v).metadata.changeCount} changes)`}
            </option>
          `
            )
            .join("")}
  ...
  addChangesDisplay: function (fileElement, fileId, versionData) {
    ...
    const hasChanges = Object.keys(changes.modified).length > 0 || Object.keys(changes.added).length > 0 || changes.removed.length > 0;
    ...
    changesContainer.innerHTML = `
      <div class="changes-header">
        <h4>📝 Changes in Version ${this.fileVersions.get(fileId).currentVersion}</h4>
          ...
          <div class="change-list">
            ${Object.entries(changes.modified)
              ...
              <div class="change-item">
                ...
                <div class="section-summary">${section.change_summary || "Modified content"}</div>
                <div class="section-preview">
                  <code>${this.escapeHtml(section.content.substring(0, 100))}${section.content.length > 100 ? "..." : ""}</code>
                </div>
              </div>
            `
              )
          ...
          <div class="change-list">
            ${Object.entries(changes.added)
              ...
              <div class="change-item">
                <span class="section-name">${name}</span>
                <span class="section-type">${section.type || "content"}</span>
                <div class="section-summary">${section.change_summary || "New content"}</div>
                <div class="section-preview">
                  <code>${this.escapeHtml(section.content.substring(0, 100))}${section.content.length > 100 ? "..." : ""}</code>
                </div>
              </div>
  ...
  switchToVersion: function (fileId, version) {
    ...
    const fileElement = document.getElementById(`block_${fileId}`) || document.getElementById(fileId) || document.querySelector(`[data-file-id="${fileId}"]`);
  ...
  showVersionDiff: function (fileId) {
    ...
    const current = this.reconstructContent(versionData.versions.get(currentVersion).sections);
    const previous = this.reconstructContent(versionData.versions.get(previousVersion).sections);
  ...
  cleanupOldVersions: function (fileId, keepVersions = 5) {
    ...
    const versions = Array.from(versionData.versions.keys()).sort((a, b) => a - b);
    ...
    for (const version of toRemove) {
      if (version !== 1) {
        ...
        console.log("🔄 🗑️ Cleaned up old version", version, "for file", fileId);
      }
    }
  },
  ...
  getMemoryStats: function () {
    ...
    return {
      totalFiles,
      totalVersions,
      averageVersionsPerFile: totalFiles > 0 ? (totalVersions / totalFiles).toFixed(1) : 0,
    };
  },
};
