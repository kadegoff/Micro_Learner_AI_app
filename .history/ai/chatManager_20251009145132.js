      ...
      while (true) {
        ...
        if (done) {
          ...
          if (this.currentStreamMessageId && !this.completeCalled) {
            ...
            // 🚨 CHECK IF BUFFER CONTAINS ERROR RESPONSE
            let finalResponse = this.streamingTextBuffer;
            let hasError = false;
            let errorData = null;

            // Check the entire buffer for error JSON
            try {
              const bufferContent = buffer.trim();
              if (bufferContent) {
                const parsedBuffer = JSON.parse(bufferContent);
                if (parsedBuffer.success === false || parsedBuffer.error) {
                  if ($DebugTestMode) {
                    console.error(
                  ...
                  hasError = true;
                  errorData = parsedBuffer;
                  // Use the error message as the response
                  finalResponse = parsedBuffer.error || "An error occurred";
                }
              }
            } catch (e) {
              // Buffer is not JSON, check individual lines for errors
              const lines = buffer.split("\n");
              for (const line of lines) {
                if (line.trim()) {
                  try {
                    const parsedLine = JSON.parse(line.trim());
                    if (parsedLine.success === false || parsedLine.error) {
                      if ($DebugTestMode) {
                        console.error(
                          "🌊 🔴 ERROR FOUND IN BUFFER LINE:",
                          parsedLine
                        );
                      }
                      hasError = true;
                      errorData = parsedLine;
                      finalResponse = parsedLine.error || "An error occurred";
                      break;
                    }
                  } catch (parseError) {
                    // Line is not JSON, continue
                  }
                }
              }
            }

            // LOG EVERYTHING BEING SENT TO handleStreamComplete
            if ($DebugTestMode) {
              console.log("🌊 === MANUAL handleStreamComplete DATA ===");
              console.log(
                "🌊 currentStreamMessageId:",
                this.currentStreamMessageId
              );
              console.log("🌊 completeCalled:", this.completeCalled);
              console.log("🌊 streamingTextBuffer:", this.streamingTextBuffer);
              console.log("🌊 finalResponse to be used:", finalResponse);
              console.log("🌊 hasError:", hasError);
              console.log("🌊 errorData:", errorData);
              console.log(
                "🌊 streamingTextBuffer length:",
                this.streamingTextBuffer?.length || 0
              );
              console.log(
                "🌊 streamingTextBuffer preview:",
                this.streamingTextBuffer?.substring(0, 200) || "empty"
              );
              ...
              console.log(
                "🌊 streamingFiles size:",
                this.streamingFiles?.size || 0
              );
              console.log(
                "🌊 streamingFiles contents:",
                Array.from(this.streamingFiles?.values() || [])
              );

              const filesData = Array.from(this.streamingFiles.values()).map(
                (file) => ({
                  id: file.id,
              ...
              console.log("🌊 Files data being sent:", filesData);
              console.log("🌊 Number of files:", filesData.length);

              // Log the complete object being passed
              const completeData = {
              ...
              console.log(
                "🌊 FULL handleStreamComplete payload:",
                completeData
              );
              console.log("🌊 === END MANUAL handleStreamComplete DATA ===");
            }

            // 🚨 CALL handleStreamComplete WITH ERROR INFO
