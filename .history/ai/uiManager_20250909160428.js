...
function startNewChat() {
  ...
  if (chatManager.currentStreamMessageId) {
    console.log(
      "Still in the middle of a message stream. Cannot start new chat."
    );

    // Create and display processing banner
    const processingBanner = document.createElement("div");
    processingBanner.textContent = "Porcessing memory, please wait";
    processingBanner.style.cssText = `
      ...
      font-weight: bold;
    `;
    document.body.appendChild(processingBanner);

    // Remove banner after 4 seconds
    setTimeout(() => {
      if (document.body.contains(processingBanner)) {
        document.body.removeChild(processingBanner);
      }
    }, 4000);
