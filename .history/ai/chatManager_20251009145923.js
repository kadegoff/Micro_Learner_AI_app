        ...
        if (chunkText.includes('"success":false')) {
          ...
          try {
            ...
            if (errorData.success === false) {
              ...
              clearInterval(timeoutCheck);
              throw new Error(
                errorData.error || "Backend error during streaming"
              );
            }
          } catch (parseError) {
            // If we can't parse it as JSON, still throw with the raw text
            reader.cancel();
            clearInterval(timeoutCheck);
            throw new Error(`Backend error: ${chunkText.substring(0, 500)}`);
          }
        }
