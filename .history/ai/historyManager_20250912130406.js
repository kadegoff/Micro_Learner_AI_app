...
var historyManager = {
  ...
  searchHistory: async function (query) {
    ...
    // Clear any existing "no matches" message AND "no more conversations" message
    const emptyState = historyContent.querySelector(".empty-search-state");
    if (emptyState) {
      emptyState.style.display = "none";
    }

    // CRITICAL: Remove any "no more conversations" indicators during search
    const noMoreIndicator = historyContent.querySelector(
      ".no-more-conversations"
    );
    if (noMoreIndicator) {
    ...
    if (totalResults === 0) {
      ...
      // ADD THIS: After showing "no matching conversations", also show "no more conversations"
      // since there are no more results for this search query
      const noMoreIndicator = document.createElement("div");
      noMoreIndicator.className = "no-more-conversations";
      noMoreIndicator.textContent = "No more conversations";
      historyContent.appendChild(noMoreIndicator);
      ...
      // ADD THIS: Show "no more conversations" at the bottom when we have results
      // but there are no more conversations for this search query
      const noMoreIndicator = document.createElement("div");
      noMoreIndicator.className = "no-more-conversations";
      noMoreIndicator.textContent = "No more conversations";
      historyContent.appendChild(noMoreIndicator);
