...
var historyManager = {
  ...
  setupHistoryInfiniteScroll: function () {
    ...
    historyContent.addEventListener("scroll", async () => {
      ...
      if (isLoading) {
        console.log("Exiting scroll handler: isLoading");
        return;
      }
      ...
      if (scrollPosition >= scrollThreshold) {
        ...
        try {
          ...
          // Remove loading indicator instead of hiding it
  ...
  getTimeAgo: function (timestamp) {
    ...
    // Parse the timestamp as UTC (add 'Z' to indicate UTC)
    const messageTime = new Date(timestamp + "Z");
    console.log("Message UTC time:", messageTime);

    // Calculate difference in seconds (convert both to UTC for accurate comparison)
    const nowUTC = Date.now(); // UTC timestamp in milliseconds
    const messageUTC = messageTime.getTime(); // UTC timestamp in milliseconds
