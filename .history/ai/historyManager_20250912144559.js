...
var historyManager = {
  ...
  searchHistory: async function (query) {
    ...
    let searchOffset = 0;
    const SEARCH_BATCH_SIZE = 10; // Load 10 results at a time

    // Function to load more search results
    const loadMoreSearchResults = async () => {
      if (isLoading || !hasMoreSearchResults) return;
      ...
      try {
        ...
        const loadingIndicator = document.createElement("div");
        loadingIndicator.className = "scroll-loading loading";
        loadingIndicator.textContent = "Loading more search results...";
        historyContent.appendChild(loadingIndicator);

        // Get search results
        const searchResults = await this.pullConversationsBackup(
          searchOffset,
          lowerQuery
        );
        ...
        if (searchResults.length === 0) {
          ...
          // Show "no more results" message if we have some results already
          if (searchOffset > 0) {
            const noMoreIndicator = document.createElement("div");
            noMoreIndicator.className = "no-more-conversations";
            noMoreIndicator.textContent = "No more search results";
            historyContent.appendChild(noMoreIndicator);
          } else {
            // No results at all
          ...
          // Setup event delegation for new items
          this.setupEventDelegation();

          // Update offset for next batch
          searchOffset += searchResults.length;

          // If we got fewer results than batch size, assume no more results
          if (searchResults.length < SEARCH_BATCH_SIZE) {
            hasMoreSearchResults = false;
        ...
        // On error, assume there might be more results
    ...
    // Set up scroll event listener for search results
    const searchScrollHandler = async () => {
      if (isLoading || !hasMoreSearchResults) return;
    ...
    historyContent.removeEventListener("scroll", searchScrollHandler);

    // Add new scroll listener for search
    historyContent.addEventListener("scroll", searchScrollHandler);

    // Load initial batch of search results
    await loadMoreSearchResults();

    console.log("Search initialized with infinite scroll");
  },
