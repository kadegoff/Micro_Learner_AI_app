...
var messageManager = {
  ...
  addMessageToHistory: function (historyEntry) {
    ...
    // Remove oldest messages from conversations if over the limit, but keep conversations
    const MAX_CONVERSATIONS_WITH_MESSAGES = 30;
    ...
    console.log(
      `[History] Maximum allowed conversations with messages: ${MAX_CONVERSATIONS_WITH_MESSAGES}`
    );
    console.log(
      `[History] Maximum total conversations: ${MAX_TOTAL_CONVERSATIONS}`
    );
    ...
    if (currentConversationCount > MAX_TOTAL_CONVERSATIONS) {
      const conversationsToRemove =
        currentConversationCount - MAX_TOTAL_CONVERSATIONS;
      console.log(
      ...
      for (const [id, conversation] of Object.entries(appState.chatHistory)) {
        ...
        if (conversation.timestamp) {
          timestampValue = new Date(conversation.timestamp).getTime();
          console.log(
            `[History] Conversation ${id} has timestamp: ${conversation.timestamp} -> ${timestampValue}`
          );
        } else {
          console.warn(
            `[History] Conversation ${id} has no timestamp, treating as oldest`
          );
          timestampValue = 0;
        }
      ...
      console.log(
        `[History] Sorted conversations (oldest first):`,
        conversationsWithTimestamps.map((c) => ({
          id: c.id,
          timestamp: c.timestamp,
        }))
      );

      // Remove the oldest conversations (up to the number needed to stay at total limit)
      const conversationsToDelete = conversationsWithTimestamps.slice(
        0,
        conversationsToRemove
      );
      console.log(
        `[History] Conversations to delete:`,
        conversationsToDelete.map((c) => c.id)
      );

      for (const conversationData of conversationsToDelete) {
        console.log(
          `[History] Removing old conversation: ${conversationData.id} (timestamp: ${conversationData.timestamp})`
        );
      ...
      console.log(
        `[History] At or under total conversation limit, skipping deletion`
      );
    }

    // Initialize the conversation object with messages array if it doesn't exist
    ...
    // Now check if this conversation has exceeded the message limit and trim if needed
    const conversation = appState.chatHistory[conversationId];
    if (conversation.messages.length > MAX_CONVERSATIONS_WITH_MESSAGES) {
      const messagesToRemove =
        conversation.messages.length - MAX_CONVERSATIONS_WITH_MESSAGES;
      console.log(
        `[History] Conversation ${conversationId} has ${conversation.messages.length} messages, removing ${messagesToRemove} oldest message(s)`
      );

      // Remove oldest messages (keep the most recent ones)
      conversation.messages = conversation.messages.slice(messagesToRemove);
      console.log(
        `[History] Trimmed conversation to ${conversation.messages.length} messages`
      );
    }
    ...
    if (historyEntry.files && Array.isArray(historyEntry.files)) {
      ...
      // Create a new array to store only file IDs for the history entry
      const savedFileIds = [];

      historyEntry.files.forEach((file) => {
        if (file.id) {
          if (historyEntry.type === "user" && file.content) {
            // For USER type: Only save the file content
            console.log(
              `[History] Saving file content only for USER: ${file.id}`
            );
            console.log("CALLING SAVING FILE 5");
            historyManager.saveFile(
              file.id,
              file.content, // Only pass the content for USER type
              conversationId
            );
          } else if (historyEntry.type === "ai") {
            // For AI type: Save the whole file object
            console.log(
            ...
            historyManager.saveFile(
              file.id,
              file, // Pass the whole file object for AI type
              conversationId
            );
          }
  ...
  accept=".txt,.js,.py,.html,.css,.json,.md,.csv,.pdf,.doc,.docx,.ppt,.pptx,.xls,.xlsx,image/png,image/jpeg,image/webp,image/gif,audio/*,video/*" // <-- REVISED
      ...
      <div class="chat-controls">
        ...
        <div class="message-edit-actions">
          ...
          <button class="edit-save-btn" onclick="messageManager.saveEdit('${messageId}')">Send</button>
        </div>
      </div>
    </div>
  </div>
...
`;
  ...
  saveEdit: function (messageId) {
    ...
    console.log("💾 🔍 STEP 4: Getting message version and metadata from HTML");
    const messageEl = document.querySelector(
  ...
  navigateVersion: function (messageId, direction) {
    ...
    if (!targetMessage) {
      ...
      allVersions.forEach((msg, index) => {
        console.log(`🧭   Available version ${index + 1}: ${msg.version || 1}`);
      });
      return;
    }
  ...
  updateMessagesAfterVersion: function (
    ...
    const getMessageElementByVersion = (messageId, version) => {
      ...
      console.log(`🔄 🔍 [STRATEGY 1] Result:`, logElementDetails(exactMatch));
      ...
      console.log(`🔄 🔍 [STRATEGY 2] Found:`, logElementDetails(elementById));
      ...
      allElements.forEach((el, i) => {
        ...
        console.log(`🔄 🔍 [STRATEGY 3] Element #${i + 1}:`, {
          match: elVersion === String(version) ? "POSSIBLE MATCH" : "NO MATCH",
    ...
    while (messagesToCheck.length > 0) {
      ...
      latestVersionsByMessageId.forEach((msg) => {
        ...
        if (!visibleMessageVersions.has(versionKey)) {
          ...
          console.log(`🔄 🔗   Added LATEST VERSION to visible: ${versionKey}`);
    ...
    messagesByID.forEach((versions, messageId) => {
      ...
      // ✅ ADDITIONAL SAFETY: Hide any other versions of this message ID that might exist in DOM
      /*const allDOMVersions = getAllMessageElements(messageId);
      allDOMVersions.forEach((domEl) => {
        const domVersion = domEl.getAttribute("data-version");
        const domVersionKey = `${messageId}.v${domVersion}`;

        if (!visibleMessageVersions.has(domVersionKey)) {
          console.log(
            `🔄 🧹 CLEANUP: Hiding DOM element ${domVersionKey} (not in visible chain)`
          );
  ...
  isChildOfVersionMessages: function (
    ...
    console.log(`🔄 🔍 📍 User message ID: ${userMessageId}`);
    console.log(`🔄 🔍 📍 Target version: ${targetVersion}`);
    console.log(`🔄 🔍 📍 Tracked responses: [${trackedResponses.join(", ")}]`);
    ...
    while (currentMsg && chainDepth < maxChainDepth) {
      ...
      if (!currentMsg.parent_message_id) {
        console.log(`🔄 🔍   ❌ NO PARENT: Chain ends at depth ${chainDepth}`);
        console.log(`🔄 🔍   📈 Final chain length: ${parentChain.length}`);
        break;
      }

      // ===== ENHANCED PARENT ID ANALYSIS =====
      console.log(`🔄 🔍 🧬 === PARENT ID ANALYSIS (DEPTH ${chainDepth}) ===`);
      ...
      console.log(`🔄 🔍 🧬 Required parent version: ${requiredParentVersion}`);
      ...
      if (requiredParentVersion && searchParentId === userMessageId) {
        ...
        if (targetVersion === requiredParentVersion) {
          ...
          console.log(`🔄 🔍 🧬 ❌ VERSION MISMATCH: Message should be hidden`);
          console.log(
            `🔄 🔍 🧬   Message ${currentMsg.id} requires version ${requiredParentVersion}`
          );
          console.log(`🔄 🔍 🧬   Currently viewing version ${targetVersion}`);
      ...
      if (!parentMsg) {
        ...
        console.log(`🔄 🔍 🔍   CHAIN TERMINATION: Cannot continue traversal`);
        break;
      }
      ...
      console.log(`🔄 🔍 🔄   Moving from ${currentMsg.id} to ${parentMsg.id}`);
    ...
    console.log(`🔄 🔍 ❌   Max depth reached: ${chainDepth >= maxChainDepth}`);
    console.log(`🔄 🔍 ❌   User message ID: ${userMessageId}`);
    console.log(`🔄 🔍 ❌   Target version: ${targetVersion}`);
    console.log(
  ...
  recreateMessageElement: function (messageData, insertAfterId) {
    ...
    if (insertAfterEl) {
      ...
      console.log("🔧 ✅ Message recreated and inserted after:", insertAfterId);
    } else {
      content.appendChild(message);
  ...
  escapeHtml: function (text) {
    ...
    console.log("Created div element:", div); // Log the created div element
