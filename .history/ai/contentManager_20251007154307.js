...
`;
    ...
    if ($DebugTestMode) {
      console.log(
        "🔍 Fullscreen preview created for block:",
        block.id,
        "Executable:",
        block.isExecutable,
        "Renderable:",
        ...
        "UI structure:",
        {
          ...
          containerClass: overlay.querySelector(".preview-content-container")
            ?.className,
          hasIframe: !!overlay.querySelector(".preview-iframe"),
          hasCodeView: !!overlay.querySelector(
            ".code-view, .code-view-enhanced"
          ),
  ...
  sanitizeHTML: function (html) {
    ...
    try {
      ...
      console.log(
        "🔍 DEBUG: Parsed document has documentElement:",
        !!doc.documentElement
      );
      ...
      console.log(
        "🔍 DEBUG: sanitizeHTML output length:",
      ...
      if (!result || result.length === 0) {
        ...
        console.log(
          "🔍 DEBUG: Parsed doc body:",
          doc.body ? doc.body.innerHTML.substring(0, 500) : "NO BODY"
        );
      }
  ...
  escapeAttribute: function (str) {
    console.log(
      "🔍 DEBUG: escapeAttribute input length:",
      str ? str.length : 0
    );
  ...
  showCodePreview: function (blockId) {
    ...
    if (blockId.startsWith("file_file_")) {
      ...
      blockId.startsWith("file_") &&
      !blockId.startsWith("file_file_")
    ) {
    ...
    function processBlockElement(element, elementId) {
      ...
      <div class="code-preview">
        <pre><code class="language-${language}">${contentManager.escapeHtml(
        block.content
      )}</code></pre>
      </div>
    `;
  ...
  isRenderableContent: function (content, extension) {
    ...
    const hasWebTags =
      /<(html|head|body|div|script|style|p|h[1-6]|img|a|ul|ol|li|table|form)/i.test(
        content
      );
  ...
  addBlock: function (blockData) {
    ...
    try {
      ...
      if (blockData.allFiles && blockData.allFiles.length > 0) {
        actualFileData = blockData.allFiles.find(
          (file) =>
            file.filename === blockData.filename ||
            file.id === blockData.id ||
            blockData.id.includes(file.id)
        );
      }
      ...
      const block = {
        ...
        language:
          sourceData.language ||
          this.getLanguageFromExtension(sourceData.extension),
        type:
          sourceData.type ||
          this.detectFileTypeFromExtension(sourceData.extension),
        size: 0, // Will be calculated
        wordCount: 0, // Will be calculated
        isExecutable:
          sourceData.is_executable || this.isExecutableFile(blockData.filename),
        mimeType: this.getFileExtension(blockData.filename),
        collapsed:
          blockData.collapsed !== undefined ? blockData.collapsed : true,
      ...
      block.isRenderable = this.isRenderableContent(
        block.content,
        block.extension
      );
