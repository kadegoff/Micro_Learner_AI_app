...
var historyManager = {
  ...
  getTimeAgo: function (timestamp) {
    ...
    if (
      ...
      // ISO format with timezone (e.g., "2025-09-10T18:08:34.000Z" or "2025-09-10T18:08:34+00:00")
      messageTime = new Date(timestamp);
    } else if (timestamp.match(/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}/)) {
      // MySQL/DB format without timezone (e.g., "2025-09-10 18:08:34.000000")
      // Assume UTC time
      messageTime = new Date(timestamp + "Z");
    } else {
      // Fallback - try to parse as is (could be local time)
      messageTime = new Date(timestamp);
    }
