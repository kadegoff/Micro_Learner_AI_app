...
addMessageToHistory: function (historyEntry) {
    ...
    if (currentConversationCount >= MAX_TOTAL_CONVERSATIONS) {
      ...
      if (conversationsToRemove > 0) {
        ...
        for (const [id, conversation] of Object.entries(appState.chatHistory)) {
          if (id === conversationId) { // Skip current conversation (might be new)
            console.log(
    ...
    if (currentConversationIndex >= MAX_CONVERSATIONS_WITH_MESSAGES) {
      ...
      console.log(
        `[History] Conversation ${conversationId} is position ${currentConversationIndex + 1}, removing all messages (keeping only metadata)`
      );
      ...
      console.log(
        `[History] Conversation ${conversationId} is position ${currentConversationIndex + 1}, keeping all ${conversation.messages.length} messages`
      );
