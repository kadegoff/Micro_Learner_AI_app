...
var historyManager = {
  ...
  searchHistory: async function (query) {
    ...
    // Track if we found ANY matches (existing + backup)
    let foundAnyMatches = visibleCount > 0;

    // Search backup if needed
    if (shouldCheckBackup) {
      // Check if loading indicator already exists before adding a new one
      const existingLoadingIndicator = historyContent.querySelector(
        ".scroll-loading.loading"
      );

      if (!existingLoadingIndicator) {
        // Create and show loading indicator only if it doesn't exist
        const loadingIndicator = document.createElement("div");
        loadingIndicator.className = "scroll-loading loading";
        loadingIndicator.textContent = "Loading more conversations...";
        historyContent.appendChild(loadingIndicator);
      ...
      try {
        ...
        const loadingIndicator = historyContent.querySelector(
          ".scroll-loading.loading"
        );
        if (loadingIndicator) {
          loadingIndicator.remove();
        ...
        backupConversations.forEach((conversation) => {
          ...
          if (!existingItem) {
            ...
            visibleCount++;
            foundAnyMatches = true; // We found matches from backup
            this.setupEventDelegation();
          }
        });
      } catch (error) {
      ...
      // Remove any existing loading indicator if backup search is not needed
      const loadingIndicator = historyContent.querySelector(
        ".scroll-loading.loading"
      );
      if (loadingIndicator) {
    ...
    // CRITICAL FIX: Show appropriate status message
    if (!foundAnyMatches) {
      ...
      // Only show "No more conversations" if we found some matches
      const existingNoMore = historyContent.querySelector(
        ".no-more-conversations"
      );
      if (!existingNoMore) {
    ...
    console.log(
      `Search completed. ${visibleCount} conversations visible. ${backupResultsFound} from backup. Found any matches: ${foundAnyMatches}`
    );
  },
