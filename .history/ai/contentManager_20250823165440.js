// contentManager.js - Enhanced File Block Manager - FIXED VERSION with proper file linking
"use strict";
...
var contentManager = {
    ...
    document.addEventListener(
      ...
      function (event) {
        ...
        if ($DebugTestMode) {
          ...
          console.log(
            "🎯 Event path:",
            event.composedPath().map((el) => el.tagName + (el.className ? "." + el.className : ""))
          );

          // Check if we can find ANY buttons in the clicked area
          const nearbyButtons = target.closest(".block-header")?.querySelectorAll(".block-btn");
          if (nearbyButtons) {
            console.log("🎯 Found buttons in header:", nearbyButtons.length);
            nearbyButtons.forEach((btn, idx) => {
              console.log(`🎯 Button ${idx}:`, btn.textContent, "Action:", btn.dataset.action, "File ID:", btn.dataset.fileId);
            });
          }
          ...
          while (checkEl && depth < 5) {
            console.log(`🎯 Level ${depth}:`, checkEl.tagName, checkEl.className);
        ...
        if (!button && target.classList && target.classList.contains("block-btn")) {
          button = target;
        }
        ...
        if (button) {
          if ($DebugTestMode) {
            ...
            console.log("🎯 Is streaming block:", button.closest(".streaming") ? true : false);
          }
          event.preventDefault();
          event.stopPropagation();
          event.stopImmediatePropagation(); // Stop ALL other handlers

          const blockElement = button.closest(".content-block") || button.closest(".file-block");
          if ($DebugTestMode) {
          ...
          if ($DebugTestMode) {
            console.log("🎯 Block ID:", blockId, "Action:", action, "File ID:", fileId);
            console.log("🎯 ContentManager blocks keys:", Array.from(contentManager.blocks.keys()));
            console.log("🎯 ChatManager streaming files keys:", chatManager.streamingFiles ? Array.from(chatManager.streamingFiles.keys()) : []);
            console.log("🎯 Streaming file exists:", chatManager.streamingFiles.has(fileId));
          }
          ...
          switch (action) {
            case "preview":
              ...
              if (blockElement.classList.contains("streaming") && chatManager.streamingFiles) {
                ...
                if (streamingFile) {
                  ...
                  if ($DebugTestMode) {
                    console.warn("🎯 No streaming file found for fileId:", fileId);
                  }
                }
              }
        ...
        if (target.closest(".block-header") && !target.closest(".block-btn") && !target.closest(".block-actions")) {
          const blockElement = target.closest(".content-block") || target.closest(".file-block");
          if (blockElement) {
  ...
  findOrCreateBlock: function (fileData) {
    if (!fileData.id) {
      console.error("❌ findOrCreateBlock requires a fileData object with an id.");
      return null;
    }
    ...
    if (this.blocks.has(blockId)) {
      console.log(`🔄 Block with ID ${blockId} already exists. Returning existing block.`);
  ...
  getBlockContent: function (blockId) {
    ...
    if (blockElement) {
      const codeElement = blockElement.querySelector(".streaming-code, code, pre code");
  ...
  preview: function (blockId) {
    ...
    if (previewWindow) {
      ...
      alert("Unable to open preview window. Please check your popup blocker settings.");
    }
  },
  ...
  `;
    ...
    modal.addEventListener("click", (e) => {
      if (e.target.classList.contains("close") || e.target.classList.contains("code-modal")) {
        modal.remove();
  ...
  createFileBlock: function (fileData, allFiles = []) {
    ...
    if (!detectedLanguage || detectedLanguage === "text" || detectedLanguage === "plaintext") {
      const extension = fileData.extension || this.getFileExtension(fileData.filename);
    ...
    if ((fileData.extension === ".html" || fileData.extension === ".htm") && allFiles.length > 0) {
      if ($DebugTestMode) {
        console.log("🔗 HTML file detected, checking for related CSS/JS files");
      }

      const cssFiles = allFiles.filter((f) => f.extension === ".css" || (f.filename && f.filename.toLowerCase().endsWith(".css")));
      const jsFiles = allFiles.filter((f) => f.extension === ".js" || (f.filename && f.filename.toLowerCase().endsWith(".js")));

      if (cssFiles.length > 0 || jsFiles.length > 0) {
        if ($DebugTestMode) {
          console.log(`🔗 Found ${cssFiles.length} CSS and ${jsFiles.length} JS files to link`);
        }
        finalContent = this.linkFilesToHTML(fileData.content, cssFiles, jsFiles);
    ...
    const block = {
      ...
      extension: fileData.extension || this.getFileExtension(fileData.filename) || ".txt",
      ...
      wordCount: fileData.word_count || (fileData.content ? fileData.content.split(/\s+/).length : 0),
      isExecutable: fileData.is_executable || fileData.extension === ".html" || fileData.extension === ".htm",
      mimeType: fileData.mime_type || this.getMimeType(fileData.extension || ".txt"),
      collapsed: true,
  ...
  previewStreamingContent: function (fileId, streamingFile) {
    if ($DebugTestMode) {
      console.log("🔍 previewStreamingContent called with fileId:", fileId, "Streaming File:", streamingFile);
    }
    ...
    if (!streamingFile) {
      ...
      if ($DebugTestMode) {
        console.warn("🔍 No streaming file found in streamingFiles, falling back to blocks:", streamingFile);
      }
    }

    if (!streamingFile || !streamingFile.content) {
      console.error("No streaming content available for fileId:", fileId);
      alert("No streaming content available for preview. Please wait for streaming to start.");
      return;
    }
    ...
    const block = {
      ...
      mimeType: streamingFile.mime_type || this.getMimeType(streamingFile.extension || ".txt"),
      linkedFiles: streamingFile.linkedFiles || [],
      allFiles: streamingFile.allFiles || [],
      isRenderable: this.isRenderableContent(streamingFile.content, streamingFile.extension),
    ...
    const previewContainer = overlay.querySelector(".preview-content-container");
    if (!previewContainer) {
    ...
    const updateViewMode = () => {
      ...
      const codeContainer = previewContainer.querySelector(".code-view, .code-view-enhanced");
    ...
    const getCurrentStreamingFiles = () => {
      ...
      if (chatManager.streamingResponseFiles && chatManager.streamingResponseFiles.length > 0) {
        chatManager.streamingResponseFiles.forEach((file) => {
          // Get the latest content from streamingFiles
          const streamingFile = chatManager.streamingFiles.get(file.id) || chatManager.streamingFiles.get(`file_${file.id}`);
      ...
      chatManager.streamingFiles.forEach((file, key) => {
        if (!currentFiles.find((f) => f.id === file.backendId || f.id === file.id)) {
          currentFiles.push({
    ...
    const updatePreview = () => {
      ...
      if ($DebugTestMode) {
        console.log("🔄 Update preview - Total streaming files:", allCurrentFiles.length);
        allCurrentFiles.forEach((f) => {
          console.log(`🔄 File: ${f.filename} (${f.extension}) - Content length: ${f.content ? f.content.length : 0}`);
        });
      }
      ...
      if (!isCodeView && (block.isExecutable || block.isRenderable)) {
        ...
        if (iframe) {
          ...
          const cssFiles = allCurrentFiles.filter((f) => f.extension === ".css" || (f.filename && f.filename.toLowerCase().endsWith(".css")));
          const jsFiles = allCurrentFiles.filter((f) => f.extension === ".js" || (f.filename && f.filename.toLowerCase().endsWith(".js")));

          // Check if we need to update (HTML changed OR linked files changed)
          const htmlChanged = block.content !== lastAppliedHtml;
          const linkedContentChanged = hasLinkedContentChanged(cssFiles, jsFiles);

          if (htmlChanged || linkedContentChanged) {
            if ($DebugTestMode) {
              console.log("🔄 Updating preview - HTML changed:", htmlChanged, "Linked content changed:", linkedContentChanged);
              if (linkedContentChanged) {
                console.log(
                  "🔄 CSS files:",
                  cssFiles.map((f) => `${f.filename}: ${f.content.length} chars`)
                );
              }
            }
            ...
            if ((block.extension === ".html" || block.extension === ".htm") && (cssFiles.length > 0 || jsFiles.length > 0)) {
...
${fullHtml}
              ...
              if ($DebugTestMode) {
                console.log("🔄 Linked files - CSS:", cssFiles.length, "JS:", jsFiles.length);
              }
            }
          ...
          `;
            ...
            if (fullHtml.includes("</body>")) {
              fullHtml = fullHtml.replace("</body>", navigationScript + "</body>");
            } else if (fullHtml.includes("</html>")) {
              fullHtml = fullHtml.replace("</html>", navigationScript + "</html>");
            } else {
            ...
            if ($DebugTestMode) {
              console.log("🔄 Preview updated with fresh content and linked files");
            }
          }
        }
      }
    ...
    if (streamingFile.isStreaming || (streamingFile.element && streamingFile.element.classList.contains("streaming"))) {
      if ($DebugTestMode) {
        console.log("🔄 Setting up polling for live streaming with dynamic file linking");
      }
      ...
      const pollInterval = setInterval(() => {
        ...
        const stillStreaming =
          chatManager.streamingFiles.has(fileId) || chatManager.streamingFiles.has(fileId.replace("file_", "")) || chatManager.streamingFiles.has(`file_${fileId}`);
        ...
        if (!stillStreaming && !anyFileStreaming) {
          ...
          const streamingIndicator = previewContainer.querySelector(".streaming-indicator");
      ...
      const closeButton = overlay.querySelector(".close-preview, #closePreviewBtn");
      if (closeButton) {
  ...
  sendFileChunk: function (fileId, chunk) {
    if ($DebugTestMode) {
      console.debug("[sendFileChunk] Received chunk for file:", fileId, "Chunk size:", chunk.length);
    }

    const file = this.streamingFiles.get(fileId) || this.streamingFiles.get(`file_${fileId}`);
    if (!file) {
      if ($DebugTestMode) {
        console.error("[sendFileChunk] File not found in streamingFiles:", fileId);
      }
      return;
    }
    ...
    const responseFile = this.streamingResponseFiles.find((f) => f.id === fileId);
    if (responseFile) {
    ...
    if (block) {
      ...
      if ($DebugTestMode) {
        console.log("[sendFileChunk] Updated block with all streaming files:", block.allFiles.length);
      }
    }
  ...
  linkFilesToHTML: function (htmlContent, cssFiles, jsFiles) {
    ...
    if (cssFiles.length > 0) {
      ...
      processedHtml = processedHtml.replace(/<\/head>/i, `<style>${allCssContent}</style></head>`);
    }

    // 3. Inject all JS content directly before the closing </body>
    if (jsFiles.length > 0) {
      const allJsContent = jsFiles.map((file) => `\n/*-- Injected from ${this.escapeHtml(file.filename)} --*/\n${file.content}`).join("\n");
      if (!/<\/body\b/i.test(processedHtml)) {
        processedHtml += "</body>"; // Ensure body tag exists
      }
      processedHtml = processedHtml.replace(/<\/body>/i, `<script>${allJsContent}</script></body>`);
    }

    // 4. Clean up: Remove the original <link> and <script> tags
    const allFiles = [...cssFiles, ...jsFiles];
    allFiles.forEach((file) => {
      const linkTagRegex = new RegExp(`<link[^>]+href\\s*=\\s*["'](?:\\.\\/|\\/)?${file.filename}["'][^>]*>`, "gi");
      const scriptTagRegex = new RegExp(`<script[^>]+src\\s*=\\s*["'](?:\\.\\/|\\/)?${file.filename}["'][^>]*>\\s*<\\/script>`, "gi");
  ...
  renderFileBlock: function (block) {
    ...
    const previewText = block.originalContent.length > 200 ? block.originalContent.substring(0, 200) + "..." : block.originalContent;
    ...
    if (block.linkedFiles && block.linkedFiles.length > 0) {
      linkedFilesInfo = `<div class="linked-files-info">
            <small>🔗 Linked: ${block.linkedFiles.map((f) => f.filename).join(", ")}</small>
        </div>`;
    }

    // Add executable badge
    const executableBadge = block.isExecutable ? '<span class="executable-badge" title="This file can run standalone">⚡ Executable</span>' : "";
    ...
    const html = `
        <div class="content-block file-block" id="${block.id}" data-type="${block.type}">
            <div class="block-header ${block.collapsed ? "collapsed" : ""}">
                <div class="block-info">
                    <div class="block-icon">${fileIcon}</div>
                    <div class="block-details">
                        <h4 class="filename">${this.escapeHtml(block.filename)}</h4>
                        <div class="block-meta">
                            <span class="file-size">${this.formatFileSize(block.size)}</span>
                            <span class="word-count">${block.wordCount} words</span>
            ...
            <div class="block-content ${block.collapsed ? "collapsed" : ""}" id="${block.id}_content">
                <div class="preview-content">
                    <pre><code class="language-${block.language}">${this.escapeHtml(previewText)}</code></pre>
                </div>
            </div>
        </div>
    `;
  ...
  previewContent: function (blockId) {
    ...
    const isHtmlFile = block.extension === ".html" || block.extension === ".htm";
    const hasHtmlContent = block.content && /<(html|head|body|div|script|style|p|h[1-6])/i.test(block.content);
    ...
    if ($DebugTestMode) {
      console.log("🔍 Block analysis:", {
        ...
        shouldBeExecutable,
        shouldBeRenderable,
        contentPreview: block.content ? block.content.substring(0, 100) : "NO CONTENT",
      });
    }
  ...
  createFullscreenPreview: function (block, startInCodeView = false) {
    ...
    if (canRender) {
      ...
      console.log("🔍 DEBUG: Original content first 200 chars:", htmlContent.substring(0, 200));

      if ((block.extension === ".html" || block.extension === ".htm") && block.allFiles) {
        if ($DebugTestMode) {
          console.log("🔗 Regenerating file links for preview");
        }
        const cssFiles = block.allFiles.filter((f) => f.extension === ".css" || (f.filename && f.filename.toLowerCase().endsWith(".css")));
        const jsFiles = block.allFiles.filter((f) => f.extension === ".js" || (f.filename && f.filename.toLowerCase().endsWith(".js")));
        htmlContent = this.linkFilesToHTML(block.originalContent, cssFiles, jsFiles);
      }

      // CRITICAL DEBUG SECTION - Let's trace exactly what happens to the HTML
      console.log("🔍 DEBUG: Before sanitization - HTML length:", htmlContent.length);
      console.log("🔍 DEBUG: Before sanitization - HTML type:", typeof htmlContent);
      console.log("🔍 DEBUG: Before sanitization - HTML preview:", htmlContent.substring(0, 300));
      ...
      console.log("🔍 DEBUG: After sanitization - preview:", safeHtml.substring(0, 300));
  ...
  `;
      ...
      if (safeHtml && safeHtml.length > 0) {
        ...
        // If sanitization failed, use original with minimal safety
        console.warn("🔍 DEBUG: Using original content due to sanitization failure");
        finalHtml = htmlContent + navigationScript;
      }

      console.log("🔍 DEBUG: Final HTML length:", finalHtml.length);
      console.log("🔍 DEBUG: Final HTML preview (first 400 chars):", finalHtml.substring(0, 400));

      // CRITICAL: Debug the escapeAttribute function
      console.log("🔍 DEBUG: About to escape attribute...");
      console.log("🔍 DEBUG: Input to escapeAttribute length:", finalHtml.length);
      console.log("🔍 DEBUG: Input contains quotes:", finalHtml.includes('"'));
      console.log("🔍 DEBUG: Input contains single quotes:", finalHtml.includes("'"));

      const escapedContent = this.escapeAttribute(finalHtml);
      console.log("🔍 DEBUG: Escaped content length:", escapedContent.length);
      console.log("🔍 DEBUG: Escaped content preview (first 400 chars):", escapedContent.substring(0, 400));
  ...
  `;
      ...
      console.log("🔍 DEBUG: iframeContent created, length:", iframeContent.length);
      console.log("🔍 DEBUG: iframeContent preview:", iframeContent.substring(0, 200));

      // Verify the srcdoc attribute is actually in the HTML
      const srcdocMatch = iframeContent.match(/srcdoc="([^"]*)/);
      if (srcdocMatch) {
        console.log("🔍 DEBUG: Found srcdoc in iframe HTML, length:", srcdocMatch[1].length);
        console.log("🔍 DEBUG: srcdoc preview:", srcdocMatch[1].substring(0, 100));
      } else {
  ...
  <div class="preview-header">
    ...
    <div class="preview-controls">
      ${canRender ? `<button class="preview-btn" id="viewCodeBtn">📝 View Code</button>` : ""}
      ${block.linkedFiles && block.linkedFiles.length > 0 ? `<button class="preview-btn" id="showLinkedBtn">🔗 Show Linked Files</button>` : ""}
...
`;
    ...
    setTimeout(() => {
      ...
      if (iframe) {
        ...
        for (let i = 0; i < iframe.attributes.length; i++) {
          ...
          attributes[attr.name] = attr.value.length > 100 ? `${attr.value.substring(0, 100)}... (${attr.value.length} total)` : attr.value;
        }
        ...
        console.log("  - srcdoc via getAttribute length:", srcdocValue ? srcdocValue.length : 0);
        console.log("  - srcdoc via property exists:", !!iframe.srcdoc);
        console.log("  - srcdoc via property length:", iframe.srcdoc ? iframe.srcdoc.length : 0);

        if (srcdocValue) {
          console.log("  - srcdoc content preview:", srcdocValue.substring(0, 200));

          // Check if it's properly HTML
          if (srcdocValue.includes("<html") || srcdocValue.includes("<body") || srcdocValue.includes("<div")) {
          ...
          iframe.setAttribute("srcdoc", "<html><body><h1>Manual Test</h1><p>This is a manual test to see if srcdoc works at all.</p></body></html>");

          setTimeout(() => {
            console.log("  - After manual set, srcdoc exists:", !!iframe.srcdoc);
            console.log("  - After manual set, srcdoc length:", iframe.srcdoc ? iframe.srcdoc.length : 0);
          }, 100);
        }
        ...
        try {
          if (iframe.contentDocument) {
            console.log("  - contentDocument.readyState:", iframe.contentDocument.readyState);
            console.log("  - contentDocument.documentElement exists:", !!iframe.contentDocument.documentElement);
            console.log("  - contentDocument.body exists:", !!iframe.contentDocument.body);

            if (iframe.contentDocument.body) {
              console.log("  - contentDocument.body.innerHTML length:", iframe.contentDocument.body.innerHTML.length);
            }
          }
        } catch (e) {
    ...
    setTimeout(() => {
      ...
      if (iframe) {
        ...
        console.log("  - srcdoc length:", iframe.srcdoc ? iframe.srcdoc.length : 0);
        console.log("  - contentDocument exists:", !!iframe.contentDocument);

        if (iframe.contentDocument && iframe.contentDocument.body) {
          console.log("  - body content exists:", iframe.contentDocument.body.innerHTML.length > 0);
          console.log("  - body background:", getComputedStyle(iframe.contentDocument.body).backgroundColor);
        }
      } else {
    ...
    function showEnhancedCodeView() {
      ...
      if (!window.chatInputManager) {
        if ($DebugTestMode) {
          console.error("chatInputManager not available, falling back to simple view");
        }
        showSimpleCodeView();
        return;
      }
      ...
      function displayEnhancedCode() {
        const contentData = window.chatInputManager.largeContentStore.get(tempPreviewId);
      ...
      `;
        ...
        setTimeout(() => {
          ...
          if (codeContainer && window.chatInputManager.createHighlightedCode) {
            codeContainer.innerHTML = window.chatInputManager.createHighlightedCode(contentData.content, contentData.language);
            setupCodeFoldHandlers();
            if ($DebugTestMode) {
              console.log("🔍 Displayed enhanced code view for block:", block.id, "Language:", contentData.language);
            }
          }
        }, 50);
      }
    ...
    function showSimpleCodeView() {
      ...
      <div class="code-view" style="background: #1e1e1e; color: #d4d4d4; padding: 20px; height: 100%; overflow: auto;">
        <pre style="margin: 0;"><code class="language-${block.language || "text"}">${self.escapeHtml(block.content)}</code></pre>
      </div>
    `;
      if (typeof hljs !== "undefined") {
        ...
        if (codeElement) {
          hljs.highlightElement(codeElement);
          if ($DebugTestMode) {
            console.log("🔍 Applied highlighting to simple code view:", block.id);
          }
        }
      }
    }
    ...
    function setupCodeFoldHandlers() {
      overlay.addEventListener("click", (e) => {
        if (e.target.classList.contains("fold-marker") || e.target.classList.contains("fold-placeholder")) {
    ...
    if (viewCodeBtn && canRender) {
      viewCodeBtn.addEventListener("click", function (e) {
        ...
        e.stopPropagation();
        const container = overlay.querySelector(".preview-content-container");
        const isShowingCode = container.querySelector(".code-view-enhanced, .code-view");
        ...
        if (isShowingCode) {
          ...
          if ((block.extension === ".html" || block.extension === ".htm") && block.allFiles) {
            const cssFiles = block.allFiles.filter((f) => f.extension === ".css" || (f.filename && f.filename.toLowerCase().endsWith(".css")));
            const jsFiles = block.allFiles.filter((f) => f.extension === ".js" || (f.filename && f.filename.toLowerCase().endsWith(".js")));
            regeneratedHTML = contentManager.linkFilesToHTML(block.originalContent, cssFiles, jsFiles);
          }
        ...
        `;
          ...
          const isStreaming =
            ...
            (chatManager.streamingFiles &&
              (chatManager.streamingFiles.has(block.id) || chatManager.streamingFiles.has(block.id.replace("block_", "")) || chatManager.streamingFiles.has(`file_${block.id}`)));
            ...
            <div class="code-view-enhanced streaming-enhanced" style="background-color: #1e1e1e; color: #d4d4d4; height: 100%; overflow: auto; padding: 20px 0; position: relative;">
              <div class="code-lines-container" style="display: flex; font-family: 'Consolas', 'Monaco', monospace; font-size: 14px; line-height: 1.5;">
                ...
                <div class="code-content" style="flex: 1; white-space: pre;">
                  <pre style="margin: 0;"><code class="language-${block.language || "text"}" style="display: block;"></code></pre>
                </div>
              </div>
            </div>
          `;
            ...
            function updateLineNumbers(content) {
              ...
              if (newLineCount !== lastLineCount) {
                ...
                for (let i = 1; i <= newLineCount; i++) {
                  lineNumbersHtml.push(`<div class="line-number" style="padding: 0 10px; height: 1.5em;">${i}</div>`);
                }
            ...
            function applySyntaxHighlighting(codeElement, content) {
              ...
              if (typeof hljs !== "undefined") {
                ...
                if (window.chatInputManager && window.chatInputManager.addFoldMarkers) {
            ...
            function addStreamingFoldMarkers(codeElement, language) {
              ...
              const foldPatterns = {
                javascript: [/^[\s]*(?:function|class|if|for|while|switch|try)\s*\(/, /^[\s]*(?:const|let|var)\s+\w+\s*=\s*(?:function|\()/, /^[\s]*\w+\s*:\s*function/],
                python: [
                  /^[\s]*(?:def|class|if|for|while|try|with)\s+/,
                  /^[\s]*@\w+/, // decorators
                ],
                java: [/^[\s]*(?:public|private|protected)?\s*(?:static)?\s*(?:class|interface|enum)\s+/, /^[\s]*(?:public|private|protected)?\s*(?:static)?\s*\w+\s+\w+\s*\(/],
            ...
            const updateStreamingCodeView = () => {
              const updatedFile =
                ...
                chatManager.streamingFiles.get(block.id.replace("block_", "")) ||
              ...
              if (updatedFile && updatedFile.content) {
                ...
                if (!isUserScrolling && codeContainer) {
                  ...
                  const wasNearBottom = codeContainer.scrollTop + codeContainer.clientHeight >= codeContainer.scrollHeight - 100;
          ...
          `;
            ...
            container.streamingInterval = setInterval(() => {
              if (
                ...
                !chatManager.streamingFiles.has(block.id.replace("block_", "")) &&
                ...
                statusIndicator.style.cssText = "position: absolute; top: 10px; right: 10px; color: #4caf50; font-size: 12px;";
    ...
    if (showLinkedBtn && block.linkedFiles && block.linkedFiles.length > 0) {
      showLinkedBtn.addEventListener("click", function (e) {
        e.preventDefault();
        e.stopPropagation();
        const linkedFilesInfo = block.linkedFiles.map((file) => `• ${file.filename} (${file.type || file.extension})`).join("\n");
    ...
    if ($DebugTestMode) {
      console.log("🔍 Fullscreen preview created for block:", block.id, "Executable:", block.isExecutable, "Renderable:", block.isRenderable, "UI structure:", {
        overlayClass: overlay.className,
        containerClass: overlay.querySelector(".preview-content-container")?.className,
        hasIframe: !!overlay.querySelector(".preview-iframe"),
        hasCodeView: !!overlay.querySelector(".code-view, .code-view-enhanced"),
        headerText: overlay.querySelector(".preview-title")?.textContent,
        hasViewCodeBtn: !!overlay.querySelector("#viewCodeBtn"),
        hasShowLinkedBtn: !!overlay.querySelector("#showLinkedBtn"),
      });
    }
  ...
  sanitizeHTML: function (html) {
    console.log("🔍 DEBUG: sanitizeHTML input length:", html ? html.length : 0);
    console.log("🔍 DEBUG: sanitizeHTML input type:", typeof html);

    if (!html) {
      console.warn("🔍 DEBUG: sanitizeHTML received falsy input");
      return "";
    }
    ...
    try {
      ...
      console.log("🔍 DEBUG: DOMParser created");

      const doc = parser.parseFromString(html, "text/html");
      console.log("🔍 DEBUG: Document parsed successfully");
      console.log("🔍 DEBUG: Parsed document has documentElement:", !!doc.documentElement);

      // Remove dangerous elements
      const dangerous = doc.querySelectorAll('script[src], link[href^="http"], iframe');
      console.log("🔍 DEBUG: Found dangerous elements:", dangerous.length);
      dangerous.forEach((el) => el.remove());

      const result = doc.documentElement.innerHTML;
      console.log("🔍 DEBUG: sanitizeHTML output length:", result ? result.length : 0);

      if (!result || result.length === 0) {
        console.error("🔍 DEBUG: sanitizeHTML produced empty result!");
        console.log("🔍 DEBUG: Original HTML:", html.substring(0, 500));
        console.log("🔍 DEBUG: Parsed doc body:", doc.body ? doc.body.innerHTML.substring(0, 500) : "NO BODY");
      }

      return result;
    } catch (error) {
      console.error("🔍 DEBUG: sanitizeHTML error:", error);
      console.log("🔍 DEBUG: Returning original HTML due to error");
      return html; // Return original on error
    }
  },

  escapeAttribute: function (str) {
    console.log("🔍 DEBUG: escapeAttribute input length:", str ? str.length : 0);
    console.log("🔍 DEBUG: escapeAttribute input type:", typeof str);

    if (!str) {
      console.warn("🔍 DEBUG: escapeAttribute received falsy input:", str);
      return "";
    }

    const result = str.replace(/"/g, "&quot;").replace(/'/g, "&#39;");
    console.log("🔍 DEBUG: escapeAttribute output length:", result.length);
    console.log("🔍 DEBUG: escapeAttribute replacements made:", {
      doubleQuotes: (str.match(/"/g) || []).length,
      singleQuotes: (str.match(/'/g) || []).length,
    });
  ...
  showCodePreview: function (blockId) {
    console.log("🔍 === SHOWCODEPREVIEW DEBUG ===");
    ...
    // CRITICAL FIX: Proper ID conversion logic
    let blockElementId = blockId;

    // Handle various ID patterns:
    // file_file_xxxxx -> block_block_xxxxx (streaming files)
    // file_xxxxx -> block_xxxxx (regular files)
    // block_xxxxx -> block_xxxxx (already correct)

    if (blockId.startsWith("file_file_")) {
      // This is a streaming file ID like "file_file_688abaeef1c12"
      // Convert to "block_block_688abaeef1c12"
      blockElementId = blockId.replace("file_file_", "block_block_");
    } else if (blockId.startsWith("file_") && !blockId.startsWith("file_file_")) {
      // This is a regular file ID like "file_xxxxx"
      // Convert to "block_xxxxx"
      blockElementId = blockId.replace("file_", "block_");
    }
    // If it already starts with "block_", leave it as is

    console.log("🔍 Looking for block element with ID:", blockElementId);
    const blockElement = document.getElementById(blockElementId);
    console.log("🔍 Block element found:", !!blockElement);

    if (!blockElement) {
      console.error("🔍 ❌ Block element not found with ID:", blockElementId);

      // Try alternative IDs for debugging
      const altIds = [
        blockId,
        blockId.replace("file_", "block_"),
        blockId.replace("file_file_", "block_block_"),
        blockId.replace("file_file_", "block_"),
        `block_${blockId}`,
        blockId + "_content",
      ];
      ...
      for (const altId of altIds) {
        ...
        console.log(`🔍 Trying alternative ID ${altId}:`, !!altElement);
        if (altElement) {
          console.log("🔍 ✅ Found element with alternative ID:", altId);
          // Use this element and update the blockElementId
          blockElementId = altId;
          const foundElement = altElement;
          console.log("🔍 Using found element:", foundElement);

          // Continue with the found element
          processBlockElement(foundElement, blockElementId);
          return;
        }
      }

      console.error("🔍 ❌ No valid block element found for any ID variation");
      return;
    }

    // Process the found element
    processBlockElement(blockElement, blockElementId);

    function processBlockElement(element, elementId) {
      // Look for the content container within the block
      let contentElement = element.querySelector(".block-content");
      if (!contentElement) {
        // Try other possible selectors
        contentElement = element.querySelector(".preview-content");
        if (!contentElement) {
          contentElement = element.querySelector(".file-content");
        }
      }
      ...
      if (!contentElement) {
        console.error("🔍 ❌ No content element found within block");
        console.log("🔍 Block element HTML:", element.outerHTML.substring(0, 300));

        // Create a content element if it doesn't exist
      ...
      console.log("🔍 Content element style.display:", contentElement.style.display);
      ...
      <div class="code-preview">
        <pre><code class="language-${language}">${contentManager.escapeHtml(block.content)}</code></pre>
      </div>
    `;

      console.log("🔍 Setting new HTML, length:", newHTML.length);
      ...
      if (block.collapsed || contentElement.classList.contains("collapsed")) {
        console.log("🔍 Block is collapsed, expanding it");
        contentElement.classList.remove("collapsed");

        // Update the block state
        block.collapsed = false;

        // Update collapse indicator if it exists
        ...
        console.log("🔍 Block is already expanded");
      }
      ...
      console.log("🔍 === SHOWCODEPREVIEW COMPLETE ===");
    }
  },
  showCode: function (blockId) {
    ...
    if (window.chatInputManager && block) {
      ...
      if (!language || language === "text" || language === "plaintext") {
        language = this.detectLanguageFromContent(block.content || block.originalContent);
        if ($DebugTestMode) {
      ...
      if ($DebugTestMode) {
        //console.log("🔍 Calling chatInputManager.viewFullContent with language:", language);
      }
  ...
  detectLanguageFromContent: function (content) {
    ...
    // CSS detection
    if (/^(\.|#|@media|@import|:root|\*\s*{)/m.test(firstLines) || /{\s*(color|background|margin|padding|font|display|position):/i.test(firstLines)) {
      return "css";
    }

    // JavaScript detection
    if (/^(import|export|const|let|var|function|class|if|for|while)\s/m.test(firstLines) || /\b(console\.log|document\.|window\.|require\(|module\.exports)/i.test(firstLines)) {
      return "javascript";
    }

    // Python detection
    if (/^(import|from|def|class|if|elif|else:|for|while|with|try:|except:|print\()/m.test(firstLines)) {
      return "python";
    }
    ...
    // SQL detection
    if (/^(SELECT|INSERT|UPDATE|DELETE|CREATE|DROP|ALTER|FROM|WHERE|JOIN)\s/im.test(firstLines)) {
      return "sql";
    }

    // XML detection
    if (/^<\?xml/.test(content.trim()) || /<[^>]+>[^<]*<\/[^>]+>/s.test(firstLines)) {
      return "xml";
    }
    ...
    // Java detection
    if (/^(package|import|public\s+class|private\s+class|public\s+static\s+void\s+main)/m.test(firstLines)) {
      return "java";
    }

    // Shell/Bash detection
    if (/^#!/.test(firstLines) || /^(echo|cd|ls|mkdir|rm|cp|mv|chmod|export)\s/m.test(firstLines)) {
      return "bash";
    }
  ...
  downloadContent: function (blockId) {
    ...
    try {
      const mimeType = block.mimeType || this.getMimeType(block.extension || ".txt");
  ...
  copyContent: function (blockId) {
    ...
    navigator.clipboard
      ...
      .then(function () {
        contentManager.showButtonFeedback(blockId, "copy", "✅ Copied", "success");
      })
      .catch(function (err) {
        if ($DebugTestMode) {
          //  console.error("Failed to copy:", err);
        }
        contentManager.showButtonFeedback(blockId, "copy", "❌ Failed", "error");
      });
  },

  showButtonFeedback: function (blockId, action, message, type) {
    const button = document.querySelector(`#${blockId} [data-action="${action}"]`);
    if (!button) return;
  ...
  createContentBlock: function (content, type = "text", filename = "", fileExtension = "") {
    ...
    const isRenderableContent = this.isRenderableContent(content, fileExtension);
    const generatedFilename = filename || this.generateSmartFilename(content, type, fileExtension);
  ...
  renderLegacyBlock: function (block) {
    const previewText = block.content.length > 200 ? block.content.substring(0, 200) + "..." : block.content;
        ...
        <div class="block-header ${block.collapsed ? "collapsed" : ""}">
          <div class="block-info">
            ...
            <div class="block-details">
              ...
              <div class="block-meta">${block.wordCount} words • ${this.formatFileSize(block.charCount)}</div>
            </div>
          </div>
          <div class="block-actions">
            <button class="block-btn primary" data-action="preview">👁️ Preview</button>
  ...
  isRenderableContent: function (content, extension) {
    ...
    // Check by content - look for HTML tags
    const hasWebTags = /<(html|head|body|div|script|style|p|h[1-6]|img|a|ul|ol|li|table|form)/i.test(content);

    // Also check for DOCTYPE
    const hasDoctype = /<!DOCTYPE\s+html/i.test(content);

    return hasWebTags || hasDoctype;
  },
  generateSmartFilename: function (content, type, extension) {
    ...
    if (extension === ".html" || extension === ".htm") {
      ...
      const functionMatch = content.match(/function\s+([a-zA-Z_$][a-zA-Z0-9_$]*)/);
      if (classMatch) {
  ...
  isExecutableFile: function (filename, extension, content) {
    ...
    const executableExts = [".html", ".htm", ".js", ".py", ".php", ".sh", ".bat"];
    ...
    if (content && /<(!DOCTYPE|html|head|body|div|script|style)/i.test(content)) {
      return true;
    }
  ...
  // ENHANCED: Add block method that preserves JSON
  addBlock: function (blockData) {
    console.log("📦 === CONTENTMANAGER.ADDBLOCK (JSON AWARE) ===");
    console.log("📦 blockData:", blockData);

    if (!blockData.id || !blockData.filename) {
      console.error("📦 ❌ Missing required id or filename");
      return false;
    }

    try {
      // Get actual file data from allFiles if available
      let actualFileData = null;
      if (blockData.allFiles && blockData.allFiles.length > 0) {
        actualFileData = blockData.allFiles.find((file) => file.filename === blockData.filename || file.id === blockData.id || blockData.id.includes(file.id));
      }

      // Use actual file data if found
      const sourceData = actualFileData || blockData;

      const block = {
        id: blockData.id,
        filename: blockData.filename,

        // CRITICAL: Preserve JSON storage if available
        originalJSON: sourceData.originalJSON || blockData.originalJSON || null,
        parsedData: sourceData.parsedData || blockData.parsedData || null,
        sections: sourceData.sections || blockData.sections || {},
        metadata: sourceData.metadata || blockData.metadata || {},

        // Display content (reconstructed from JSON if available)
        content: sourceData.sections || blockData.sections || {},
        //originalContent: this.getDisplayContent(sourceData),

        // File properties
        extension: this.getFileExtension(blockData.filename),
        language: sourceData.language || this.getLanguageFromExtension(sourceData.extension),
        type: sourceData.type || this.detectFileTypeFromExtension(sourceData.extension),
        size: 0, // Will be calculated
        wordCount: 0, // Will be calculated
        isExecutable: sourceData.is_executable || this.isExecutableFile(blockData.filename),
        mimeType: this.getFileExtension(blockData.filename),
        collapsed: blockData.collapsed !== undefined ? blockData.collapsed : true,
        linkedFiles: blockData.linkedFiles || [],
        allFiles: blockData.allFiles || [],
        isStreaming: blockData.isStreaming || false,
        isRenderable: false, // Will be calculated

        // Update tracking
        isUpdate: sourceData.isUpdate || false,
        updateMetadata: sourceData.updateMetadata || null,
      };

      // Calculate derived properties
      block.size = block.content.length;
      // block.wordCount = block.content.split(/\s+/).length;
      block.isRenderable = this.isRenderableContent(block.content, block.extension);

      this.blocks.set(blockData.id, block);

      console.log("📦 ✅ Blocks array:", Array.from(this.blocks.values()));
  ...
  detectFileTypeFromExtension: function (extension) {
    const ext = extension.toLowerCase();
    const typeMap = {
      ".html": "html",
      ".htm": "html",
  ...
  // NEW: Get display content with JSON awareness
  getDisplayContent: function (sourceData) {
    // If we have JSON format, reconstruct content
    if (sourceData.parsedData && sourceData.parsedData.sections) {
      return this.reconstructContentFromJSON(sourceData.parsedData);
    }

    // If we have original JSON string, try to parse and reconstruct
    if (sourceData.originalJSON) {
      try {
        const parsed = JSON.parse(sourceData.originalJSON);
        if (parsed.sections) {
          return this.reconstructContentFromJSON(parsed);
        }
      } catch (e) {
    ...
    return sourceData.content || sourceData.originalContent || "";
  },

  // Helper method to get file extension (add if missing)
