...
var historyManager = {
  ...
  searchHistory: async function (query) {
    ...
    // Track which backup conversations we've already loaded to prevent duplicates
    if (!historyContent.dataset.loadedBackupIds) {
      historyContent.dataset.loadedBackupIds = "";
    }
    const loadedBackupIds = historyContent.dataset.loadedBackupIds
      .split(",")
      .filter((id) => id);
    ...
    if (shouldCheckBackup) {
      ...
      try {
        ...
        backupConversations.forEach((conversation) => {
          // Check if this conversation is already in the DOM or already loaded from backup
          const existingItem = historyContent.querySelector(
            `[data-conversation-id="${conversation.conversation_id}"]`
          );

          // Check if we've already loaded this backup conversation
          const isAlreadyLoaded = loadedBackupIds.includes(
            conversation.conversation_id
          );

          if (!existingItem && !isAlreadyLoaded) {
            ...
            // Add to our list of loaded backup IDs to prevent duplicates
            loadedBackupIds.push(conversation.conversation_id);
            historyContent.dataset.loadedBackupIds = loadedBackupIds.join(",");
