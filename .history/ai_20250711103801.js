...
var aiWindowState = {
  ...
  setupMessageProcessor: function () {
    ...
    // Process control messages every 200ms (less aggressive)
      ...
      try {
        ...
        if (messageData) {
          ...
          if (
            ...
            // Clear the message immediately to prevent reprocessing
    ...
    // Check less frequently to reduce conflicts
    setInterval(messageProcessor, 200);

    // Listen for direct postMessage (keep this for immediate response)
    ...
    // REMOVED: Auto-restore interval that was causing conflicts
    // REMOVED: Periodic state broadcasting that was causing state mismatches
  },
  ...
  processRestoreMessage: function () {
    ...
    // Only process if actually minimized and not already restoring
    if (!this.isMinimized || this.restoreInProgress) {
      console.log("❌ Ignoring restore - not minimized or already in progress");
      return;
    }
    ...
    // Execute restore with delay to ensure clean state
    setTimeout(() => {
