...
class EnhancedAudioCapture {
  ...
  async captureWindowsSystemAudio() {
    if ($DebugTestMode) {
      console.log("🎵 [044] Trying Windows WASAPI loopback method 1...");
    }

    try {
      if ($DebugTestMode) {
        console.log("🎵 [045] Creating Windows method 1 constraints...");
      }
      ...
      if ($DebugTestMode) {
        console.log("🎵 [046] ✅ Windows WASAPI loopback method 1 successful");
        console.log(
          "🎵 [047] Windows stream tracks:",
      ...
      if ($DebugTestMode) {
        console.log(
          "🎵 [048] ⚠️ Windows WASAPI method 1 failed:",
          error.message
        );
        console.log("🎵 [049] Trying Windows method 2...");
      }

      // Method 2: Try with different constraints
      try {
        if ($DebugTestMode) {
          console.log("🎵 [050] Creating Windows method 2 constraints...");
        }
        ...
        if ($DebugTestMode) {
          console.log("🎵 [051] ✅ Windows method 2 successful");
          console.log(
            "🎵 [052] Windows method 2 stream tracks:",
            stream.getTracks().length
          );
        }
        return stream;
        ...
        if ($DebugTestMode) {
          console.log(
            "🎵 [053] ❌ All Windows methods failed:",
            error2.message
          );
          console.log("🎵 [054] Windows error 2 details:", error2);
        }
        return null;
      }
    }
  ...
  async createMixedStream() {
    if ($DebugTestMode) {
      console.log(
        "🎵 [147] Creating mixed audio stream (system + microphone)..."
      );
      console.log("🎵 [148] Step 1: Getting system audio...");
    }
    ...
    try {
      ...
      if ($DebugTestMode) {
        console.log(
          ...
          systemStream ? "✅ Success" : "❌ Failed"
        );
        console.log("🎵 [150] Step 2: Getting microphone...");
      }
      ...
      if ($DebugTestMode) {
        console.log(
          ...
          micStream ? "✅ Success" : "❌ Failed"
        );
        console.log("🎵 [152] Evaluating stream combination options...");
      }
      ...
      if (!systemStream) {
        if ($DebugTestMode) {
          console.log(
            "🎵 [153] ⚠️ No system audio available, using microphone only"
          );
        }
      ...
      if (!micStream) {
        if ($DebugTestMode) {
          console.log(
            "🎵 [154] ⚠️ No microphone available, using system audio only"
          );
        }
      ...
      if ($DebugTestMode) {
        console.log("🎵 [155] Both streams available, creating audio mixer...");
        console.log("🎵 [156] Creating audio context for mixing...");
      }
      ...
      if ($DebugTestMode) {
        console.log("🎵 [157] Creating media stream sources...");
      }
      ...
      if ($DebugTestMode) {
        console.log(
          "🎵 [164] ❌ Failed to create mixed stream:",
          error.message
        );
        console.log("🎵 [165] Mixed stream error details:", error);
        console.log("🎵 [166] Attempting microphone fallback...");
      }
      ...
      try {
        const micStream = await this.getMicrophoneStream();
        if ($DebugTestMode) {
          console.log("🎵 [167] ⚠️ Fallback: Using microphone only");
        }
        this.micStream = micStream;
        return micStream;
      } catch (micError) {
        if ($DebugTestMode) {
          console.log(
            "🎵 [168] ❌ Even microphone fallback failed:",
            micError.message
          );
          console.log("🎵 [169] Microphone fallback error details:", micError);
        }
        throw micError;
      }
    }
