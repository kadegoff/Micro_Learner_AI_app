...
class EnhancedAudioCapture {
  ...
  async captureWithDesktopCapturer() {
    if ($DebugTestMode) {
      console.log("🎵 [013] METHOD 1: Electron Desktop Capturer starting...");
    }

    // ✅ FIXED: Better Electron environment detection
    const isElectron = !!(
      window.electronAPI ||
    ...
    try {
      ...
      // ✅ CRITICAL FIX: Handle the response properly based on the method
      if (result.method === "native-desktop-capturer" && result.source) {
        if ($DebugTestMode) {
          console.log("🎵 [020] Single source returned:", result.source.name);
          console.log("🎵 [021] Source ID:", result.source.id);
        }

        try {
          // ✅ FIXED: Use proper Chrome media source constraints for Electron
          const constraints = {
            audio: {
              mandatory: {
            ...
            video: false, // ✅ IMPORTANT: No video for audio-only capture
          };

          if ($DebugTestMode) {
            console.log("🎵 [022] Using constraints:", constraints);
          }

          // ✅ CRASH-SAFE: Add timeout and better error handling
          const stream = await Promise.race([
            navigator.mediaDevices.getUserMedia(constraints),
            new Promise((_, reject) =>
              setTimeout(
                () => reject(new Error("getUserMedia timeout after 8s")),
                8000
              )
            ),
          ]);
          ...
          if ($DebugTestMode) {
            console.log("🎵 [023] getUserMedia completed successfully");
          }

          // ✅ VALIDATE: Check if stream has audio tracks
          if (stream && stream.getAudioTracks().length > 0) {
            const audioTrack = stream.getAudioTracks()[0];

            if ($DebugTestMode) {
              console.log("🎵 [024] ✅ Desktop capturer successful!");
              console.log("🎵 [025] Audio track label:", audioTrack.label);
              console.log(
                "🎵 [026] Audio track settings:",
                audioTrack.getSettings()
              );
              console.log(
                "🎵 [027] Total audio tracks:",
                stream.getAudioTracks().length
              );
            }

            // ✅ VALIDATE: Check if this looks like actual system audio
            ...
            if (isLikelyMicrophone) {
              if ($DebugTestMode) {
                console.log(
                  "🎵 [028] ⚠️ Rejecting - appears to be microphone:",
                  audioTrack.label
                );
              }
            ...
            this.currentMethod = "Desktop Capturer (Native)";
            return stream;
          } else {
            if ($DebugTestMode) {
              console.log("🎵 [029] ⚠️ Stream has no audio tracks");
            }
            if (stream) {
              stream.getTracks().forEach((track) => track.stop());
            }
          ...
          if ($DebugTestMode) {
            console.log(
              "🎵 [030] ❌ getUserMedia failed:",
              streamError.message
            );
            console.log("🎵 [031] Error details:", streamError);
          }

          // ✅ SPECIFIC ERROR HANDLING
          if (streamError.message.includes("timeout")) {
            console.log("🎵 [032] ⚠️ Timeout - system may be slow to respond");
          } else if (streamError.message.includes("NotAllowedError")) {
            console.log("🎵 [033] ⚠️ Permission denied for desktop capture");
          } else if (streamError.message.includes("NotFoundError")) {
            console.log("🎵 [034] ⚠️ Desktop capture source not found");
          }
      ...
      // ✅ HANDLE: Multiple sources array (fallback for older implementations)
      else if (
        result.sources &&
        ...
        if ($DebugTestMode) {
          ...
          result.sources.forEach((source, index) => {
            console.log(
              `🎵 [036.${index}] Source: ${source.name} (${source.id})`
            );
          });
        }
        ...
        for (let i = 0; i < result.sources.length; i++) {
          ...
          try {
            if ($DebugTestMode) {
              console.log(`🎵 [037.${i}] Trying source: ${source.name}`);
            }
            ...
            const stream = await Promise.race([
              navigator.mediaDevices.getUserMedia(constraints),
              new Promise((_, reject) =>
                setTimeout(() => reject(new Error("Source timeout")), 5000)
              ),
            ]);
            ...
            if (stream && stream.getAudioTracks().length > 0) {
              if ($DebugTestMode) {
                console.log(
                  `🎵 [038.${i}] ✅ Source ${source.name} successful`
                );
              }
              this.systemStream = stream;
              this.currentMethod = `Desktop Capturer (${source.name})`;
              return stream;
            } else {
              if ($DebugTestMode) {
                console.log(
                  `🎵 [039.${i}] ⚠️ Source ${source.name} has no audio`
                );
              }
              if (stream) {
                stream.getTracks().forEach((track) => track.stop());
            ...
            if ($DebugTestMode) {
              console.log(
                `🎵 [040.${i}] ⚠️ Source ${source.name} failed:`,
                sourceError.message
              );
            }
          }
        ...
        if ($DebugTestMode) {
          console.log("🎵 [041] ❌ All sources from array failed");
        }
      }
      ...
      // ✅ HANDLE: Unexpected result format
        ...
        if ($DebugTestMode) {
          console.log("🎵 [042] ❌ Unexpected result format:", result);
          console.log(
            "🎵 [043] Expected: success=true with source or sources array"
          );
        }
      }
      ...
      if ($DebugTestMode) {
        console.log(
          "🎵 [044] ❌ Desktop capturer method failed:",
          error.message
        );
        console.log("🎵 [045] Full error:", error);
      }

      // ✅ RECOVERY: Don't crash, just return null to try next method
      return null;
    }
  }
