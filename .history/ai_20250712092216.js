...
const checkScreenshotAttachments = () => {
  try {
    ...
    if (screenshotData) {
      ...
      if (Date.now() - message.timestamp < 5000 && message.type === "ADD_SCREENSHOT_ATTACHMENT") {
...
var chatInputManager = {
  ...
  chatInputManager.addScreenshotAttachment = function(attachment) {
  console.log("📸 Adding screenshot attachment to input:", attachment);
  ...
  if (chatInput) {
    chatInput.focus();
    chatInput.placeholder = "Add a message about this screenshot...";
  }
...
chatInputManager.showScreenshotPreview = function(attachment) {
  ...
  preview.innerHTML = `
    ...
    <div class="screenshot-preview-image">
      <img src="data:image/png;base64,${attachment.data}" alt="Screenshot preview" />
    </div>
  `;
  ...
  const chatContainer = document.querySelector(".chat-input-container, #chatContainer");
  if (chatContainer) {
...
chatInputManager.removeScreenshotPreview = function() {
...
chatInputManager.formatFileSize = function(bytes) {
  if (bytes < 1024) return bytes + " B";
  if (bytes < 1024 * 1024) return Math.round(bytes / 1024) + " KB";
  return Math.round(bytes / (1024 * 1024)) + " MB";
};
...
chatInputManager.sendMessage = function() {
  const input = document.getElementById("chatInput");
