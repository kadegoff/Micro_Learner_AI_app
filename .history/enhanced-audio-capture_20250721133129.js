...
async function testStreamForSystemAudio(stream, duration = 1500) {
  if ($DebugTestMode) {
  ...
  try {
    ...
    return new Promise((resolve) => {
      ...
      if ($DebugTestMode) {
        console.log(`🎵 Sampling audio for ${duration}ms (${maxSamples} samples)...`);
      }
      ...
      const testInterval = setInterval(() => {
        ...
        if ($DebugTestMode && samples % 5 === 0) {
          console.log(`🎵 Sample ${samples}/${maxSamples}: ${average.toFixed(2)} (max: ${maxLevel.toFixed(2)})`);
        }
        ...
        if (samples >= maxSamples) {
          ...
          if (isMicrophoneOnly) {
            if ($DebugTestMode) {
              console.log(`🎵 ❌ Rejecting: Appears to be microphone-only device`);
            }
            resolve(false);
            return;
          }
