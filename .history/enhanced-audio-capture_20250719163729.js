...
class EnhancedAudioCapture {
  ...
  // Method 1: Electron Desktop Capturer
  async captureWithDesktopCapturer() {
    ...
    // Check if we're in Electron environment with desktopCapturer
    if (typeof require === "undefined") {
      if ($DebugTestMode) {
        console.log(
          "🎵 [015] ⚠️ Not in Electron environment - require not available"
        );
      }
      return null;
    }
    ...
    try {
      ...
      if (!desktopCapturer) {
        if ($DebugTestMode) {
          console.log("🎵 [016] ⚠️ desktopCapturer not available");
        }
        return null;
      }

      if ($DebugTestMode) {
        console.log("🎵 [017] Getting desktop sources...");
      }
      ...
      if (!sources || sources.length === 0) {
        if ($DebugTestMode) {
          console.log("🎵 [018] ⚠️ No desktop sources found");
        }
        return null;
      }

      if ($DebugTestMode) {
        console.log("🎵 [019] Found", sources.length, "desktop sources");
        sources.forEach((source, index) => {
          console.log(
            `🎵 [020.${index}] Source: ${source.name} (${source.id})`
          );
        });
      }
      ...
      for (let i = 0; i < sources.length; i++) {
        ...
        try {
          if ($DebugTestMode) {
            console.log(`🎵 [021.${i}] Trying source: ${source.name}`);
          }
          ...
          if (stream && stream.getAudioTracks().length > 0) {
            if ($DebugTestMode) {
              console.log(
                `🎵 [022.${i}] ✅ Desktop capturer successful with source: ${source.name}`
              );
            }
            return stream;
          } else {
            if ($DebugTestMode) {
              console.log(
                `🎵 [023.${i}] ⚠️ Source ${source.name} has no audio tracks`
              );
            }
            if (stream) {
          ...
          if ($DebugTestMode) {
            console.log(
              `🎵 [024.${i}] ⚠️ Source ${source.name} failed:`,
              sourceError.message
            );
          }
        }
      ...
      if ($DebugTestMode) {
        console.log("🎵 [025] ❌ All desktop sources failed");
      }
      return null;
    } catch (error) {
      if ($DebugTestMode) {
        console.log("🎵 [026] ❌ Desktop capturer failed:", error.message);
        console.log("🎵 [027] Desktop capturer error details:", error);
      }
      return null;
    }
  }
