...
class EnhancedAudioCapture {
  ...
  async safeCaptureDisplayMedia() {
    this.log("info", "METHOD 1: Ultra-safe getDisplayMedia");
    ...
    try {
      ...
      if (stream && stream.getAudioTracks().length > 0) {
        this.log("info", "Safe display media successful");
        return this.trackStream(stream);
      }

      this.log("warn", "Display media has no audio tracks");
      if (stream) {
        stream.getTracks().forEach((track) => track.stop());
      }
      return null;
    } catch (error) {
      this.log("error", "Safe display media failed:", error.message);
      return null;
    }
  }

  // Enhanced display media with fallbacks
  async captureSystemAudioEnhanced() {
    this.log("info", "METHOD 1: Enhanced Display Media");
    ...
    try {
      ...
      if (stream && stream.getAudioTracks().length > 0) {
        this.log("info", "Enhanced display media successful");
        return this.trackStream(stream);
      }
    } catch (error) {
  ...
  async captureSystemAudioPlatformSpecific() {
    this.log("info", "METHOD 2: Platform-specific audio capture");
...
if (typeof window !== "undefined") {
  ...
  window.initializeEnhancedAudioCapture = initializeEnhancedAudioCapture;
...
if (typeof module !== "undefined" && module.exports) {
  module.exports = {
    ...
    testAudioSources,
    initializeEnhancedAudioCapture,
  };
}
