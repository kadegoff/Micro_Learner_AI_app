...
setupMessageProcessor: function () {
  ...
  // Simple, direct message checking (like transcript window)
    ...
    try {
      ...
      if (messageData) {
        ...
        if (Date.now() - message.timestamp < 5000 && 
            (message.type === "RESTORE_AI" || message.type === "AI_RESTORE_REQUEST")) {
  ...
  // FORCE restore regardless of internal state checks
  // (The user clicked restore, so restore!)
  
  // Notify BEFORE showing the window
  this.notifyControlCenter("AI_RESTORING");

  // Show window immediately (no complex state checks)
  document.body.style.display = "block";
  document.body.style.visibility = "visible";
  ...
  // Complete restoration after brief delay
  setTimeout(() => {
    try {
      window.focus();
