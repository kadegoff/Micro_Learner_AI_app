...
var aiWindowState = {
  ...
  setupMessageProcessor: function () {
    console.log("📨 Setting up centralized message processor");

    const messageProcessor = () => {
      // Don't skip if restore is in progress - allow recovery
      try {
        ...
        for (let key of messageKeys) {
          ...
          if (messageData) {
            ...
            // Accept ANY restore-related message type
            const isRestoreMessage =
              message.type === "RESTORE_AI" ||
              message.type === "AI_RESTORE_REQUEST" ||
              message.type === "RESTORE_WINDOW" ||
              message.action === "restore" ||
              message.action === "show" ||
              message.action === "unminimize";

            if (Date.now() - message.timestamp < 10000 && isRestoreMessage) {
              console.log("📨 Processing restore message:", message);

              // Clear ALL message keys to prevent conflicts
              messageKeys.forEach((k) => localStorage.removeItem(k));

              // Reset any stuck flags before processing
              if (this.restoreInProgress) {
                console.log("🔧 Clearing stuck restoreInProgress flag");
                this.restoreInProgress = false;
              }

              this.processRestoreMessage();
              return; // Exit after processing first valid message
            }
          }
        }

        // Also check for simple flags
        ...
        // Reset flags on any error
        this.restoreInProgress = false;
      }
    };

    // Check every 300ms (slightly less aggressive)
    setInterval(messageProcessor, 300);
    ...
    window.addEventListener("message", (event) => {
      if (event.data && event.data.source === "control") {
        ...
        if (isRestoreMessage) {
          ...
          this.restoreInProgress = false; // Reset flag
          this.processRestoreMessage();
        }
      }
    });
  ...
  // FIXED: Better restore function
  restoreFromMinimized: function () {
    console.log("🔄 AI restoreFromMinimized called");
    ...
    // Complete restoration after animation
    setTimeout(() => {
    ...
    // SAFETY: Clear restore flag after timeout in case something goes wrong
    setTimeout(() => {
      if (this.restoreInProgress) {
        console.log("⚠️ Clearing stuck restoreInProgress flag after timeout");
        this.restoreInProgress = false;
      }
    }, 2000);
  },
