...
async function captureSystemAudioDeviceEnum() {
  if ($DebugTestMode) {
    console.log("🎵 [115] METHOD 4: System audio device enumeration starting...");
  ...
  try {
    ...
    if ($DebugTestMode) {
      ...
      devices.filter(d => d.kind === "audioinput").forEach((device, i) => {
        console.log(`🎵 Device ${i}: "${device.label}" | ID: ${device.deviceId}`);
      });
    }

    // ✅ FIXED: Prioritize actual system audio devices
    const systemDevices = devices.filter((device) => {
      ...
      // ✅ PRIORITY 1: Explicit system audio devices (BEST)
      const isSystemAudio = (
      ...
      // ✅ PRIORITY 2: Default devices (MAY work for system audio)
      const isDefaultDevice = label.includes("default");
      ...
      return isSystemAudio || isDefaultDevice;
    });

    // ✅ SORT: System audio devices first, then default devices
    systemDevices.sort((a, b) => {
      const aLabel = a.label.toLowerCase();
      const bLabel = b.label.toLowerCase();
      
      // Stereo Mix gets highest priority
      if (aLabel.includes("stereo mix")) return -1;
      if (bLabel.includes("stereo mix")) return 1;
      
      // Other system audio devices second
      const aIsSystemAudio = aLabel.includes("loopback") || aLabel.includes("monitor") || aLabel.includes("system");
      const bIsSystemAudio = bLabel.includes("loopback") || bLabel.includes("monitor") || bLabel.includes("system");
      
      if (aIsSystemAudio && !bIsSystemAudio) return -1;
      if (bIsSystemAudio && !aIsSystemAudio) return 1;
      
      // Default devices last
      return 0;
    });

    if ($DebugTestMode) {
      console.log(`🎵 [119] Found ${systemDevices.length} potential system audio devices (prioritized):`);
      systemDevices.forEach((device, index) => {
        console.log(`🎵 [120.${index}] System device: ${device.label || "No label"} | ${device.deviceId}`);
      });
    }
    ...
    for (let i = 0; i < systemDevices.length; i++) {
      ...
      try {
        if ($DebugTestMode) {
          console.log(`🎵 [121.${i}] Trying device: ${device.label || device.deviceId}`);
          console.log(`🎵 [122.${i}] Creating constraints for system device...`);
        }
        ...
        if ($DebugTestMode) {
          console.log(`🎵 [123.${i}] System device stream obtained, testing for audio...`);
        }

        // ✅ CRITICAL: Actually test for audio signal
        const hasAudio = await testStreamForSystemAudio(stream, 1500);

        if (hasAudio) {
          if ($DebugTestMode) {
            console.log(`✅ Found working system audio device: ${device.label}`);
          }
          return stream;
        } else {
          if ($DebugTestMode) {
            console.log(`🎵 [125.${i}] Device ${device.label} has no audio signal, trying next...`);
          }
        ...
        if ($DebugTestMode) {
          console.log(`🎵 [126.${i}] ⚠️ Device ${device.label || device.deviceId} failed:`, deviceError.message);
        }
      }
    }
  ...
  async testStreamForSystemAudio(stream, duration = 1500) {
    ...
    try {
      ...
      // ✅ CRITICAL: Test for actual audio signal
      ...
      return new Promise((resolve) => {
        ...
        if ($DebugTestMode) {
          console.log(
            `🎵 Sampling audio for ${duration}ms (${maxSamples} samples)...`
          );
        }
        ...
        const testInterval = setInterval(() => {
          ...
          if (samples >= maxSamples) {
            ...
            // ✅ FIXED THRESHOLD: Be more sensitive for system audio
            // System audio is often quieter than microphone
            const threshold = 2.0; // Lowered from 5.0
            ...
            // ✅ ADDITIONAL CHECK: Reject obvious microphone-only devices
            const trackLabel = audioTrack.label.toLowerCase();
            const isMicrophoneOnly =
              ...
              maxLevel < 10.0; // Very low signal suggests mic in quiet room

            if (isMicrophoneOnly) {
              if ($DebugTestMode) {
                console.log(
                  `🎵 ❌ Rejecting: Appears to be microphone-only device`
                );
              }
              resolve(false);
              return;
