...
var aiWindowState = {
  ...
  setupPlatformSpecificStyles: function () {
    // Same as transcript window - simple and clean
  ...
  setupEventListeners: function () {
    // CRITICAL: Copy the transcript window's simple event listener setup
    ...
    // Emergency restore shortcut (Ctrl+Shift+A)
    document.addEventListener("keydown", (e) => {
      if (e.ctrlKey && e.shiftKey && e.code === "KeyA") {
        console.log("🚨 Emergency AI restore triggered");
        this.restoreFromMinimized();
      }
    });
  },

  // CRITICAL: Copy the transcript's heartbeat system
  ...
  setupCommunication: function () {
    // CRITICAL: Copy the transcript's communication setup

    // Listen for restore messages from control center
    window.addEventListener("message", (event) => {
      console.log("🔍 AI: Received message:", event.data);
      if (event.data && event.data.type === "RESTORE_AI") {
        console.log("🔍 AI: Processing RESTORE_AI message");
        this.restoreFromMinimized();
      }
    });

    // Monitor localStorage for control messages - SIMPLIFIED
        ...
        if (messageData) {
          ...
          if (Date.now() - message.timestamp < 3000) {
            if (
              ...
              console.log("🔍 AI: Found control message:", message.type);
              this.restoreFromMinimized();
            }
          }
        }

        // Check for simple restore flags
        const restoreFlags = [
          "ai_restore_message",
          "restore_ai",
          "show_ai_window",
          ...
          if (value) {
            try {
              ...
              if (parsed.timestamp && Date.now() - parsed.timestamp < 3000) {
                console.log("🔍 AI: Found restore flag:", flag);
                this.restoreFromMinimized();
              }
            } catch (e) {
              if (value === "true") {
                console.log("🔍 AI: Found simple restore flag:", flag);
    ...
    // Check for control messages every 500ms (same as transcript)
  ...
  checkStartupState: function () {
    ...
    if (sessionMinimized === "true") {
      ...
      console.log("🔍 AI: Starting normally");
  ...
  // CRITICAL: Copy transcript's simple minimize logic
  minimizeToControl: function () {
    ...
    console.log("🔽 AI: Starting minimize animation...");

    setTimeout(() => {
      console.log("🔽 AI: Hiding window after animation");
      document.body.style.display = "none";
      document.body.classList.remove("minimizing");
      this.isMinimized = true;

      // Set session flag (critical for proper state tracking)
      ...
      console.log("🔽 AI: Minimize complete");
    }, 400); // Match animation duration
  },

  // CRITICAL: Copy transcript's simple restore logic
  restoreFromMinimized: function () {
    ...
    console.log("🔼 AI: Starting restore process");
      ...
      methods.forEach((method) => {
        if (window.electronAPI[method]) {
          try {
            ...
            console.log(`✅ AI: ${method} called successfully`);
          } catch (e) {
    ...
    console.log("🔼 AI: Starting restore animation...");
    ...
    setTimeout(() => {
      ...
      try {
        ...
        console.log("⚠️ AI: window.focus() failed:", e);
      }

      // Notify completion
      this.notifyControlCenter("AI_RESTORED");
      console.log("🔼 AI: Restore complete");
    }, 400); // Match animation duration
  },

  // Copy transcript's notification system
  notifyControlCenter: function (messageType) {
    ...
    try {
      ...
      console.log("📡 AI: Notification sent:", messageType);
  ...
  handleResize: function () {
    ...
    if (isFullscreen !== this.isMaximized) {
      ...
      document.body.classList.toggle("maximized", isFullscreen);
      this.updateMaximizeButton();
    }
  },
  ...
  saveWindowState: function () {
    ...
    localStorage.setItem("ai_window_state", JSON.stringify(state));
  },
};
