...
var chatInputManager = {
  ...
  showFallbackPreview: function (attachment) {
    ...
    // ✅ FIXED: Use attachment.data instead of undefined cleanData
  ...
  <div class="screenshot-preview-header">
    ...
    <span class="screenshot-filename">${attachment.filename}</span>
    <span class="screenshot-size">${this.formatFileSize(attachment.size)}</span>
    <button class="remove-screenshot-btn" onclick="chatInputManager.removeScreenshotPreview()">✕</button>
  </div>
  <div class="screenshot-preview-image">
    <img src="${imageData}" 
         ...
         onerror="console.error('📸 ❌ Image failed to load:', this.src.substring(0, 100)); this.style.display='none'; this.parentNode.innerHTML='<p>📷 Preview failed to load</p>'" />
  </div>
`;

    // ✅ ADDED: Actually insert the preview into DOM
    const chatContainer = document.querySelector(
      ".chat-input-container, #chatContainer"
    );
    if (chatContainer) {
  ...
  showScreenshotPreview: function (attachment) {
    ...
    if (typeof attachment.data !== "string") {
      console.error(
        "📸 attachment.data is not a string:",
    ...
    console.log("📸 Data sample:", cleanData.substring(0, 50));
    console.log(
      "📸 Data ends with:",
      cleanData.substring(cleanData.length - 10)
    );
    ...
    // CREATE the preview container element (this was missing!)
    const previewContainer = document.createElement("div");
...
`;
    ...
    // ✅ ENHANCED: Better container selection with detailed logging
    ...
    console.log(
      ...
      chatContainer.className || chatContainer.id
    );

    try {
      if (chatContainer === document.body) {
        // ✅ ENHANCED: For body, use fixed positioning
      ...
      // ✅ ADDED: Verify insertion
      setTimeout(() => {
        const insertedElement = document.getElementById("screenshotPreview");
        if (insertedElement) {
          const rect = insertedElement.getBoundingClientRect();
          console.log("📸 ✅ Preview verified in DOM at:", rect);
          if (rect.width === 0 || rect.height === 0) {
            console.warn(
              "📸 ⚠️ Preview has zero dimensions, might be hidden by CSS"
            );
          }
        } else {
          console.error("📸 ❌ Preview not found in DOM after insertion");
        }
      }, 100);
    } catch (error) {
