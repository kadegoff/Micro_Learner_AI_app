...
notifyControlCenter: function (messageType) {
  ...
  try {
    ...
    window.dispatchEvent(new CustomEvent('aiWindowStateChange', {
  ...
  setupCommunication: function () {
    ...
    // Monitor localStorage for control messages
    const checkControlMessages = () => {
      try {
        ...
        if (messageData) {
          ...
          if (
            Date.now() - message.timestamp < 2000 &&
    ...
    // Check for control messages every 500ms
    setInterval(checkControlMessages, 500);

    // Only hide if explicitly minimized in current session
    ...
    if (sessionMinimized === "true") {
      document.body.style.display = "none";
      this.isMinimized = true;
      console.log("AI starting minimized from current session");
    } else {
      localStorage.removeItem("ai_minimized");
      this.isMinimized = false;
