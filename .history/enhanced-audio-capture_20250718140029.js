...
class EnhancedAudioCapture {
  ...
  async safeCaptureDisplayMedia() {
    if ($DebugTestMode) {
      console.log("🎵 [013] ULTRA-SAFE display media starting...");
    }

    try {
      // CRASH FIX: Remove ALL Chrome-specific constraints
      const ultraSafeConstraints = {
        video: false,
        audio: true, // Just basic audio, no advanced options
      };

      if ($DebugTestMode) {
        console.log("🎵 [015] Ultra-safe constraints:", ultraSafeConstraints);
      }
      ...
      if (stream && stream.getAudioTracks().length > 0) {
        if ($DebugTestMode) {
          console.log("🎵 [019] ✅ Ultra-safe display media successful");
        }
        return stream;
      }
      ...
      if ($DebugTestMode) {
        console.log(
          "🎵 [023] ❌ Ultra-safe display media failed:",
          error.message
        );
      }
      return null;
  ...
  // CRASH FIX: Replace captureSystemAudioEnhanced with this minimal version:
  async captureSystemAudioEnhanced() {
    if ($DebugTestMode) {
      console.log("🎵 [025] MINIMAL system audio starting...");
    }

    try {
      // Method 1: Try basic display media first
      const stream = await this.safeCaptureDisplayMedia();
      if (stream) {
        if ($DebugTestMode) {
          console.log("🎵 [028] ✅ Basic display media successful");
        }
        return stream;
      }

      // Method 2: Fall back to microphone only
      if ($DebugTestMode) {
        console.log("🎵 [030] Falling back to microphone...");
      }
      ...
      if ($DebugTestMode) {
        console.log("🎵 [032] ✅ Microphone fallback successful");
      }
      return micStream;
    } catch (error) {
      if ($DebugTestMode) {
        console.log("🎵 [033] ❌ All methods failed:", error.message);
      }
      return null;
    }
  }
  ...
  async captureWindowsSystemAudio() {
    ...
    try {
      // Remove all the native electron stuff that's causing crashes
      const stream = await navigator.mediaDevices.getDisplayMedia({
        video: false,
        audio: true,
      });
      ...
      if (stream && stream.getAudioTracks().length > 0) {
        if ($DebugTestMode) {
          console.log("🎵 [047] ✅ Windows safe capture successful");
        }
        return stream;
      }
    } catch (error) {
    ...
    try {
      ...
      if (systemStream && micStream) {
        ...
        try {
          ...
          if (systemStream) {
            console.log("🎵 System audio WILL be transcribed by Vosk"); // ✅ ADD
            this.finalStream = systemStream;
            return systemStream; // ✅ This stream goes to setupCrashSafeAudioProcessing()
          }
        } catch (mixError) {
          if ($DebugTestMode) {
            console.log(
