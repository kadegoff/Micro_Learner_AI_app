...
// Replace your existing minimize button event listener with this:
...
if (minimizeBtn) {
  minimizeBtn.addEventListener("click", function (e) {
    ...
    console.log("🔽 Minimize button clicked - starting minimize process");

    // Temporarily disable glitch prevention
    if (window.preventMinimize) {
      window.preventMinimize = false;
    }

    // Force the minimize to happen
...
var aiWindowState = {
  ...
  minimizeToControl: function () {
    ...
    // Notify control center that AI is being minimized
    ...
    if (window.electronAPI && window.electronAPI.minimizeWindow) {
      ...
      setTimeout(() => {
        ...
        document.body.style.visibility = "hidden"; // Additional hide method
        ...
        // Store minimized state for current session only
...
restoreFromMinimized: function () {
  console.log("🔄 AI restoreFromMinimized called, isMinimized:", this.isMinimized);
  ...
  // Clear minimized state from all storage locations
  ...
  // Notify control center that AI is being restored
  ...
  // In ai.js - Replace the notifyControlCenter method in aiWindowState object

  notifyControlCenter: function (messageType) {
    console.log("🔔 Notifying control center:", messageType);

    try {
      // Store the state immediately in localStorage with control-specific key
      if (messageType === "AI_MINIMIZED") {
        localStorage.setItem("control_ai_minimized", "true");
        console.log("✅ Set control_ai_minimized to true");
      } else if (messageType === "AI_RESTORED") {
        localStorage.removeItem("control_ai_minimized");
        console.log("✅ Removed control_ai_minimized");
      }

      // If control center is in the same app, use postMessage
      ...
      // Also try localStorage for cross-window communication
      const messageData = {
        type: messageType,
        timestamp: Date.now(),
        source: "ai",
      ...
      console.log("📤 Sent message via localStorage:", messageData);

      // Clear message after a longer delay to ensure it's received
      setTimeout(() => {
      ...
      window.dispatchEvent(
        new CustomEvent("aiWindowStateChange", {
          detail: { type: messageType, source: "ai" },
        })
      );
    } catch (error) {
...
function setupWindowControls() {
  ...
  if (minimizeBtn) {
    ...
    newMinimizeBtn.addEventListener(
      "click",
      function (e) {
        e.preventDefault();
        e.stopPropagation();
        ...
        console.log("🔽 MINIMIZE BUTTON CLICKED - FORCING MINIMIZE");

        // Immediately disable all window operation blocking
        window.preventMaximize = false;
        window.preventMinimize = false;

        // Force minimize with a slight delay to ensure state is cleared
        setTimeout(() => {
          aiWindowState.minimizeToControl();
        }, 10);
