...
class EnhancedAudioCapture {
  constructor() {
    ...
    // ADDED: Resource tracking for cleanup
    this.resources = {
      streams: new Set(),
      contexts: new Set(),
      timers: new Set(),
  ...
  // FIXED: Ultra-safe display media constraints
  async safeCaptureDisplayMedia() {
    ...
    try {
      // CRITICAL FIX: Use only the safest constraints
      const safeConstraints = {
        video: false, // FIXED: Video OFF to prevent crashes
        audio: {
          ...
          sampleRate: 16000,
          channelCount: 1,
          // CRITICAL: NO Chrome-specific options that cause crashes
        },
      };

      // Add timeout protection
      const stream = await Promise.race([
        navigator.mediaDevices.getDisplayMedia(safeConstraints),
        new Promise((_, reject) =>
          setTimeout(() => reject(new Error("Display media timeout")), 5000)
        ),
      ]);
  ...
  // NATIVE Electron system audio capture
  async captureSystemAudioEnhanced() {
    if ($DebugTestMode) {
      console.log(
        "🎵 [025] METHOD 1: NATIVE Electron system audio starting..."
      );
    }

    // Method 1: Native Electron desktopCapturer
    try {
      if ($DebugTestMode) {
        console.log("🎵 [026] Attempting NATIVE Electron screen capture...");
      }

      if (!window.electronAPI?.getScreenCaptureSource) {
        throw new Error("getScreenCaptureSource not available");
      }

      const source = await window.electronAPI.getScreenCaptureSource();
      if (!source) {
        throw new Error("No screen capture source available");
      }

      if ($DebugTestMode) {
        console.log("🎵 [027] Screen source obtained:", source.name, source.id);
      }
      ...
      if ($DebugTestMode) {
        console.log("🎵 [028] ✅ NATIVE system audio captured successfully!");
        console.log("🎵 [029] Stream tracks:", stream.getAudioTracks().length);
      }
      ...
      if ($DebugTestMode) {
        console.log("🎵 [030] ❌ Native capture failed:", error.message);
        console.log("🎵 [031] Falling back to browser methods...");
      }

      // Fallback to original browser methods
      let stream = await this.safeCaptureDisplayMedia();
      if (stream) {
        if ($DebugTestMode) {
          console.log("🎵 [032] Fallback method succeeded");
        }
        return stream;
      }

      if ($DebugTestMode) {
        console.log("🎵 [033] All methods failed, returning null");
      }
      return null;
    }
  }
  ...
  async captureWindowsSystemAudio() {
    if ($DebugTestMode) {
      console.log("🎵 [044] Windows NATIVE system audio capture starting...");
    }

    try {
      // Method 1: Native Electron Windows capture
      if (window.electronAPI?.getScreenCaptureSource) {
        if ($DebugTestMode) {
          console.log("🎵 [045] Using native Windows screen capture...");
        }

        const source = await window.electronAPI.getScreenCaptureSource();
        if (source) {
          if ($DebugTestMode) {
            console.log("🎵 [046] Windows screen source:", source.name);
          }

          // FIXED: Safe constraints without Chrome-specific options
          const stream = await Promise.race([
            navigator.mediaDevices.getUserMedia({
              audio: {
                // REMOVED ALL mandatory/chrome constraints
                echoCancellation: false,
                noiseSuppression: false,
                autoGainControl: false,
                sampleRate: 16000,
            ...
            new Promise((_, reject) =>
              setTimeout(() => reject(new Error("Audio capture timeout")), 3000)
            ),
          ]);

          if (stream && stream.getAudioTracks().length > 0) {
            if ($DebugTestMode) {
              console.log("🎵 [047] ✅ Windows native capture successful!");
            }
            return stream;
          }
        }
      ...
      // Method 2: Fallback to display media
      ...
      if ($DebugTestMode) {
        console.warn("🎵 [048] Windows capture failed:", error.message);
      }
    }
  ...
  async stop() {
    ...
    try {
      ...
      for (const context of this.resources.contexts) {
        try {
      ...
      console.log("✅ Enhanced audio capture stopped safely");
    } catch (error) {
