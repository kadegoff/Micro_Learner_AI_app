...
function startStreamMonitor(stream, onStreamLost = null) {
  ...
  let monitorInterval = setInterval(() => {
    ...
    // Optional: Log that stream is still good (comment out if too verbose)
    // if ($DebugTestMode) {
    //   console.log("🎵 ✅ Stream monitor: Stream still active");
    // }
  }, 100); // Every 100ms
  ...
  try {
    ...
    if (result && result.stream) {
      ...
      const monitorId = startStreamMonitor(result.stream, (reason) => {
        ...
        // Optional: Auto-restart capture on stream loss
        // setTimeout(() => captureSystemAudioFixed(), 1000);
      });
