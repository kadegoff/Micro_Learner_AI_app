...
setupMessageProcessor: function () {
  ...
  const checkRestoreMessages = () => {
    try {
      ...
      if (messageData) {
        console.log("📨 AI: Found control_message in localStorage:", messageData);
        try {
          const message = JSON.parse(messageData);
          console.log("📨 AI: Parsed control message:", message);
          console.log("📨 AI: Message age:", Date.now() - message.timestamp, "ms");
          console.log("📨 AI: Message type:", message.type);
          
          if (Date.now() - message.timestamp < 5000) {
            console.log("📨 AI: Message is fresh (< 5 seconds)");
            
            if (message.type === "RESTORE_AI" || message.type === "AI_RESTORE_REQUEST") {
              console.log("🚀 AI: PROCESSING AI RESTORE MESSAGE:", message.type);
              console.log("🚀 AI: Current isMinimized state:", this.isMinimized);
              console.log("🚀 AI: Current restoreInProgress:", this.restoreInProgress);
              console.log("🚀 AI: Current window display:", document.body.style.display);
              console.log("🚀 AI: Current window visibility:", document.body.style.visibility);
      ...
      if (aiRestoreData) {
        ...
        try {
          ...
          if (Date.now() - message.timestamp < 5000) {
            ...
            console.log("📞 AI: CALLING restoreFromMinimized() from ai_restore_message...");
            this.restoreFromMinimized();
            return;
          }
        } catch (parseError) {
          console.error("📨 AI: Error parsing ai_restore_message:", parseError);
        }
      }
      ...
      if (simpleRestore === "true") {
        ...
        console.log("📞 AI: CALLING restoreFromMinimized() from simple flag...");
      ...
      if (showAI === "true") {
        ...
        console.log("📞 AI: CALLING restoreFromMinimized() from show_ai_window...");
        this.restoreFromMinimized();
      }
  ...
  setInterval(() => {
    if (!this.isMinimized && document.body.style.display !== "none") {
      ...
      localStorage.setItem("ai_heartbeat_debug", JSON.stringify(heartbeatData));
    }
  }, 1000); // Every second
  ...
  window.addEventListener("message", (event) => {
    ...
    if (event.data && event.data.source === "control") {
      ...
      if (event.data.type === "RESTORE_AI" || event.data.action === "restore") {
        console.log("🚀 AI: Direct postMessage restore received");
        console.log("📞 AI: CALLING restoreFromMinimized() from postMessage...");
        this.restoreFromMinimized(); // Direct call
      } else {
