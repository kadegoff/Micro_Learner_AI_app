...
handleLargeContent: function (content) {
  ...
  console.log("📄 Detected type:", contentType, "language:", language);
  ...
  if (!this.attachments) {
    this.attachments = [];
    console.log("📄 Initialized attachments array");
  }
  
  // Add to attachments
  this.attachments.push(attachment);
  console.log("📄 Added large content attachment, total attachments:", this.attachments.length);
  
  // Show preview
  this.showLargeContentPreview(attachment);
  
  // Update input placeholder
  const chatInput = document.getElementById("chatInput");
  if (chatInput) {
    chatInput.placeholder = `📄 Document attached (${this.formatFileSize(attachment.size)}) - add your message...`;
  }
}
...
var chatManager = {
  // ENHANCED sendMessage function - Replace your existing one with this
  sendMessage: function (
    ...
    setTimeout(() => {
      ...
      console.log("📤 Attachment summary:", messageData.attachment_summary);
...
generateSmartFilename: function (content, contentType) {
  ...
  if (contentType === 'javascript') {
    ...
    const titleMatch = content.match(/<title[^>]*>([^<]*)<\/title>/i);
    if (titleMatch && titleMatch[1].trim()) {
      baseName = titleMatch[1].trim().replace(/[^a-zA-Z0-9\s]/g, '').replace(/\s+/g, '_').toLowerCase();
    } else {
...
showLargeContentPreview: function (attachment) {
  ...
  const contentPreview = attachment.content.length > 300 
  ...
  preview.innerHTML = `
    <div class="preview-header">
      <div class="preview-info">
        ...
        <div class="preview-details">
          <span class="preview-filename">${this.escapeHtml(attachment.filename)}</span>
          <div class="preview-meta">
            <span class="preview-size">${this.formatFileSize(attachment.size)}</span>
      ...
      <div class="preview-actions">
        <button class="preview-btn view-btn" onclick="chatInputManager.viewLargeContent('${attachment.filename}')">
          👁️ View
        </button>
    ...
    <div class="preview-content">
      <pre><code class="language-${attachment.language || 'text'}">${this.escapeHtml(contentPreview)}</code></pre>
    </div>
  `;
...
removeLargeContentPreview: function () {
  const preview = document.querySelector('.large-content-preview');
  if (preview) {
    preview.remove();
    console.log("🗑️ Large content preview removed");
  }
  
  // Remove from attachments
  if (this.attachments) {
    this.attachments = this.attachments.filter(att => att.type !== 'large_content');
    console.log("🗑️ Removed large content from attachments");
  }
...
viewLargeContent: function (filename) {
  const attachment = this.attachments?.find(att => att.filename === filename);
  if (!attachment) {
...
var chatInputManager = {
  ...
  addScreenshotAttachment: function (attachment) {
    ...
    if (!attachment) {
      ...
      this.showErrorNotification("No screenshot data received");
      return;
    }

    if (!attachment.data) {
      console.error("📸 No image data in attachment");
      this.showErrorNotification("Screenshot has no image data");
      return;
    }

    if (typeof attachment.data !== "string") {
      console.error("📸 Image data is not a string:", typeof attachment.data);
      this.showErrorNotification("Invalid screenshot data format");
      return;
    }

    if (attachment.data.length === 0) {
      console.error("📸 Image data is empty");
      this.showErrorNotification("Screenshot data is empty");
      return;
    }

    console.log("📸 ✅ All validations passed, proceeding with preview");

    // ✅ ENHANCED: Ensure required properties
    if (!attachment.filename) {
      attachment.filename = `screenshot_${Date.now()}.png`;
      console.log("📸 Generated filename:", attachment.filename);
    }

    if (!attachment.size) {
      // Estimate size from base64 data
      attachment.size = Math.round((attachment.data.length * 3) / 4);
      console.log("📸 Estimated size:", attachment.size);
    }
    ...
    // ✅ ENHANCED: Show preview with better error handling
    try {
      this.showScreenshotPreview(attachment);
      console.log("📸 ✅ Screenshot preview created successfully");
    } catch (error) {
      console.error("📸 Failed to show main preview:", error);
      try {
        this.showFallbackPreview(attachment);
        console.log("📸 ✅ Fallback preview created");
      } catch (fallbackError) {
        console.error("📸 Fallback preview also failed:", fallbackError);
        this.showErrorNotification(
          `Failed to show screenshot: ${error.message}`
        );
      }
    }
  },
  ...
  removeScreenshotPreview: function () {
    ...
    // Remove attachment
    if (this.attachments) {
      this.attachments = this.attachments.filter((att) => att.type !== "image");
    }
    ...
    // Remove send button active state if no attachments
    ...
    console.log("📸 Screenshot preview removed");
  },
 ...
 setupPasteHandler: function () {
  ...
  chatInput.addEventListener("paste", (e) => {
    ...
    this.handlePaste(e); // ← NOW CALLS REAL FUNCTION!
  });

  console.log("✅ Paste handler attached to chat input");
},

// ✅ ADD: The actual handlePaste function
handlePaste: function (e) {
  const clipboardData = e.clipboardData || window.clipboardData;
  if (!clipboardData) return;
  ...
  try {
    ...
    setTimeout(() => {
      ...
      if (chatInput) {
        ...
        console.log("✅ Max direct input threshold:", chatInputManager.maxDirectInput);
      } else {
