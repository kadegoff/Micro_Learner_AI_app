...
function installPip() {
  ...
  if (platform === "win32") {
    ...
    exec("curl https://bootstrap.pypa.io/get-pip.py -o get-pip.py && python get-pip.py", (error) => {
      if (error) {
        console.error("Failed to install pip. Please install Python and pip manually.");
        process.exit(1);
      }
      installDependencies();
    });
