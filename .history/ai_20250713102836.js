...
var chatInputManager = {
  ...
  handleFileAttachments: function (files) {
    ...
    Array.from(files).forEach((file) => {
      const attachmentId = `file_${++this.attachmentCounter}_${Date.now()}`;
      ...
      if (this.isTextFile(file)) {
        ...
        reader.onload = (e) => {
          ...
          this.createImageAttachment(file, imageData, attachmentId);
        };
        reader.readAsDataURL(file);
      } else {
        // Handle other file types (PDFs, etc.)
        this.createGenericFileAttachment(file, attachmentId);
      }
    });
  },
