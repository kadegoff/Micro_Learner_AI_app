  ...
  sendMessage: function (
    ...
    setTimeout(() => {
      ...
      if (Environment.isElectron) {
        ...
        // Listen for any response to mark as received
        const responseHandler = () => {
          responseReceived = true;
        };

        // Temporarily listen for responses
        if (window.electronAPI.onNewAIResponse) {
          window.electronAPI.onNewAIResponse(responseHandler);
        }

        // Send via Electron
        window.electronAPI.sendChatMessage(messageData);

        // Set a timeout to fallback to direct API
        setTimeout(() => {
          if (!responseReceived) {
            console.warn(
              "⏱️ No response after 5 seconds, falling back to direct API"
            );
...
// Add these properties to your existing aiWindowState object
var aiWindowState = {
  isMinimized: false,
  isMaximized: false, // ADD THIS LINE

  // ADD THESE NEW METHODS for maximize functionality
  ...
  updateMaximizeButton: function () {
    ...
    if (maximizeBtn) {
      ...
      // Update button appearance for Windows
  ...
  handleResize: function () {
    // Detect if window is maximized based on screen size
    const isFullscreen =
  ...
  // ENHANCED: Better communication setup that includes maximize handling
  setupCommunication: function () {
    ...
    // Set up resize handler for maximize detection
    ...
    // Listen for restore messages from control center
    window.addEventListener("message", (event) => {
      console.log("📨 AI window received message:", event.data);
      if (event.data.type === "RESTORE_AI" && event.data.source === "control") {
        console.log("🔄 Restore message received via postMessage");
        this.restoreFromMinimized();
      }
    });

    // Listen for custom events
    window.addEventListener("controlRestoreAI", (event) => {
      console.log("🎯 AI window received custom restore event:", event.detail);
      this.restoreFromMinimized();
    });

    // ENHANCED: Monitor localStorage for control messages more aggressively
    const checkControlMessages = () => {
      try {
        const messageData = localStorage.getItem("control_message");
        if (messageData) {
          const message = JSON.parse(messageData);
          console.log("📨 AI found control message:", message);

          if (
            Date.now() - message.timestamp < 10000 && // Extended timeout
            message.type === "RESTORE_AI" &&
            message.source === "control"
          ) {
            console.log("🔄 Restore message received via localStorage");
            this.restoreFromMinimized();

            // Clear the message to prevent duplicate processing
            localStorage.removeItem("control_message");
          }
        }
      } catch (error) {
    ...
    // Check for control messages very frequently
    setInterval(checkControlMessages, 100); // Every 100ms for responsiveness

    // ENHANCED: Check startup state more thoroughly
    ...
    // Check if ANY indicator shows minimized state
    ...
    if (shouldStartMinimized) {
      ...
      // Ensure all flags are set consistently
      ...
      // Notify control center about state
      setTimeout(() => {
      ...
      // Notify that we're visible
      setTimeout(() => {
        this.notifyControlCenter("AI_RESTORED");
      }, 500);
    }

    // ADDITIONAL: Set up periodic state broadcasting
    setInterval(() => {
      // Broadcast current state every 5 seconds to keep control panel synced
      const currentState = this.isMinimized ? "AI_MINIMIZED" : "AI_RESTORED";

      // Only broadcast if there's a mismatch
      const controlMinimized = localStorage.getItem("control_ai_minimized");
      const shouldBeMinimized = controlMinimized === "true";

      if (this.isMinimized !== shouldBeMinimized) {
        console.log(
          "🔄 State mismatch detected, broadcasting current state:",
          currentState
        );
        this.notifyControlCenter(currentState);
      }
    }, 5000);
  },

  // FIXED: Enhanced AI window minimization
  minimizeToControl: function () {
    console.log("🔽 minimizeToControl called");

    // Disable any window operation blocking
    if (window.preventMaximize) window.preventMaximize = false;
    if (window.preventMinimize) window.preventMinimize = false;

    // Show notification first
    this.showMinimizeNotification();

    // IMMEDIATELY set state and notify - don't wait
    this.isMinimized = true;
    console.log("🔽 Set isMinimized to true");

    // Notify BEFORE hiding the window
    this.notifyControlCenter("AI_MINIMIZING");
    console.log("🔽 Sent AI_MINIMIZING notification");

    // Add minimizing class for animation
    document.body.classList.add("minimizing");
    console.log("🔽 Added minimizing class");

    // Force hide the window immediately if Electron API is available
    if (window.electronAPI && window.electronAPI.minimizeWindow) {
      console.log("🔽 Using Electron minimize");
      window.electronAPI.minimizeWindow();

      // Notify minimized state immediately after Electron call
      setTimeout(() => {
        this.notifyControlCenter("AI_MINIMIZED");
        console.log("🔽 Sent AI_MINIMIZED notification (Electron)");
      }, 100);
    } else {
      console.log("🔽 Using DOM hide method");

      // Hide the window with DOM methods
      document.body.style.display = "none";
      document.body.style.visibility = "hidden";
      window.blur();

      console.log("🔽 Window hidden via DOM");

      // Clean up and notify
      document.body.classList.remove("minimizing");

      // Store minimized state IMMEDIATELY
      sessionStorage.setItem("ai_minimized_this_session", "true");
      localStorage.setItem("ai_minimized", "true");
      localStorage.setItem("control_ai_minimized", "true");

      // Notify control center that minimize is complete
      this.notifyControlCenter("AI_MINIMIZED");
      console.log("🔽 Sent AI_MINIMIZED notification (DOM)");
    }

    // ADDITIONAL: Force multiple notifications to ensure delivery
    setTimeout(() => {
      this.notifyControlCenter("AI_MINIMIZED");
      console.log("🔽 Sent delayed AI_MINIMIZED notification");
    }, 200);

    setTimeout(() => {
      this.notifyControlCenter("AI_MINIMIZED");
      console.log("🔽 Sent final AI_MINIMIZED notification");
    }, 500);
  },

  // ENHANCED: Better restore function
  restoreFromMinimized: function () {
    ...
    if (!this.isMinimized) {
      console.log("❌ AI is not minimized, ignoring restore");
      return;
    }

    console.log("🔼 Restoring AI window from minimized state");

    // Notify BEFORE showing the window
    ...
    // Clear minimized state from all storage locations IMMEDIATELY
    sessionStorage.removeItem("ai_minimized_this_session");
    localStorage.removeItem("ai_minimized");
    localStorage.removeItem("control_ai_minimized");
    console.log("🧹 Cleared all minimized state flags");

    // After animation completes, finalize restore
    setTimeout(() => {
      ...
      this.notifyControlCenter("AI_RESTORED");
      console.log("✅ AI window restore complete");
    }, 400);

    // Send additional notifications to ensure delivery
    setTimeout(() => {
      this.notifyControlCenter("AI_RESTORED");
    }, 600);
  },

  // FIXED: Enhanced notification system for AI window
  notifyControlCenter: function (messageType) {
    console.log("🔔 AI notifying control center:", messageType);

    try {
      // CRITICAL: Set localStorage flags IMMEDIATELY and CONSISTENTLY
      if (messageType === "AI_MINIMIZING") {
        localStorage.setItem("control_ai_minimized", "true");
        localStorage.setItem("ai_minimized", "true");
        console.log("✅ Set AI minimizing flags");
      } else if (messageType === "AI_MINIMIZED") {
        localStorage.setItem("control_ai_minimized", "true");
        localStorage.setItem("ai_minimized", "true");
        console.log("✅ Set AI minimized flags");
      } else if (
        ...
        localStorage.removeItem("control_ai_minimized");
        localStorage.removeItem("ai_minimized");
        console.log("✅ Cleared AI minimized flags");
      }
      ...
      // Method 1: Direct localStorage communication (most reliable)
      localStorage.setItem("ai_message", JSON.stringify(messageData));
      console.log("📤 Stored ai_message in localStorage");

      // Method 2: postMessage (for same-origin windows)
      try {
      ...
      try {
        window.dispatchEvent(
          new CustomEvent("aiWindowStateChange", {
            detail: messageData,
            bubbles: true,
          })
        );
      } catch (e) {
        console.warn("Custom event failed:", e);
      }

      // Method 4: Force control panel state check via special flag
      localStorage.setItem("force_control_check", Date.now().toString());

      console.log("📤 Sent all AI notification methods for:", messageType);

      // Clear message after delay but keep flags
      setTimeout(() => {
