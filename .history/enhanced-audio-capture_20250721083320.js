...
class EnhancedAudioCapture {
  ...
  async getBestAudioStream() {
    ...
    try {
      ...
      if (mixedStream) {
        ...
        // ✅ NEW: Start monitoring the stream automatically
        this.startStreamMonitoring(mixedStream);
  ...
  async stop() {
    ...
    try {
      // ✅ NEW: Stop stream monitoring first
      if (this.streamMonitor) {
        this.streamMonitor.stopMonitoring();
      }

      // Clean up mixer components
  ...
  // ✅ NEW: Start stream monitoring with callbacks
  startStreamMonitoring(stream) {
    if ($DebugTestMode) {
      console.log("🎵 [CAPTURE] Starting stream monitoring...");
    }

    this.streamMonitor.startMonitoring(stream, {
      onValid: (health) => {
        // Stream is healthy - optional callback
        if ($DebugTestMode && Date.now() % 5000 < 100) {
          // Log every 5 seconds
          console.log("🎵 [CAPTURE] Stream healthy:", health.summary);
        }
      },

      onInvalid: (health, failures) => {
        console.warn(
          `🎵 [CAPTURE] ⚠️ Stream invalid (${failures} failures):`,
          health.issues
        );
      },

      onEnded: (health) => {
        console.error("🎵 [CAPTURE] ❌ Stream ended:", health.summary);
        // Automatically try to reconnect when stream ends
        this.handleStreamReconnect();
      },

      onReconnectNeeded: (health, failures) => {
        console.log(
          "🎵 [CAPTURE] 🔄 Triggering auto-reconnect after",
          failures,
          "failures"
        );
        this.handleStreamReconnect();
  ...
  // ✅ NEW: Handle automatic stream reconnection
  async handleStreamReconnect() {
    if ($DebugTestMode) {
      console.log(
        "🎵 [CAPTURE] 🔄 Attempting automatic stream reconnection..."
      );
    }
    ...
    try {
      ...
      // Reset destroyed flag since we're reconnecting
      this.destroyed = false;

      // Attempt to get new stream
      const newResult = await this.getBestAudioStream();

      if (newResult && newResult.stream) {
        console.log("🎵 [CAPTURE] ✅ Stream reconnected successfully");

        // Notify main app of reconnection success
        if (window.onAudioReconnected) {
          window.onAudioReconnected(newResult);
        }
      } else {
        throw new Error("Failed to obtain new stream");
      }
    } catch (error) {
      console.error("🎵 [CAPTURE] ❌ Stream reconnection failed:", error);

      // Notify main app of reconnection failure
      if (window.onAudioReconnectFailed) {
        window.onAudioReconnectFailed(error);
      }
    }
  }

  // ✅ NEW: Manual method to stop monitoring
  ...
  getMonitoringStatus() {
    return this.streamMonitor ? this.streamMonitor.getStatus() : null;
  }
...
// In main-renderer.js or wherever you use the audio capture
async function startAudioCapture() {
  const result = await globalAudioCapture.getBestAudioStream();
  if (result && result.stream) {
    console.log("✅ Audio stream obtained and monitoring started");

    // ✅ NEW: Set up global callbacks for reconnection events
    window.onAudioReconnected = (newStreamResult) => {
      console.log("🎵 [MAIN] Stream reconnected, updating UI...");
      // Update your UI or restart recognition with new stream
      // For example, if you have speech recognition:
      // restartSpeechRecognition(newStreamResult.stream);
    };

    window.onAudioReconnectFailed = (error) => {
      console.error("🎵 [MAIN] Auto-reconnect failed:", error);
      // Show error to user, maybe provide manual retry button
      // updateUIWithError("Audio connection lost. Please try again.");
    };

    // Use the stream...
    // ✅ NOW: Stream is automatically monitored every 100ms
  }
}

// ✅ NEW: Optional - Manual monitoring control
function toggleStreamMonitoring(enabled) {
  if (!globalAudioCapture) return;

  if (enabled) {
    if (globalAudioCapture.finalStream) {
      globalAudioCapture.startStreamMonitoring(globalAudioCapture.finalStream);
    }
  } else {
    globalAudioCapture.stopStreamMonitoring();
  }
}

// ✅ NEW: Get current monitoring status
function getAudioMonitoringStatus() {
  return globalAudioCapture ? globalAudioCapture.getMonitoringStatus() : null;
}
