  ...
  try {
    ...
    if (screenshotData) {
      ...
      if (
        ...
        // Add to chat input
        chatInputManager.addScreenshotAttachment(message.attachment);
      }
    }
  } catch (error) {
    console.warn("Error checking screenshot attachments:", error);
  }
};
...
`;
  ...
  showScreenshotPreview: function (attachment) {
    ...
    // RELAXED VALIDATION - just check if it's not empty
    ...
    // OPTIONAL: More lenient base64 check
    if (!/^[A-Za-z0-9+/=\s]*$/.test(cleanData)) {
...
`;
    ...
    // Find the chat container and INSERT the preview (this was also missing!)
    const chatContainer =
    ...
    if (chatContainer) {
      ...
      console.error("📸 ❌ Could not find chat container to insert preview");
    }
