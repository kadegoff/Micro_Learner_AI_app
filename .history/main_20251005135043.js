...
function startVoskProcess() {
  ...
  try {
    // 🔥 CRITICAL FIX: Use spawn directly with proper hiding
    const { spawn } = require("child_process");

    const pythonProcess = spawn(
      "python",
      ["-u", "vosk_realtime.py", modelPath],
      {
        cwd: __dirname,
        // ✅ CRITICAL: Hide console windows on Windows
        windowsHide: true,
        // ✅ Prevent creating new console window
        detached: false,
        // ✅ Use pipes instead of inheriting console
        stdio: ["pipe", "pipe", "pipe"],
        // ✅ Windows-specific: don't create console window
        shell: false,
        // ✅ Hide the process completely
        windowsVerbatimArguments: false,
      }
    );

    // Convert to PythonShell-like interface for compatibility
    voskProcess = {
    ...
    pythonProcess.stdout.on("data", (data) => {
      ...
      lines.forEach((line) => {
        if (line.trim()) {
          if ($DebugTestMode) {
            logToFile("Vosk message received:", line);
          }
          ...
          try {
            ...
            // ... keep your existing message handling code here ...
            // Copy all the message handling logic from your original function
          } catch (parseError) {
            if ($DebugTestMode) {
              logToFile("Failed to parse Vosk output:", parseError);
              logToFile("Raw message was:", line);
            }
          }
        }
      });
    ...
    pythonProcess.stderr.on("data", (data) => {
      if ($DebugTestMode) {
        logToFile("Vosk stderr:", data.toString());
      }
    });
    ...
    pythonProcess.on("close", (code) => {
      if ($DebugTestMode) {
        logToFile(`Vosk process closed with code ${code}`);
      }
      voskProcess = null;
    });
  } catch (error) {
