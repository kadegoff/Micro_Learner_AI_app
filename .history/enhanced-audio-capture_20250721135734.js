...
class EnhancedAudioCapture {
  ...
  async createMixedStream() {
    if ($DebugTestMode) {
      console.log(
        "🎵 Creating FIXED mixed stream with BOTH system + microphone..."
      );
    }
    ...
    try {
      ...
      // ✅ FIXED: Handle different combinations with BETTER microphone priority
      if (systemStream && micStream) {
        console.log(
          "🎵 ✅ MIXING BOTH: System audio + Microphone with FIXED levels!"
        );
        ...
        // ✅ CRITICAL FIX: Ensure audio context is running
        if (audioContext.state === "suspended") {
          await audioContext.resume();
          console.log("🎵 ✅ Audio context resumed for mixing");
        }

        const systemSource = audioContext.createMediaStreamSource(systemStream);
        const micSource = audioContext.createMediaStreamSource(micStream);

        // ✅ FIXED: Create individual gain nodes with PROPER levels
        const systemGain = audioContext.createGain();
        const micGain = audioContext.createGain();

        // ✅ CRITICAL FIX: Give microphone MUCH higher priority and system audio LOWER
        systemGain.gain.value = 0.1; // System audio at 10% (was 20%)
        micGain.gain.value = 2.0; // Microphone at 200% (was 100%)

        console.log("🎵 🔧 FIXED GAIN LEVELS:");
        console.log("🎵    System gain: 0.1 (10%)");
        console.log("🎵    Microphone gain: 2.0 (200%)");

        // ✅ FIXED: Use explicit channel merging for better control
        const merger = audioContext.createChannelMerger(2);
        const destination = audioContext.createMediaStreamDestination();

        // Connect with explicit routing
        systemSource.connect(systemGain);
        micSource.connect(micGain);

        // ✅ OPTION 1: Direct connection (simpler, might work better)
        systemGain.connect(destination);
        micGain.connect(destination);

        // ✅ ALTERNATIVE OPTION 2: Channel merger (uncomment if needed)
        // systemGain.connect(merger, 0, 0); // System to left channel
        // micGain.connect(merger, 0, 1);    // Mic to right channel
        ...
        // ✅ DIAGNOSTIC: Test the mixed stream
        console.log("🎵 🧪 TESTING mixed stream...");
        const mixedTracks = destination.stream.getAudioTracks();
        console.log("🎵 Mixed stream tracks:", mixedTracks.length);
        if (mixedTracks.length > 0) {
          console.log("🎵 Mixed track label:", mixedTracks[0].label);
          console.log("🎵 Mixed track state:", mixedTracks[0].readyState);
          console.log("🎵 Mixed track enabled:", mixedTracks[0].enabled);
        }

        return destination.stream;
      }

      // ✅ FIXED: Prefer microphone over system audio when only one available
      else if (micStream) {
        console.log("🎵 ✅ Using MICROPHONE ONLY (preferred)");

        // ✅ BOOST: Even for mic-only, apply some gain boost
        const audioContext = new (window.AudioContext ||
          window.webkitAudioContext)({
          sampleRate: 16000,
        });
        ...
        micGain.gain.value = 1.5; // 150% boost for mic-only
        console.log("🎵 🔧 Microphone-only gain: 1.5 (150%)");
