...
if (result.method === "native-desktop-capturer" && result.source) {
  if ($DebugTestMode) {
    console.log("🎵 [020] SAFE APPROACH: Using getDisplayMedia instead of getUserMedia");
  ...
  try {
    ...
    if ($DebugTestMode) {
      console.log("🎵 [022] Calling getDisplayMedia with safe constraints...");
    }
...
if (stream && stream.getAudioTracks().length > 0) {
  stream.getVideoTracks().forEach((track) => track.stop());
      ...
      if ($DebugTestMode) {
        ...
        console.log("🎵 [026] Total audio tracks:", stream.getAudioTracks().length);
      }
