...
async function startListening() {
  ...
  try {
    ...
    try {
      ...
      console.error("❌ Enhanced audio capture failed:", enhancedError.message);
      throw new Error(
  ...
  try {
    ...
    if (stream) {
      ...
      const testInterval = setInterval(() => {
        ...
        console.log(`Audio test sample ${sampleCount}: ${average.toFixed(2)}`);
...
window.startAudioMonitor = function (intervalMs = 2000) {
  ...
  window.audioMonitorInterval = setInterval(() => {
    ...
    if (lastGoodState && lastGoodState !== currentStateKey) {
