...
var chatInputManager = {
  ...
  createGenericAttachmentElement: function (element, attachment) {
    ...
    <div class="attachment-preview generic-preview">
      <div class="attachment-content" style="position: relative;">
        <div style="display: flex; align-items: center; gap: 8px; padding: 8px; background: #a1a1a1; border-radius: 4px;">
  ...
  createContentAttachmentElement: function (element, attachment) {
    ...
    <div class="attachment-preview content-preview">
      <div class="attachment-content" style="position: relative;">
        <pre style="font-size: 12px; background: #f5f5f5; padding: 8px; border-radius: 4px; overflow: hidden; margin: 0; max-height: 200px; overflow-y: auto;"><code>${this.escapeHtml(
          contentPreview
        )}</code></pre>
        <button class="remove-attachment" onclick="chatInputManager.removeAttachment('${
          attachment.id
  ...
  setupEventListeners: function () {
    ...
    if (attachmentButton && attachmentInput) {
      ...
      attachmentButton.replaceWith(attachmentButton.cloneNode(true));
      attachmentInput.replaceWith(attachmentInput.cloneNode(true));

      // Get fresh references
      const newAttachmentButton = document.getElementById("attachmentButton");
      const newAttachmentInput = document.getElementById("attachmentInput");

      // FIXED: Proper click handling with timing
      newAttachmentButton.addEventListener("click", (e) => {
        e.preventDefault();
        e.stopPropagation();
        console.log("📎 Attachment button clicked");

        // FIXED: Reset value BEFORE opening dialog
        newAttachmentInput.value = "";

        // FIXED: Small delay ensures value is cleared
        setTimeout(() => {
          newAttachmentInput.click();
        }, 10);
      });

      // FIXED: Better change handler with error handling
      newAttachmentInput.addEventListener("change", (e) => {
        e.preventDefault();
        e.stopPropagation();
        ...
        if (files && files.length > 0) {
          try {
            // FIXED: Process files with error handling
            this.handleFileAttachments(files);
            console.log("📎 ✅ Files processed successfully");
          } catch (error) {
            console.error("📎 ❌ Error processing files:", error);
            this.showErrorNotification("Failed to process selected files");
          }
        }

        // FIXED: Don't clear immediately - let browser handle it
        // Clearing happens in button handler on next click
      });
    }
  },
  ...
  handleFileAttachments: function (files) {
    console.log("📎 Processing file attachments:", files.length);

    // FIXED: Show processing feedback
    const attachmentButton = document.getElementById("attachmentButton");
    if (attachmentButton) {
    ...
    Array.from(files).forEach((file, index) => {
      ...
      try {
        ...
        this.showErrorNotification(`Failed to process ${file.name}`);
      }
    });
  },
