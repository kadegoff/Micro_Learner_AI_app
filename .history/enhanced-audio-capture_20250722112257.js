...
async createMixedStream() {
  if ($DebugTestMode) {
    console.log("🎵 Creating TRULY MIXED stream with separate device captures...");
  }

  try {
    // STEP 1: Get Stereo Mix specifically (for system audio)
    ...
    // STEP 2: Get REAL microphone (not Stereo Mix)
    let micStream = null;
    const realMicrophones = devices.filter(device => 
      device.kind === 'audioinput' && 
    ...
    if (realMicrophones.length > 0) {
      console.log("✅ Found real microphone:", realMicrophones[0].label);
      micStream = await navigator.mediaDevices.getUserMedia({
        audio: {
          deviceId: { exact: realMicrophones[0].deviceId },
          echoCancellation: true,
          noiseSuppression: true,
          autoGainControl: true
        }
    ...
    // STEP 3: Handle different combinations
    if (systemStream && micStream) {
      console.log("🎉 BOTH STREAMS CAPTURED ON SEPARATE DEVICES!");
      console.log("🎵 System audio device:", systemStream.getAudioTracks()[0].label);
      console.log("🎤 Microphone device:", micStream.getAudioTracks()[0].label);
      ...
      // Better mixing levels
      systemGain.gain.value = 0.7;  // System audio at 70%
      micGain.gain.value = 1.5;     // Microphone at 150%
      ...
      console.log("✅ Windows should now show BOTH devices as active!");
      return destination.stream;
      
    } else if (systemStream) {
      console.log("⚠️ Only system audio available");
      this.systemStream = systemStream;
      this.finalStream = systemStream;
      return systemStream;
      
    } else if (micStream) {
      console.log("⚠️ Only microphone available");
      this.micStream = micStream;
    ...
    console.error("❌ createMixedStream failed:", error);
    throw error;
  }
}
