...
class EnhancedAudioCapture {
  ...
  async createMixedStream() {
    if ($DebugTestMode) {
      console.log(
        "🎵 Creating TRULY MIXED stream with separate device captures..."
      );
    }
    ...
    try {
      ...
      const stereoMixDevice = devices.find(
        (device) =>
      ...
      let micStream = null;
      const realMicrophones = devices.filter(
        (device) =>
          device.kind === "audioinput" &&
      ...
      if (systemStream && micStream) {
        ...
        console.log(
          "🎵 System audio device:",
          systemStream.getAudioTracks()[0].label
        );
        console.log(
          "🎤 Microphone device:",
          micStream.getAudioTracks()[0].label
        );
