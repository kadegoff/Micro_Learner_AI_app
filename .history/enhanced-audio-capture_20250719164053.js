...
class EnhancedAudioCapture {
  ...
  async captureWithDesktopCapturer() {
    ...
    try {
      ...
      // If we get sources, try to use them
      if (
        result.sources &&
        ...
        if ($DebugTestMode) {
          ...
          result.sources.forEach((source, index) => {
            console.log(
              `🎵 [021.${index}] Source: ${source.name} (${source.id})`
            );
          });
        }
        ...
        for (let i = 0; i < result.sources.length; i++) {
          ...
          try {
            if ($DebugTestMode) {
              console.log(`🎵 [022.${i}] Trying source: ${source.name}`);
            }

            // ✅ FIXED: Use getDisplayMedia for screen capture with audio
            const stream = await navigator.mediaDevices.getDisplayMedia({
              video: {
                width: { ideal: 1 },
                height: { ideal: 1 },
            ...
            if (stream && stream.getAudioTracks().length > 0) {
              if ($DebugTestMode) {
                console.log(
                  `🎵 [023.${i}] ✅ Desktop capturer successful with source: ${source.name}`
                );
              }
              ...
              if ($DebugTestMode) {
                console.log(
                  `🎵 [024.${i}] ⚠️ Source ${source.name} has no audio tracks`
                );
              }
              if (stream) {
            ...
            if ($DebugTestMode) {
              console.log(
                `🎵 [025.${i}] ⚠️ Source ${source.name} failed:`,
                sourceError.message
              );
            }
          }
        ...
        if ($DebugTestMode) {
          console.log("🎵 [026] ❌ All desktop sources failed");
        }
        return null;
      }

      // If no sources in result, fall back to direct display media
      if ($DebugTestMode) {
        console.log(
          "🎵 [027] No sources returned, trying direct display media..."
        );
      }
      ...
      if (directStream && directStream.getAudioTracks().length > 0) {
        if ($DebugTestMode) {
          console.log("🎵 [028] ✅ Direct display media successful");
        }
        // Stop video track, keep audio
        directStream.getVideoTracks().forEach((track) => track.stop());
        return directStream;
      }

      if ($DebugTestMode) {
        console.log("🎵 [029] ❌ Direct display media has no audio");
      }
      if (directStream) {
