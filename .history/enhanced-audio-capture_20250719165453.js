...
class EnhancedAudioCapture {
  ...
  async captureWithDesktopCapturer() {
    if ($DebugTestMode) {
      console.log("🎵 [013] METHOD 1: CRASH-SAFE Desktop Capturer starting...");
    }
    ...
    try {
      ...
      // Use the electronAPI method with timeout protection
      ...
      // ✅ CRITICAL FIX: Instead of using getUserMedia with chromeMediaSource,
      // use getDisplayMedia which is much more stable
      if (result.method === "native-desktop-capturer" && result.source) {
        if ($DebugTestMode) {
          console.log(
            "🎵 [020] Using SAFE getDisplayMedia instead of getUserMedia"
          );
          console.log("🎵 [021] Source info available but using safer method");
        }

        try {
          // ✅ SAFER APPROACH: Use getDisplayMedia instead of getUserMedia with chromeMediaSource
          ...
          if ($DebugTestMode) {
            console.log("🎵 [022] SAFE getDisplayMedia completed successfully");
          }
          ...
          if (stream && stream.getAudioTracks().length > 0) {
            ...
            if ($DebugTestMode) {
              console.log("🎵 [023] ✅ SAFE Desktop capturer successful!");
              console.log("🎵 [024] Audio track label:", audioTrack.label);
              console.log(
                "🎵 [025] Audio track settings:",
            ...
            // ✅ IMPORTANT: Stop video tracks immediately to save resources
            stream.getVideoTracks().forEach((track) => track.stop());

            // ✅ VALIDATE: Check if this looks like actual system audio, not microphone
            ...
            this.currentMethod = "Desktop Capturer (Safe Mode)";
            return stream;
          } else {
            if ($DebugTestMode) {
          ...
          if (streamError.message.includes("timeout")) {
            console.log("🎵 [031] ⚠️ Timeout - system may be slow to respond");
          } else if (streamError.message.includes("NotAllowedError")) {
            console.log("🎵 [032] ⚠️ Permission denied for desktop capture");
          } else if (streamError.message.includes("NotFoundError")) {
            console.log("🎵 [033] ⚠️ Desktop capture source not found");
          }
      ...
      // ✅ HANDLE: Multiple sources array (fallback for older implementations)
        ...
        if ($DebugTestMode) {
          ...
          console.log(
            "🎵 [035] Using SAFE fallback method for multiple sources"
          );
        }

        // For multiple sources, also use the safer getDisplayMedia approach
        try {
          const stream = await Promise.race([
            navigator.mediaDevices.getDisplayMedia({
              video: {
            ...
            new Promise((_, reject) =>
              setTimeout(() => reject(new Error("Multi-source timeout")), 5000)
            ),
          ]);

          if (stream && stream.getAudioTracks().length > 0) {
            if ($DebugTestMode) {
              console.log("🎵 [036] ✅ Multi-source safe method successful");
            }
            stream.getVideoTracks().forEach((track) => track.stop());
            this.systemStream = stream;
            this.currentMethod = "Desktop Capturer (Multi-source Safe)";
            return stream;
          } else {
            if ($DebugTestMode) {
              console.log("🎵 [037] ⚠️ Multi-source safe method has no audio");
            }
            if (stream) {
              stream.getTracks().forEach((track) => track.stop());
            }
          ...
          if ($DebugTestMode) {
            console.log(
              "🎵 [038] ⚠️ Multi-source safe method failed:",
              multiError.message
            );
          }
        }
        ...
        if ($DebugTestMode) {
          console.log("🎵 [039] ❌ All safe methods for sources array failed");
        }
      }
      ...
      // ✅ HANDLE: Unexpected result format
        ...
        if ($DebugTestMode) {
          ...
          console.log(
            "🎵 [041] Expected: success=true with source or sources array"
          );
        }
      }
      ...
      if ($DebugTestMode) {
        console.log(
          "🎵 [042] ❌ SAFE Desktop capturer method failed:",
          error.message
        );
